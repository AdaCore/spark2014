...
pathes written below each node can be used to reconstruct easily the high level
view of the tree.
...
ressemble standard imperative implementations of red black trees except for to two notable
points. The first one, is that we had to comply with the restriction imposed by the
SPARK language, most notably, the forbiddance of pointers (access types in Ada). To
alleviate this restriction, we choose to implement binary trees inside an array, indexes
working as pointers in a memory region. To simplify our implentation, we do not consider
...
doing unitary modfications of the tree while preserving the invariant. The unitary
...
implementation, we resorted to processing the whole memory region as a whole, therefore
...
of the unitary functions can directly modify the internal structure of the tree. The implementation
...
Following our leveled approach, search trees are defined outside of binary tree's implementation.
...
concerns into the serach tree layer. Indeed, balancing functions do not preserve balancedness, as they
are to be called on unbalanced trees, but they do preserve order. Note that implementing the balancing
functions at this level avoids the need of lifting low level tree handling functions such as Plug and
...
colors to a search tree and using balancing functions rebalance the tree after an insertion.
