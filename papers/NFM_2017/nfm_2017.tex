...
\caption{\label{fig-rbt} Type invariant of search trees.}
\end{center}
\end{figure}

In our implemenation, we have only verified the simplest part of the red black tree invariant,
that is, that a red node can only have black children. It is given in Figure~\ref{fig-rbt-inv}.
...
\end{lstlisting}
\end{small}
\caption{\label{fig-rbt-inv} Type invariant of red black trees.}
\end{figure}
...
of Plug is presented in Figure~\ref{fig-binary-body}, it is schematized in Figure~\ref{fig-binary-body2}.
...
example. Its effect is demonstrated in Figure~\ref{fig-search-fun2}.
...
from T1 in F. The proof considers every node in the tree, starting from those closer to the root
as demonstrated in Figure~\ref{fig-proof-ind2}. The loop goes from 1 to the maximum size of any branch
in the forest F. As an
...
is the node to consider to be able to order values stored at arbitrary nodes. An example of this
reasoning is schematized in Figure~\ref{fig-proof-wit2}.
