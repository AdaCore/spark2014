...
{\it ECSS-E-ST-70-01C ``Space engineering -- Spacecraft on-board control procedures''}.
...
All the contracts have been checked by dynamic testing. This phase is quite classical, except for the fact that preconditions and postconditions were also tested. Then, \gnatprove has been applied. The following table provides for each type of checks, their number and the number proved:
...
\end{lstlisting}
\gnatprove is not yet able to verify index checks when accessing an array whose dimension is defined by a record discriminant.
%%, for example:
%%\begin{lstlisting}
