...
\subsubsection{Mode Transition Table}

Our first example is a partial modeling of SRS mode transition table
...
mode to another one. Those conditions are the specific state of the
on-board, the actions of the driver and an explicit priority. For example,
transition from mode Stand By (SB) to mode Full Supervision (FS)
is conditioned by fulfilling condition 10, at priority 7. Condition 10
states that \textit{(valid Train Data is stored on board) AND (MA + SSP +
gradient on-board) AND (no specific mode is required by a Mode
Profile)}.

We have converted the above conditions into \newspark Boolean variables, with
an explicit priority variable, thus modeling all possible transitions of the
transition table. For example, the above condition becomes:
...
function condition_transition_SB_to_FS
  return Boolean
is (condition_10 and priority = 7);
\end{lstlisting}
...
separated by 10 delimiters. Each delimiter of type \SPARK{Delimiter_Entry}
contains the delimiter position and the associated constant value for the
function.
...
We wanted to check requirement §4.6.1.5 of SRS that states that all transitions
are exclusive, even at the same priority. We used \SPARK{Contract_Cases} to
express this requirement. For example, for transitions from mode Stand By:
...
contracts to express the specifications of all subprograms.  For example, for
procedure \SPARK{Restrictive_Merge}, we specified that, given two valid (\ie
with strictly increasing delimiters) step functions \SPARK{SFun1} and
...
transition tables, breaking curve equations, \etc Overall, we were quite
satisfied, as we were able to express most of the requirements in a formal
way. The very expressive data structures of \newspark (records, arrays,
enumerations, \etc) compared to other specification languages were very helpful
and we found that it lead to quite readable specifications.

The second goal was to evaluate the automatic proving capabilities of \newspark
on some parts of the specification.

Regarding the mode transition table, we were not able to prove the
contracts we specified, because the contracts were not
provable! We discovered with this experiment that the transitions given
in the specification table lack details to determine if the transitions
are exclusive or not. Those details are given in other parts of the
specification and it would be a major work to formalize them.

Regarding step functions, we were able to prove all subprogram
...
Detecting improper initialization; Identifying innefective
...
The source code of Tokeneer was proven to be free of runtime
...
The \oldspark version of the code performed proof of functional
behaviour on a limited number of packages. In order to measure the
expressiveness and proving power of the \newspark tools, a specific
package, on which functional behaviour proof had not been attempted,
was selected and then fully augmented with functional behaviour
annotations. Package \emph{admin}, which is the package that was
augmented, contains the state of the administrator of the system and a
set of operations that administrators can perform.
...
``AuditManager'' or a ``SecurityOfficer''. Each of the aforementioned
types of administrators have a set of predefined operations that they
are allowed to perform. Function \emph{OpIsAvailable} takes as input
an administrator and a string that is read from the keyboard and
...
operation is returned, otherwise \emph{NullOp} is returned.
...
--#                   PrivTypes.Guard));
\end{lstlisting}
Here, the postcondition \oldspark annotation serve only as a test
case. It ensures that if a non null operation was returned and if that
...
cases. If there exists some operation in Opt that matches the string
read from the keyboard and this operation is allowed for the current
administrator then the result of function \emph{OpIsAvailable} is this
operation, otherwise, the result is \emph{NullOp}.

The \newspark tools discharge all 24 verification conditions
associated with the augmented \emph{admin} package in a couple of
seconds. The \oldspark tools also prove all properties associated with
the non-augmented \emph{admin} package but a total of 12 user rules is
