...
In this paper we describe our approach to solve those two problems in the
design of the \newspark language and the associated formal verification tool
\gnatprove. We use case studies from three different domains developed in the
context of the \hilite project and discuss the results of using our tools on
those case studies.
...
%% Achieving a full conversion of the entire code base
...
This study will focus on proving the biggest possible subset of the
existing verification using the \newspark toolset. Furthermore, partial
flow analysis will be attempted on a selected subset of files.
...
\begin {itemize}
  ...
  \item Provide feedback and suggesting features/changes that might improve the
    tools.
\end {itemize}

\section{Common Findings and Dissimilarities}

We will discuss the common findings between the three case studies, which may
have an impact on the current development of \newspark, and the
dissimilarities, which may be explained by differences in domains, backgrounds,
processes, \etc

\section{Conclusion}

We will summarize the outcome of the three case studies, and present the
planned future evolutions of \newspark.
