...
\paragraph{Description of the Software:}
...
SPARK has been first experimented on a solar wing management Software (for a spacecraft such as the ATV / Automated Transfer Vehicle).
This piece of code uses a mathematical library which implementation is not in SPARK, implying that it could not be formally proved, but only tested. However, the interface of this mathematical library is in SPARK. The contracts defined on the mathematical library can then be used to prove the application code.
...
\subsubsection{Subprograms not in SPARK}

The origins of subprograms not yet in SPARK are mainly the following:

\begin{itemize}
\item unchecked conversion

The unchecked conversion are used in a library allowing reading external inputs.
All the concerned subprograms are very small and shall be validated by intensive testin because there are out of the perimeter of HiLite and of SPARK.
\item tagged type
...
Our initial objectives were that SPARK should fulfill the following requirements:
