...
\todo[inline]{The authors should improve the introduction to provide a better motivation, namely by explaining the challenges
being addressed and clearly highlighting the main contributions of the work}
...
\todo[inline]{Reviewer 3 finds the section 3 with no benefit to the reader, except to
  recall why aliasing is an issue. He thinks the paper definitely miss some
  information on how SPARK would benefit from this AI, e.g. in terms
  of additional language features while this is the purpose of the section}
...
\centering
  \captionsetup{justification=centering,margin=0.6cm}
   ...
   \caption{A variant of the \var{Swap\_Contents} procedure where aliasing would create problems in SPARK.}
   \label{fig:spark_ex1}
\end{figure}

If SPARK ignored aliasing, it would conclude that this version of procedure \var{Swap\_Contents} always swaps the contents of its parameters \var{X\_Param} and \var{Y\_Param}.
...
derive that the values \var{X\_Param.all} and \var{Y\_Param.all} have been swapped. Similarly, flow analysis would derive wrong data dependencies if possible aliasing is not taken into account.
...
\todo[inline]{Reviewer 3 suggests to move the related work to section 2}
