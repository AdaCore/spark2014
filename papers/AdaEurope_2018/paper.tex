...
The main idea of the \textit{move} operation is a complete transfer of the ownership from the right hand side to the left hand side in an assignment operation, where both left- and right-hand-side objects are owning access variables in the unrestricted state, or the assignment operation is part of a function return.
...
\smallskip
In addition to considering certain assignment statements to be \textit{moves}, we also considers the assignment
operations inherent in passing an \keyword{out} or \keyword{in out} parameter to be moves, as well
as a function return (as mentioned above).
...
\smallskip
The assignment operations that are considered borrowing are those that \textit{initialize} a stand-alone object of an \textit{anonymous} access-to-variable type, or an \keyword{in} parameter of a (named or anonymous) access-to-variable
...
code snippet of Figure \ref{fig:borrow_ex1} as a simple example of borrowing. \var{X} and \var{Y} are both access-to-variable objects. We want to swap the objects designated by the two
...
the contents of their designated objects safely possible. We allow an \keyword{in} parameter or a stand-alone constant of an owning access type to provide read/write access to its designated object to accommodate existing practice in the use of such constant access-to-variable values to update their designated objects.
...
access object and the copy can only be used for gaining read access to the designated object.
...
While simple cases of parameter aliasing like \var{Swap(X,X)} are already illegal in Ada, more complex cases that involve, for example, dynamic array indexing are neither
