...
The three "Prove..." entries run |GNATprove| in the mode given by the project file, or
in the default mode "all" if no mode is specified.
...
|GNATprove| can be run from GNATbench.

A :menuselection:`SPARK` menu is available when :

* "GNATbench Integration with SPARK" feature is installed,
*  gnatprove software is found at GNATbench startup,
*  current selection owns to an Ada project.

To check "GNATbench Integration with SPARK" feature installation use
Help => About Eclipse => Installation Details => Installed Software menu.

To install "GNATbench Integration with SPARK" feature, go to Help => Install New Software... menu.

The :menuselection:`SPARK` menu contains the following entries:
