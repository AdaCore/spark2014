...
|GNATprove| relies on contracts to perform its analysis. Contracts
that users have added on subprograms are implicitly trusted by callers
of the subprograms and are verified when the subprograms' bodies are
analyzed.

But no contracts are compulsory. In the absence of user-provided
contracts |GNATprove| internally synthesizes and uses its own
contracts. However, not all contracts can be automatically
inferred. Only the following contracts are automatically generated by
the tools:
...
* Global
  ...
  .. note ::
     ...
     Omitting a ``Global`` aspect does *NOT* have the same effect as
     having a ``Global => null`` contract. The former results in the
     generation and use of an internal ``Global`` contract while the
     later instructs flow analysis to check that the subprogram
     utilizes no global variables.

* Depends

* Default_Initial_Condition

  |GNATprove| knows when a private type is fully default initialized
  even if a Default_Initial_Condition aspect is not present.

* Post

  For expression functions, the bodies of the functions act as their
  postcondition.

It is important to know which contracts to write in order to achieve
