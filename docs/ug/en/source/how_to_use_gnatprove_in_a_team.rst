...
do a check out of the sources/environment, and use therefore their own
...
Most warnings issued by GNATprove have a specific tag, as detailed in
...
The special tag ``all`` can be used in combination with the three switches
``-W``, ``-A`` and ``-D`` to perform the corresponding action for all warnings
that have a tag. Some warnings do not have a tag and cannot be controlled using
these switches.

In addition, the ``--warnings`` switch implements a separate mechanism to
influence warning messages more globally:

* ``--warnings=off`` suppresses all warnings
* ``--warnings=error`` stops analysis when warnings were issued, similar to errors
...
Some notes about the difference and interaction between ``--warnings`` and the
three other switches. First, ``--warnings=error`` does not promote warnings to
errors, it simply stops analysis after issuing warnings, similar to errors
(unless ``-k`` was provided).  Second, ``--warnings`` acts after the ``-W``,
``-D`` and ``-A`` switches. This means that for example ``--warnings=off`` does
not disable warnings that have been promoted into an error using the ``-D``
switch. On the other hand, ``--warnings=off`` disables even warnings that have
been explicitly enabled using the ``-W`` switch, anywhere on the command-line.
...
Pragma ``Warnings`` interacts with the ``-W``, ``-A`` and ``-D`` switches in a
similar way as the ``--warnings`` switch does. This means that pragma
``Warnings`` can't suppress a warning that has been promoted into an error via
``-D``, but still suppresses warnings that have been explicitly enabled via
``-W``.
...
control) for a project, they can use the ``--replay`` option to reproduce/recheck
...
* [SPARK_ITERABLE_FOR_PROOF]
  ...
  * the function ``Model``
  ...
  [A check is generated by |GNATprove| to verify ``Element`` functions, so there
  are no associated assumptions.]
...
* [ADA_SUBPROGRAMS]
  ...
  * :ref:`Modifies Clauses` (only explicit)
