...
GNAT Studio provides several dedicated features for launching |GNATprove| and
examining its output. These features will become available when |GNATprove| is
installed and found on your PATH.

:menuselection:`SPARK` Menus
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The :menuselection:`SPARK` menu in the menu bar at the top of the GNAT Studio
window provides the following actions:
...
.. _counterexamples_and_gnattest:

Additional Functionalities for Counterexamples
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
are available on your PATH, additional check-related operations appear in the
:menuselection:`SPARK` contextual menu in the Locations tab when clicking on a
line that contains a failed check message.

.. note::

   The features below are based on new and experimental features in GNAT DAS.
   Please make sure to also consult the
   `GNATtest <https://docs.adacore.com/live/wave/gnatdas/html/gnatdas_ug/gnattest/gnattest_part.html>`_
   and
   `GNATfuzz <https://docs.adacore.com/live/wave/gnatdas/html/gnatdas_ug/gnatfuzz/gnatfuzz_part.html>`_
   documentation to learn about these tools and their current limitations.

.. note::

   To use the features below, you must first set up GNATtest's `test generation
   support <https://docs.adacore.com/live/wave/gnatdas/html/gnatdas_ug/gnattest/gnattest_part.html#setting-up-the-test-generation-runtime>`_.
...
.. list-table::
   ...
   * - Submenu
     - Action
   * - Generate Executable Test with Counterexample
     - Generates a GNATtest harness including a test case that exercises the
       current subprogram with input values taken from the counterexample.

       (This option is applicable only when |GNATprove| has identified a
       counterexample. See :ref:`Understanding Counterexamples`.)
   * - Generate Counterexamples with GNATtest
     - Runs GNATtest on the current subprogram to generate a ``.ce`` file with
       counterexample candidates.

       This file can be passed to |GNATprove| through the ``--gnattest-values``
       switch.
   * - Generate Counterexamples with GNATfuzz
     - Same as above, but uses GNATfuzz as the value generator.

Enhanced Display of Contracts
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
