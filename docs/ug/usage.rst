...
The menu ``Prove All`` runs |GNATprove| on all files in the project, both in
the root project and in projects that are included in the root project. The
menu ``Prove Root Project`` only runs |GNATprove| on files in the root
project. If main files are given for the root project, only those files that
are needed to build the main files will be analyzed. On a project that has
neither main files nor includes other projects, menus ``Prove All`` and ``Prove
Root Project`` are equivalent.
