...
compiler is able to prove some of them; 2) the same process applies to checks
that SPARK would generate and checks that SPARK does not know about, because
they originate in language constructs that are not in SPARK; 3) the same checks
are generated whatever the proof chain used, through SPARK or through Why.
...
Ada uses the Universal Character Set (ISO/IEC 10646:2003) whereas SPARK/Why use
only the full Latin-1 set. Identifiers outside the Latin-1 set should therefore
be translated.
...
For each global variable of mode ``in'', it may be necessary to add a
precondition to the subprogram which asserts the implicit corresponding type
invariant, when the original type for this variable has a dynamic flavor
(subtype with dynamic bounds, discriminant record).
...
the same as those respected by a subtype with the same bounds.
...
\paragraph{Quantified Expressions} A quantified expression should be translated
differently in Why depending on the location where it appears in the generated
Why code:
\begin{itemize}
\item in predicate position, it should be translated into the corresponding
  predicate in Why (\verb|for all| in Ada becomes \verb|forall| in Why;
  \verb|for some| in Ada becomes \verb|exists| in Why);
\item in term position, it should be translated into a temporary boolean
  binding whose value is contrained to be true whenever the predicate holds,
  which requires hosting this equivalence as the left part of an implication in
  the closest enclosing predicate;
\item in program position, it should be translated into a temporary boolean
  binding whose value is contrained to be true whenever the predicate holds,
  which requires hosting this equivalence as a postcondition of a hidden
  expression in the closest enclosing unit-valued expression.
\end{itemize}

\begin{example}
When appearing in predicate position:
\begin{verbatim}
procedure P with Pre => (for all X in Index => Prop (X));
\end{verbatim}
the quantified expression should be translated into a predicate:
\begin{verbatim}
forall X : Index. Prop (X)
\end{verbatim}

When appearing in term position:
\begin{verbatim}
procedure P with Pre => if (for all X in Index => Prop (X)) then Y;
\end{verbatim}
the quantified expression should be translated into an equivalence:
\begin{verbatim}
let tmp = any_bool() in
(tmp = true <-> forall X : Index. Prop (X)) -> if tmp then Y
\end{verbatim}

When appearing in program position:
\begin{verbatim}
B : Boolean := (for all X in Index => Prop (X));
\end{verbatim}
the quantified expression should be translated into a hidden expression:
\begin{verbatim}
let tmp = any_bool() in
[ {} bool { tmp = true <-> forall X : Index. Prop (X) } ];
B := tmp
\end{verbatim}
\end{example}
