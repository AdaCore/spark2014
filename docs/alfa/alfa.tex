...
a much simpler form than the usual functions, which makes them easier to
translate into proof predicates, 3) can be defined in a package specification
which makes them available for proof even if the package body is not.
...
is all other variable paths read or written.
...
tagged types, that is subprograms which can be subject to dynamic dispatch:
\begin{enumerate}
...
\item inherited \verb|Pre'Class| and \verb|Post'Class| aspects are inherited by
  ...
  Subtitution Principle: inherited preconditions are or'ed, which results in a
...
want to obey the Liskov Subtitution Principle. In this case, the standard
mandates that none of the subprograms in this inheritance hierarchy has a
\verb|Pre'Class| aspect. The \verb|Pre| aspect should be used instead.

In order to perform formal verification of dipatching calls, we require that
the program obeys the Liskov Subtitution Principle. As a result, a dispatching
...
\end{verbatim}
The variables above should be splitted into individual subcomponents as follows:
\begin{verbatim}
...
each subprogram parameter should be splitted into a set of path-variables
...
splitted in those path-variables which correspond to variable paths in
...
during the translation of subprograms. However, as seen in
Section~\ref{ALFA:calls}, we should check that overriding subprograms obey the
Liskov Subtitution Principle. Ada rules for inheritance of \verb|Pre'Class| and
\verb|Post'Class| ensure that this is the case for preconditions and
postconditions. Thus, the only remaining part of the contract to check is the
frame condition. In order to ensure automatically that the Liskov Subtitution
Principle is obeyed by the frame condition, we should generate a new subprogram
declaration used in dispatching calls, whose frame condition aggregates all the
frame conditions of all the overridings of the subprogram along the hierarchy:
\begin{enumerate}
\item any path-variable which is ``in out'' in some overriding is kept as ``in
  out'';
\item any other path-variable which is ``in'' in some overriding and ``out'' in
  some other overriding is kept as ``in out'';
\item any other path-variable which is ``in'' or ``out'' in some overriding is
  kept.
\end{enumerate}

\begin{example}
  Suppose a primitive subprogram A.F is overriden by B.F and C.F:
  \begin{itemize}
  \item A.F takes AI as ``in'', AO as ``out'' and AOI as ``in out'';
  \item B.F takes BI as ``in'' and AI as ``out'';
  \item C.F takes AO as ``in''.
  \end{itemize}

  Then the aggregated frame condition takes AI, AO and AIO as ``in out'' and BI
  as ``in''. It is more restrictive than any of the frame conditions of A.F,
  B.F or C.F.
\end{example}
