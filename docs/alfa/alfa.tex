...
initial value is not read. The read-write-set is all other variable paths read
or written.
...
Library-level packages, local packages, child packages and generic packages are
all in ALFA.

Formal packages for generic packages and subprograms are not in ALFA, as they
may require quite some work for no clear benefit.
...
recursion between subprograms as well as 

 Then, a subprogram P.F in ALFA should be translated into the
