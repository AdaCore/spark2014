...
one, except ot applies to components of variables instead of entire variables.
...
Since contract verification and verification of absence of run-time errors are
independent verification objectives, which require different (although
compatible) choices during the translation.  Therefore, we define three modes
for the translation:
\begin{enumerate}
...
write to component \verb|X| of variable \verb|V| does not count as a write to
...
The GNAT front-end defines an expansion phase which simplifies the work needed
