...
\begin{proof}
  The proof proceeds by induction on the number of guarded statements.
  %
  \begin{align*}
    \spost{\gstatement{}{1 .. k}}{P} &\symdef \spost{s_1}{P}\land ... \land \spost{s_{\pat}}{P} \land... \land \spost{s_k}{P} && \text{\sppar}
  \end{align*}
  %
  $s_{\pat}$ is either a guarded exist or a guarded assignment. 
  %
  \subproof{$s_{\pat}$ over Guarded exit :} $s_{\pat} \symdef t \rightarrow \KWexit$
  \begin{align*}
    \spost{\gstatement{}{1 .. k}}{P} 
      &\symdef \spost{s_1}{P}\land ... \land \spost{s_{\pat}}{P} \land... \land \spost{s_k}{P} && \text{\sppar}\\
      &= \spost{s_1}{P}\land ... \land \neg t \land P \land... \land \spost{s_k}{P} && \text{\spexit}\\
      &\Rightarrow P 
  \end{align*}

  \subproof{$s_{\pat}$ over Guarded assignment :} $s_{\pat} \symdef g \rightarrow a := e$ 
  \begin{align*}
    \spost{\gstatement{}{1 .. k}}{P} 
      &\symdef \spost{s_1}{P}\land ... \land \spost{s_{\pat}}{P} \land... \land \spost{s_k}{P} && \text{\sppar}\\
      &= \spost{s_1}{P}\land ... \land (\exists b) (\subst{g}{a}{b} \land a = \subst{e}{a}{b} \land \\ 
      &\subst{P}{a}{b}) \land... \land \spost{s_k}{P} && \text{by \spscalar}\\
      &= \spost{s_1}{P}\land ... \land P \land... \land \spost{s_k}{P} && \text{a is not in P}\\
      &\Rightarrow P \\
  \end{align*}
...
\newcommand{\sli}[2]{\ensuremath{\mathit{SLI}(#1,#2)}\xspace}

\begin{definition}
  \emph{(Stronguest Loop Invariant)} An invariant \inv is a stronguest loop 
  invariant of a loop where the loop statements are reduced to a pattern \pat 
  \sli{\inv}{\pat} 
  if it implies any loop invariant $\inv^{'}: \inv \impl \inv^{'}$.
  In our case it consists to compute the stronguest postcondition of \pat by
  starting from state $\mathit{true}$ : $\sli{\inv}{\pat} = \spost{\pat}{\mathit{true}}$.
...
\begin{definition}
  ...
  \begin{enumerate}
   \item \sli{\inv}{\pat}
   \item \inv is a loop invariant of any loop where \pat occurs in the loop
...
\begin{lemma}
  If \Inv{\idx} is the strongest loop invariant starting from $\mathit{true}$ for a loop
  $L$ reduced to a given pattern \pat ($L~=~loop~~\pat~~end$) then \Inv{\idx} is an invariant for
  any loop $L'$ ($L'~=~loop~~ \gstatement{exit}{1..n}~||~\gstatement{assign}{1..m}~||\pat~~end$) 
  where \pat occurs and write-disjoints with the others guarded exits statements 
  \gstatement{exit}{1..n} and guarded assignment statements \gstatement{assign}{1..m}.
\label{lemma:proof-deduction}
\end{lemma}

\begin{proof}
  Let \Inv{\idx}, at loop iteration \idx in range $\idxinitial .. \idxfinal$, 
  the strongest loop invariant of $L$ reduced to a
  given pattern \pat ($\spost{\pat}{\Inv{\idx}}=\Inv{\idx+1}$). If \Inv{\idx} is the loop 
  invariant of $L'$ then stronguest postcondition of $L'$ implies \Inv{\idx}. 
  Therefore, let us prove that the stronguest postcondition of $L'$ starting by $\Inv{\idx}$ implie \Inv{\idx+1} 
  ($\spost{\gstatement{exit}{1..n} || \gstatement{assign}{1..m} || \pat}{\Inv{\idx}}\Rightarrow \Inv{\idx}$). 
  ...
  \begin{align*}
    ...
    &=\spost{\gstatement{exit}{1..n}}{\Inv{\idx}} \land 
      \spost{\gstatement{assign}{1..n}}{\Inv{\idx}} \land
      \spost{\pat}{\Inv{\idx}}\\
    &=\spost{\gstatement{exit}{1..n}}{\Inv{\idx}} \land 
      \spost{\gstatement{assign}{1..n}}{\Inv{\idx}} \land
      \Inv{\idx+1}\\
    &\Rightarrow \Inv{\idx+1}  
  \end{align*}
  This proves that \Inv{\idx} is a loop invariant at iteration $\idx$ of every loop where pattern
  \pat occurs.
\end{proof}

\begin{theorem}
  $\sli{\inv}{\pat} \Rightarrow \capture{\inv}{\pat}$
\label{theo:proof-deduction}
\end{theorem}
\begin{proof}
  Trivially by lemma \ref{lemma:proof-deduction} 
\end{proof}
...
\patternnote Afterwards, to prove that a invariant suggested for each of our patterns 
  ...
  to this pattern, since the theorem \ref{theo:proof-deduction} give us the point 2 of 
...
\begin{proof}
  ...
  Let us prove part only part (1) of the pattern capture definition. 
  Let $l$ be the loop containing the pattern only: 
  $\gstatement{exit}{1..n} = \o{} \land \gstatement{assign}{1..m} = \o{}$
  ...
  \begin{align*}
    ...
    &=\neg t_0^\at{\idx} \land (\forall j)(\seen{\idxinitial}{j}{\idx} \impl \neg t_{0}^\at{j}) && \text{by \vpp}\\
...
$\begin{array}{l}
  \KWloop \KWover i \KWin 1~..~N \\
  ~~~~ \textit{invariant}: (\idx = 1 \impl \mathit{flag} = \mathit{flag}_0) \land 
        (\idx \neq 1 \impl flag = true)\\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &=(\exists b) (a = e^\at{\idxinitial}[a\leftarrow b] \land \Inv{\idx}[a\leftarrow b])    && \text{by \spscalar}\\
    &=(\exists b) (a = e_0^\at{\idxinitial}[a \leftarrow b] \land \Inv{\idxinitial+1}[a\leftarrow b]) && \text{by \vpp}\\ 
    &=(a = e_0^\at{\idxinitial}) \land (\exists b) (\Inv{\idxinitial+1}[a\leftarrow b])               && \text{$a$ not in $e_0^\at{\idx}$}\\
...
\lor 
& ((\exists j)(\seen{\idxinitial}{j}{\idx} \land a = e_0^\at{j} \land (\forall k)(\between{j}{k}{i} \impl 
...
\begin{proof}
  ...
  \begin{align*}
    \spost{
    &(a \bullet e^\at{\idx}) \rightarrow a := e^\at{\idx}}{a = a_0}\\
    &=\begin{array}{ll}
           & \neg (a \bullet e^\at{\idx}) \land (a = a_0) \\
      \lor & (\exists b) ((a \bullet e^\at{\idx})[a\leftarrow b] \land 
      a = e^\at{\idx}[a\leftarrow b] \land (a = a_0)[a\leftarrow b])
      \end{array} && \text{by \spscalar}\\
    &=\begin{array}{ll}
           & \neg (a \bullet e_0^\at{\idx}) \land (a = a_0) \\
      \lor & (a = e_0^\at{\idx}) \land 
       (\exists b) ((b \bullet e_0^\at{\idx}) \land (b = a_0))
      \end{array}  && \text{by \vpp}\\
    &=\begin{array}{ll}
           & \neg (a \bullet e_0^\at{\idx}) \land (a = a_0) \\
      \lor & (a = e_0^\at{\idx}) \land (a_0 \bullet e_0^\at{\idx}) 
      \end{array}
  \end{align*}
  %
  Now assume \Inv{\idx} at loop iteration \idx. Notice that \Inv{\idx} does not
...
\pattern{
  ...
  &(a = a_0) \land (\forall j)(\seen{\idxinitial}{j}{\idx} \impl \neg g_0^\at{j})\\
  \lor 
  & (\exists j)(\seen{\idxinitial}{j}{\idx} \land g_0^\at{j} \land a = e_0^\at{j} \land (\forall k)(\between{j}{k}{\idx} \impl \neg g_0^\at{k}))
}
...
1. reduced case: $\gstatement{exit}{1..n} = \o{} \land \gstatement{assign}{1..m} = \o{}$
  ...
  \begin{align*}
    ...
    &A_2=(\exists j)(\idxinitial \leq j < \idx \land g_0^\at{j} \land a = e_0^\at{j} \land (\forall k)(j < k < \idx+1 \impl \neg g_0^\at{k}) \\    
  \end{align*}
  %
  \begin{align*}
  ...
  \begin{align*}
    ...
    &\phantom{B}
       \lor (a = e_0^\at{\idx}) \land g_0^\at{\idx} \land (\exists j)(\idxinitial \leq j < \idx \land g_0^\at{j} \land 
       (\forall k)(j < k < \idx \impl \neg g_0^\at{k}))\\
    &B\Leftrightarrow (a = e_0^\at{\idx}) \land g_0^\at{\idx} \land (\exists j)(\idxinitial \leq j < \idx \land g_0^\at{j} 
       \land (\forall k)(j < k < \idx \impl \neg g_0^\at{k}))\\
  \end{align*}
  %
  \begin{align*}
    &A_2 \lor B = (\exists j)(\idxinitial \leq j < \idx \land g_0^\at{j} \land a = e_0^\at{j} \land (\forall k)(j < k < \idx+1 \impl \neg g_0^\at{k})\\
    &\phantom{A_2 \lor B} 
       \lor (a = e_0^\at{\idx}) \land g_0^\at{\idx} \land (\exists j)((\idxinitial \leq j < \idx \land g_0^\at{j}) \land
       (\forall k)(j < k < \idx \impl \neg g_0^\at{k}))\\
    &\phantom{A_2 \lor B} 
...
1. reduced case: $\gstatement{exit}{1..n} = \o{} \land \gstatement{assign}{1..m} = \o{}$
  ...
  \begin{align*}
    \spost{
    & g^? \rightarrow a := e^\at{\idx}}{\Inv{\idx}\\
    }
    &=\Inv{\idx} \lor (\exists b) (a = e^\at{\idx}[a\leftarrow b] \land \Inv{\idx}[a\leftarrow b])   && \text{by \spscalar}\\
    &=\Inv{\idx} \lor (\exists b) (a = e_0^\at{\idx}[a\leftarrow b] \land \Inv{\idx}[a\leftarrow b])   && \text{by \vpp}\\
    &=((a = a_0) \lor (\exists j)(\idxinitial \leq j < \idx \land a = e_0^\at{j})) \lor ( a = e_0^\at{\idx} \land  \\
    &\phantom{B}
     (\exists b)(b = a_0) \lor (\exists j)(\idxinitial \leq j < \idx \land b = e_0^\at{j})) && \text{by unfolding $\Inv{\idx}$} \\
    &=((a = a_0) \lor (\exists j)(\idxinitial \leq j < \idx \land a = e_0^\at{j})) \lor (a = e_0^\at{\idx} \land \mathit{true}) && \text{by taking $b=a_0$}\\
    &=((a = a_0) \lor (\exists j)(\idxinitial \leq j < \idx+1 \land a = e_0^\at{j}))\\
    &=\Inv{\idx+1} && \text{by \curinv}
  \end{align*}
  %
  This proves that \Inv{\idx} is the strongest loop invariant on the 
