...
%%
%%Proof preliminaires
\newcommand{\spstrong}{\textsc{sp-stronguest}\xspace}
\newcommand{\spimply}{\textsc{sp-imply}\xspace}
...
\begin{definition} 3 \emph {(Strongest Postcondition)}\\
Given the statement $\texttt{S}$ and the precondition $P$, the formula, denoted by \spost{S}{P},
is the strongest postcondition iff
$$(\forall \sigma')(\sigma' \vDash \spost{\texttt{S}}{P} \Leftrightarrow 
   \exists \sigma, \{\texttt{S},\sigma\} \rightsquigarrow \sigma' \land \sigma \vDash P)$$
\end{definition}

\begin{corollary} \emph {Strongest Postcondition consequences}\\
\label{corollary:sp}
Given the statement $S$, the precondition $P$ and the postcondition $Q$ we have the following consequences
of stronguest postcondition in the partial correctness satisfaction relation of Hoare ($\vDash_{par}$).
\begin{itemize}
\item $\vDash_{par} \{ P \} \texttt{S} \{ \spost{S}{P} \} \quad \quad \mathsf{(1)} $
\item if $\vDash_{par} (\{ P \} \texttt{S} \{ Q \})$ then $(\spost{S}{P} \Rightarrow Q) \quad \quad (\spstrong)$ 
\item if $(P \Rightarrow Q)$ then $(\spost{\texttt{S}}{P} \Rightarrow \spost{\texttt{S}}{Q}) \quad \quad (\spimply)$ 
\end{itemize}
\end{corollary}
...
$\{\texttt{S},\sigma\} \rightsquigarrow \sigma'$ and $\sigma \vDash P$. 
From the definition of $\spost{S}{P}$ we get that $\sigma' \vDash \spost{\texttt{S}}{P}$ as required.
}
\subproof{
To verify that (2) holds let assume that $\vDash_{par} \{ P \} \texttt{S} \{ Q \}$ and let 
$\sigma \vDash P$. If $\{\texttt{S},\sigma\} \rightsquigarrow \sigma'$ then $\sigma \vDash P$ 
(because $\vDash_{par} \{ P \} \texttt{S} \{ Q \}$). So 
$\exists \sigma, \{\texttt{S},\sigma\} \rightsquigarrow \sigma'$ and $\sigma \vDash P$
so clearly $\sigma' \vDash \spost{\texttt{S}}{P}$ by the definition of $\spost{S}{P}$.
}
\subproof{
To verify that (3) holds let $\sigma'$ states, such that 
$\sigma' \vDash \spost{\texttt{S}}{P}$, from the definition 3 of $\spost{S}{P}$ we get that
$\exists \sigma, \{\texttt{S},\sigma\} \rightsquigarrow \sigma' \quad \mathsf{(a)}$ and 
...
$ \exists \sigma, \{\texttt{S},\sigma\} \rightsquigarrow \sigma' \land \sigma \vDash Q$, 
so clearly $\sigma' \vDash \spost{\texttt{S}}{Q}$ by the definition of $\spost{S}{Q}$.
}
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  %
  By property (\spstrong) on $SP$, any postcondition $Q$ such that
  $\{P\} C \{Q\}$ holds,  is implied by $\spost{C}{P}$. We instantiate
  this property on the Hoare triple above. We obtain (not that $g_{exit}^\at{\idx}$ is value preserving):
  \begin{align*}
    \spost{\texttt{B}}{\idxinitial \leq i \leq \idxfinal \land \neg g_{exit}^\at{\idx} \land \Psi(i)} 
    &\Rightarrow &\\
    \Psi(i+1)
    &\Rightarrow &\\
   \idxinitial \leq i \leq \idxfinal \land \neg g_{exit}^\at{\idx} \land \Psi(i+1)
  \end{align*}
  %
  By inductive hypothesis we have 
  $(\forall \idx) (\idxinitial \leq \idx \leq \idxfinal \Rightarrow (\Phi(\idx) \Rightarrow \Psi(\idx)))$\\
  %
  Especially we apply \spimply property in $\Phi(\idx) \Rightarrow \Psi(\idx)$ and we get
  \begin{align*}
    \spost{\texttt{B}}{\Phi(\idx) \land \idxinitial \leq \idx \leq \idxfinal \land \neg g_{exit}^\at{\idx}} &\Rightarrow&\\
    &\spost{\texttt{B}}{\Psi(\idx) \land \idxinitial \leq \idx \leq \idxfinal \land \neg g_{exit}^\at{\idx}}& \\
    &\Rightarrow&\\
...
%\ifdefined\longversion
  ...
  %
  \begin{eqnarray*}
    &&A=(\exists A') \left(\begin{array}{ll}
             &(\forall j)(\seen{j}{\idx} \land g_0^\at{j} \impl A'[j] = e_0^\at{j}) \\
             \land & g_0^\at{\idx} \Rightarrow (A[\idx] = e_0^\at{\idx}) \\
        \end{array}\right)\\
    &&B=(\exists A') \left(\begin{array}{ll}
             & (\forall j)(\seen{j}{\idx} \land \neg g_0^\at{j} \impl A'[j] = A_0[j]) \\
            \land & (\forall j) (\inbounds{A}{j} \land \neg g_0^\at{\idx}) \Rightarrow (A[j] = A'[j])) \\
        \end{array}\right)\\
    &&C=(\exists A') \left(\begin{array}{ll}
             & (\forall j)(\neg \seen{j}{\idx} \land \inbounds{A'}{j} \impl A'[j] = A_0[j])\\
             \land & (\forall j) ((\inbounds{A}{j} \land j \neq \idx) \Rightarrow (A[j] = A'[j])
        \end{array}\right)\\
  \end{eqnarray*}
  %
  - simplify $B$ : $(\forall j)(\neg g_0^\at{j} \impl A'[j] = A_0[j])$ so
  \begin{eqnarray*}
    &&B=(\exists A') \left(\begin{array}{ll}
             & (\forall j)(\seen{j}{\idx} \land \neg g_0^\at{j} \impl A_0[j] = A_0[j]) \\
            \land & (\forall j) (\inbounds{A}{j} \land \neg g_0^\at{\idx}) \Rightarrow A[j] = A_0[j]) \\
        \end{array}\right)\\
    &&B=\begin{array}{ll}
             & (\forall j)(\seen{j}{\idx} \land \neg g_0^\at{j} \impl A_0[j] = A_0[j]) \\
            \land & (\forall j) (\inbounds{A}{j} \land \neg g_0^\at{\idx}) \Rightarrow A[j] = A_0[j]) \\
        \end{array}\\
    &&B=(\forall j)(\seen{j}{\idx+1} \land \neg g_0^\at{j} \impl A[j] = A_0[j]) \quad \quad \quad \mathsf{(1)}
  \end{eqnarray*}
...
\pattern{
  ...
  \land &
  (\forall k_1, k_2)((\exists j_1,j_2) (k_1 < k_2 \impl 
  \left(\begin{array}{cc}
