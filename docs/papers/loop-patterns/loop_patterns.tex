...
\textit{value-preserving expressions} ($vpp$) :  
\begin{equation}
 e=e_0
\label{eq:vpp}
\end{equation}
...
We use the strongest postconditions calculations to prove that our invariants are precis
We recall here the rules of Strongest Postconditions for statements ("guarded exit" and 
"guarded assign" that we are interested in our loop modelisation.  
...
sp(t^\at{i+1} \rightarrow \KWexit,Inv(i))   &= \neg t^\at{i+1} \land Inv(i) && \text{by \eqref{eq:exit_sp}}\\
                                            ...
                                            &= Inv(i+1) && \text{by symbolic manipulation}
\end{align*}
...
sp(\mathit{true} \rightarrow a := e^\at{i},Inv(i))  &= sp(\mathit{true} \rightarrow a := e_0^\at{i},Inv(i))  && \text{by \eqref{eq:vpp}}\\
                                                    ...
                                                    &= a = e_0^\at{i} (\exists b) (Inv(i)[a \leftarrow b]) && \text{a not in $e_0^\at{i}$}\\
