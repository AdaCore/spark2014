...
\begin{proof}
  ...
  \curinv is a \textit{precis} loop invariant if only \curpattern is 
  a only statement in the loop. Proof by  contradiction (trough Definition 
  Pattern Capture: erasing by Yannick) the precis invariant is the strongest postcondition 
 (definition by Johannes) .
  
\end{proof}

\begin{theorem}
  \emph{(Search Pattern Invariant Capture the Loop)} \curinv given in \loopmodel
  is a Loop Invariant and it preserves any Loop Invariant.  
\label{theo:preserve-search-in-loop}
\end{theorem}
