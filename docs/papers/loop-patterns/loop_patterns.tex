...
\begin{definition}
  \emph{(Pattern)} A pattern is a subset of guarded exits and guarded assignments
  from a loop body.
\end{definition}
...
2. general case \\
  ...
  \noindent
  We start from invariant $\mathit{true}$ before the loop.
  In this case, only guarded exit statements \gstatement{exit}{1..n} 
  occur before the current pattern \curpattern. 
  %
  \begin{align*}
    \spost{
      &\gstatement{exit}{1..n};\\
  ...
  %
  Now assume \Inv{\idx} at loop iteration \idx, and let us prove that
  \Inv{\idx+1} holds at loop iteration $\idx+1$ after executing first 
  the guarded assignments of loop iteration \idx, then the guarded exits 
  of loop iteration $\idx+1$ and finally the search pattern of loop iteration
  $\idx+1$. Notice that \Inv{\idx} does not mention any variable, only initial 
  values of variables and current value of the loop index.
  %
  \begin{align*}
    \spost{
...
\pattern{
  A scalar variable $a$ is modified in at most exactly a single assignment, whose
...
%
\begin{theorem}
  \emph{(Certain Single Update Pattern Invariant Capture the Pattern)} \Inv{\idx} given by \curinv
