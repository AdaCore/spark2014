...
\caption{Simple loops syntax}\label{syntax}
\end{figure}
%==================================================================================
%==================================================================================
\subsection{Definitons and foundations}

\subsubsection*{Semantic constraints on loops}
...
% by starting in the fact that all the variables in the loop is egal to the value of the variable
% at loop entry (we denode this fact by the special predicat $\mathrm{Init}_0(P)$)
if: 
\begin{itemize}
...
\begin{proof}
Induction on the value of index $\idx$ 
\ifdefined\longversion
(or on the number of $n$ of loop iterations?).\\
\fi

\subproof{\textbf{Base Case} : $\idx=\idxinitial$}
By definition, as $\Psi$ is an inductive invariant under $\Init(P) $, the following holds:
$$(\Init(P) \land \idx= \idxinitial) \Rightarrow R(\idxinitial)$$
Also by (a), we have :
$$
\Phi(\idxinitial) \Leftrightarrow (\Init(P) \land \idx= \idxinitial) \Rightarrow  \Psi(\idxinitial)
$$


\subproof{\textbf{Inductive Step} : $\idxinitial \leq \idx=k$} 
\ifdefined\longversion
If $k > w$ then \ref{max-stm}  ??? is trivialy true. 
\fi
We assume that $k\leq w$ and the inductive hypthesis $H(k) \equiv (\idx= k \land \Phi(i))
\Rightarrow (\idx=k \land \Psi(\idx))$. Let us prove $H(k+1)$.\\

As $k\leq $, we can apply hypothesis (b) on the particular index value $\idx=k$. 
\begin{align*}
 \idx=k+1 \land \Phi(\idx) \land \idxinitial \leq \idx \leq \idxfinal+1
  & \Leftrightarrow &  \quad \mathsf{(1)}\\
  & \spost{\texttt{B; \idx=\idx+1}}{\idx=k \land \idxinitial \leq \idx \leq \idxfinal \land \Phi(\idx)}   & \\
\end{align*}
The \emph{Imply-Sp} property says: 
$$(P \Rightarrow Q) \Rightarrow (\spost{C}{P} \Rightarrow \spost{C}{Q}) \quad \quad \quad \mathrm{(Sp-Imply)}$$
\subsubproof{Proof of \emph{Imply-Sp} TODO with the semantic and definition of SP}

We Instantiate this property taking $P$ as the induction hypothesis
together with  $\idxinitial \leq \idx \leq\idxfinal$. We obtain:
\begin{align*}
(\idx=k \land \idxinitial \leq \idx \leq\idxfinal \land \Phi(\idx) & \Rightarrow 
\idx=k \land \idxinitial \leq \idx \leq\idxfinal \land \Psi(i)) \Rightarrow & \quad \mathrm{H(k)}\\
& \spost{\texttt{B; \idx=\idx+1}}{\idx=k \land \idxinitial \leq \idx \leq\idxfinal \land \Phi(\idx)} \Rightarrow &\\
& \spost{\texttt{B; \idx=\idx+1}}{\idx=k \land \idxinitial \leq \idx \leq \idxfinal \land \Psi(\idx)}& \\
\end{align*}
As the induction hypothesis $H(k)$ and $\idxinitial \leq \idx \leq\idxfinal$
holds, we can use this property together with (1) to derive:
\begin{align*}
 \idx=k+1 \land \idxinitial \leq \idx \leq \idxfinal+1 \land \Phi(\idx) 
 & \Leftrightarrow & \\
 & \spost{\texttt{B; \idx=\idx+1}}{\idx=k \land \idxinitial \leq i \leq \idxfinal \land \Phi(i)} \Rightarrow & \\
 & \spost{\texttt{B; \idx=\idx+1}}{\idx=k \land \idxinitial \leq \idx \leq \idxfinal  \land \Psi(\idx)} & \\
\end{align*}
But $\Psi$ is an inductive invariant,  and thus, the Hoare triple below holds.
$$
\{i=k \land \Psi(i) \land \idxinitial \leq i \leq \idxfinal \} 
\texttt{B; i=i+1} 
\{i=k+1 \land \idxinitial \leq i \leq \idxfinal+1 \land \Psi(i) \}
$$ 
Let us call (2), the postcondition of this triple:
\begin{align*}
i=k+1 \land \idxinitial \leq i \leq \idxfinal+1 \land \Psi(k+1) & \quad \mathsf{(2)}
\end{align*}
By property (Sp-Strongest) on $SP$, any postcondition $Q$ such that
$\{P\} C \{Q\}$ holds,  is implied by $\spost{C}{P}$. We instantiate
this property on the Hoare triple above. We obtain:
\begin{align*}
\spost{\texttt{B; i=i+1}}{i=k \land R(i) \land \idxinitial \leq i \leq \idxfinal} 
&\Rightarrow &\\
\idx= k+1 \land \idxinitial \leq \idx \leq \idxfinal+1 \land \Psi(\idx)
\end{align*}
We use this result to complete the deduction starting from ():
\begin{align*}
 i=k+1 \land \idxinitial \leq i \leq \idxfinal+ 1 \land \Phi(i) 
 & \Leftrightarrow & \\
 & \spost{\texttt{B; \idx=\idx+1}}{\idx=k \land \idxinitial \leq i \leq \idxfinal \land \Phi(i)} \Rightarrow &\\
 & \spost{\texttt{B; \idx=\idx+1}}{\idx=k \land \idxinitial \leq \idx \leq \idxfinal \land \Psi(\idx)}  \Rightarrow & \\
 \idx= k+1 \land \idxinitial \leq \idx \leq \idxfinal+1 \land \Psi(\idx)
\end{align*}
Which allow us to conclude $\idx=k+1 \land \Phi(\idx) \Rightarrow \idx= k+1 \land
R(\idx)$ as desired. 
...
\label{lem:pattern-captures}
  ...
  in  our \loopform, captures the pattern \pat becomes to prouve that $\phi(i)$ is the strongest 
  loop invariant reduced to pattern \pat.
\end{lemma}
...
\patternnote Notice that this particular pattern allows 
...
\begin{proof}
  ...
  then $(\forall j) (\seen{j}{\idx} \impl \neg g_0^\at{\idx}) = \mathit{true}$\\ 
  so :
  $$i = \idxinitial\Leftrightarrow(\forall j) (\seen{j}{\idx} \impl \neg g_0^\at{\idx}) \land (i = \idxinitial)$$
  ...
  \begin{eqnarray*}
    ...
    &\Leftrightarrow&\\
    &\inrange{\idx} \land (\forall j)(\seen{j}{\idx+1} \impl \neg g_{0}^\at{j})&\\
    &\Leftrightarrow&\\
    &\inrange{\idx} \land \Inv{\idx+1}
  \end{eqnarray*}

  This proves that \Inv{\idx} is the strongest loop invariant on the 
  loop reduced to the pattern. 
  By deduction through the theorem \ref{lem:pattern-captures}, \Inv{\idx} 
...
\begin{proof}
  ...
  \end{eqnarray*}
  %
  - Computation of $A \lor B_2$
  \begin{eqnarray*}
    A \lor B_2 &\Leftrightarrow& 
                \left(\begin{array}{cc}
                     &(\exists j)(\seen{j}{\idx} \land g_0^\at{j} \land a = e_0^\at{j} \\
                     &\land (\forall k)(\betweens{j}{k}{\idx+1} \impl \neg g_0^\at{k}))\\
                     &\land\\
  ...
  \end{eqnarray*}
  %
  - Computation of $B_1 \lor A \lor B_2$
  \begin{eqnarray*}
    B_1 \lor A \lor B_2 &\Leftrightarrow& 
               \left(\begin{array}{cc}
...
\ifdefined\longversion
  ...
  \subproof{Proof of loop invariant initalization:}
  In a loop reduced only to this pattern, no variable is modified, excepted
  $A$ and current value of the loop index. At the beginning of the loop,\\
  $i = \idxinitial$,
  $(\forall j)(\idxinitial \leq j \leq \idxfinal \impl A[j] = A_0[j])$\\
  $i = \idxinitial$, $\nexists j$ such that $\idxinitial \leq j <i$, i.e $(\seen{j}{\idx})=\mathit{false}$\\
  then $\Inv{\idx} = (\forall j)(\neg \seen{j}{\idx} \land \inbounds{A}{j} \impl A[j] = A_0[j])$, so \\
  $i = \idxinitial$, $\neg \seen{j}{\idx} \land \inbounds{A}{j} \equiv \idxinitial \leq j \leq \idxfinal$.\\
  then 
  $$(\Inv{\idx} \land \idx = \idxinitial) \Leftrightarrow (i = \idxinitial \land (\forall j)(\idxinitial \leq j \leq \idxfinal \impl A[j] = A_0[j]))$$
  ...
  By \vpp :
  \begin{eqnarray*}
    &&\spost{g^\at{\idxinitial} \rightarrow A[\idxinitial] := e^\at{\idxinitial}}{A = A_0}\\
    && \spost{g_0^\at{\idxinitial} \rightarrow A[\idxinitial] := e_0^\at{\idxinitial}}{A = A_0}\\
    &&=(\exists A')\left(\begin{array}{ll}
                         & (\forall j)(\idxinitial \leq j \leq \idxfinal \impl A'[j] = A_0[j]) \\
                         \land & g_0^\at{\idxinitial} \Rightarrow (A[\idxinitial] = e_0^\at{\idxinitial}) \\
                         \land & (\forall j)( \inbounds{A}{j} \land \neg (g_0^\at{\idxinitial} \land j= \idxinitial) \Rightarrow (A[j] = A'[j])
                   \end{array}\right)\\
  \end{eqnarray*}
  %
  Developping :
  \begin{eqnarray*}
    ...
    &&\Leftrightarrow \left(\begin{array}{ll}
                           & (\forall j)(\idxinitial \leq j \leq \idxfinal \impl A'[j] = A_0[j]) \\
                           \land & g_0^\at{\idxinitial} \impl (A[\idxinitial] = e_0^\at{\idxinitial}) \\
                           \land & (\forall j)( \inbounds{A}{j} \land (\neg g_0^\at{\idxinitial} \lor j \neq \idxinitial) \Rightarrow (A[j] = A'[j]))
                      \end{array}\right) \\
    &&\Leftrightarrow \left(\begin{array}{ll}
                           & (\forall j)(\idxinitial \leq j \leq \idxfinal \impl A'[j] = A_0[j]) \\
                           \land & g_0^\at{\idxinitial} \impl A[\idxinitial] = e_0^\at{\idxinitial} \\
                           \land & (\forall j) ((\inbounds{A}{j} \land \neg g_0^\at{\idxinitial}) \lor (\inbounds{A}{j} \land j \neq \idxinitial) \Rightarrow (A[j] = A'[j])
                      \end{array}\right) \\
  \end{eqnarray*}
  %
  By using $((A \lor B) \impl C) \Leftrightarrow ((A \impl C) \land (B \impl C))$ we have
  \begin{eqnarray*}
    &&\Leftrightarrow \left(\begin{array}{ll}
                           & (\forall j)(\idxinitial \leq j \leq \idxfinal \impl A'[j] = A_0[j]) \\
                           \land& g_0^\at{\idxinitial} \impl (A[\idxinitial] = e_0^\at{\idxinitial}) \\
                           \land & (\forall j) \left(\begin{array}{ll} 
                                                  &(\inbounds{A}{j} \land \neg g_0^\at{\idxinitial}) \Rightarrow (A[j] = A'[j]) \\
                                                  \land &(\inbounds{A}{j} \land j \neq \idxinitial) \Rightarrow (A[j] = A'[j])
                                              \end{array}\right)
                      \end{array}\right) \\
    &&~~~~~~~~~~~~~~~~~\\
    &&\Leftrightarrow \left(\begin{array}{ll}
                           & (\forall j)(\idxinitial+1 \leq j \leq \idxfinal \impl A'[j] = A_0[j]) \\
                           \land & j = \idxinitial \impl  (A'[j] = A_0[j]) \\
                           \land & g_0^\at{\idxinitial} \impl A[\idxinitial] = e_0^\at{\idxinitial} \\
                           \land & (\forall j) (\inbounds{A}{j} \land \neg g_0^\at{\idxinitial}) \Rightarrow (A[j] = A'[j])) \\
  ...
  \begin{eqnarray*}
    &&\Leftrightarrow \left(\begin{array}{ll}
                           & g_0^\at{\idxinitial} \impl A[\idxinitial] = e_0^\at{\idxinitial} \\
                           \land & \left(\begin{array}{ll}
                                        & (\forall j) (\inbounds{A}{j} \land \neg g_0^\at{\idxinitial}) \Rightarrow (A[j] = A'[j])) \\
                                        \land & j = \idxinitial \impl  A'[j] = A_0[j]) \\
                                   \end{array}\right) \\
                           \land & \left(\begin{array}{ll}
                                        & (\forall j) ((\inbounds{A}{j} \land j \neq \idxinitial) \Rightarrow (A[j] = A'[j])\\
                                        \land & (\forall j)(\idxinitial+1 \leq j \leq \idxfinal \impl A'[j] = A_0[j]) \\
                                   \end{array}\right) \\
                      \end{array}\right) \\
  \end{eqnarray*}
  %
  ...
  \begin{eqnarray*}
    &&\Leftrightarrow \left(\begin{array}{ll}
                           \land & (\forall j)\seen{j}{\idxinitial+1} g_0^\at{j} \impl A[j] = e_0^\at{j} \\
                           \land & (\forall j) (\seen{j}{\idxinitial+1} \land \neg g_0^\at{\idxinitial}) \Rightarrow (A[j] = A_0[j])) \\
