...
($e=e_0$) is a loop-invariant. We denote $e^\at{\idx}$ a \textit{value-preserving expression} 
...
Sufficient conditions for an expression $e$ to be translatable are the following:
\begin{enumerate}
...
because $B[Index]$ use a update variable $Index$ (therefore it's not translatable). If we transform it as below we have 
