...
$$
\sigma' \vDash_{par}  \spost{C}{P}  \Leftrightarrow \exists
\sigma. (\eval{C}{\sigma} \rightsquigarrow \sigma' \land \sigma \vDash_{par}  P)
$$
\end{definition} 

\begin{corollary}[Properties on $sp(c,P)$] The following are useful properties of predicate \spost{C}{P}:
\begin{align*}
[\sppost]  &   \quad \vDash_{par} \{P\} ~C ~ \{ \spost{C}{P} \} & \textnormal{ ~is actually a postconditon} \\
...
$\{C,\sigma\} \rightsquigarrow \sigma'$ and $\sigma \vDash_{par} P$. 
From the definition of $\spost{S}{P}$ we get that $\sigma' \vDash_{par} \spost{C}{P}$ as required.
}
\subproof{
To verify that $[\spstrong]$ holds let assume that $\vDash_{par} \{ P \} C \{ Q \}$ and let 
$\sigma \vDash_{par} P$. If $\{C,\sigma\} \rightsquigarrow \sigma'$ then $\sigma \vDash_{par} Q$ 
(because $\vDash_{par} \{ P \} C \{ Q \}$). So if 
$\exists \sigma, \{C,\sigma\} \rightsquigarrow \sigma'$ and $\sigma \vDash_{par}P$
(i.e, clearly $\sigma' \vDash_{par}\spost{C}{P}$ by the definition of $\spost{S}{P}$) then $\sigma \vDash_{par} Q$.
}
\subproof{
To verify that $[\spmonoton]$ holds let $\sigma'$ states, such that 
$\sigma' \vDash_{par} \spost{C}{P}$, from the definition of $\spost{C}{P}$ we get that
$\exists \sigma, \{C,\sigma\} \rightsquigarrow \sigma' \quad \mathsf{(a)}$ and 
$\sigma \vDash_{par} P \quad \mathsf{(b)}$.\\
As $(P \Rightarrow Q)$ therefore we get that $\sigma \vDash_{par} Q \quad \mathsf{(c)}$.
Now with $(a)$ and $(c)$ we get that 
$ \exists \sigma, \{C,\sigma\} \rightsquigarrow \sigma' \land \sigma \vDash_{par} Q$, 
so clearly $\sigma' \vDash_{par} \spost{C}{Q}$ by the definition of $\spost{C}{Q}$.
}
\end{proof}
...
\item[(c)] $\spost{B}{\framecond{\prog} \land \preservecond{\prog}{\idx} \land \idxinitial < \idx \leq \idxfinal \land \neg g_{exit} \land \phi(\idx)} 
             \Leftrightarrow  
            \framecond{\prog} \land \preservecond{\prog}{\idx+1} \land \idxinitial < \idx \leq \idxfinal \land  \phi(\idx+1)$
\end{itemize}
...
(\forall \idx. \idxinitial < \idx \leq \idxfinal) \land (\framecond{\prog} \land \preservecond{\prog}{\idx} \land \phi(i) 
...
\\
  ...
  \begin{align*}
    \initcond{\prog} \land \neg g_{exit} \land \phi(\idxinitial) 
    \Rightarrow
    \neg g_{exit} \land \psi(\idxinitial)
  \end{align*}
  %
  By applying \spmonoton we get that 
  \begin{align*}
    \spost{\texttt{B}}{ \initcond{\prog} \land \neg g_{exit} \land \phi(\idxinitial)} 
    \Rightarrow
    \spost{\texttt{B}}{\neg g_{exit} \land \psi(\idxinitial)}
  \end{align*}
  %
  ...
  \begin{align*}
     \framecond{\prog} \land \preservecond{\prog}{\idxinitial+1} \land  \phi(\idxinitial+1)
    \Rightarrow
    \spost{\texttt{B}}{\neg g_{exit} \land \psi(\idxinitial)} \quad \quad \mathsf{(2)}
  \end{align*}
  %
  With $\mathsf{(2)}$ and $\mathsf{(1)}$ for ($\idx = \idxinitial$) we get that
  \begin{align*}
    \framecond{\prog} \land \preservecond{\prog}{\idxinitial+1} \land  \phi(\idxinitial+1)
  ...
  $$
  \framecond{\prog} \land \preservecond{\prog}{\idx} \land \idxinitial < \idx \leq \idxfinal \land \phi(i) 
  \Rightarrow 
  \psi(i)
  $$
  By adding $g_{exit}$ to both side of the formula we get that
  $$
  \framecond{\prog} \land \preservecond{\prog}{\idx} \land \idxinitial < \idx \leq \idxfinal \land \neg g_{exit} \land \phi(i) 
  \Rightarrow 
  \neg g_{exit} \land \psi(i)
  $$
  %
  By applying \spmonoton, we get that
  \begin{align*}
    \spost{\texttt{B}}{\framecond{\prog} \land \preservecond{\prog}{\idx} \land \idxinitial < \idx \leq \idxfinal \land \neg g_{exit} \land \phi(i)} 
    \Rightarrow
    \spost{\texttt{B}}{\neg g_{exit} \land \psi(\idx)}
  \end{align*}
  %
  ...
  \begin{align*}
    \spost{B}{\framecond{\prog} \land \preservecond{\prog}{\idx} \land \idxinitial < \idx \leq \idxfinal \land \neg g_{exit} \land \phi(\idx)} 
             \Leftrightarrow  \\
    \framecond{\prog} \land \preservecond{\prog}{\idx} \land \idxinitial < \idx \leq \idxfinal \land  \phi(\idx+1)
  \end{align*}
  %
  So 
  \begin{align*}
   \framecond{\prog} \land \preservecond{\prog}{\idx} \land \idxinitial < \idx \leq \idxfinal \land  \phi(\idx+1)
   \Rightarrow
   \spost{\texttt{B}}{\neg g_{exit} \land \psi(\idx)} \quad \mathsf{(3)}
  \end{align*}
  %
  By $\mathsf{(3)}$ and $\mathsf{(1)}$ we get that
  \begin{align*}
    \framecond{\prog} \land \preservecond{\prog}{\idx} \land \idxinitial < \idx \leq \idxfinal \land  \phi(\idx+1)
    \Rightarrow \psi(\idx+1)
  \end{align*}  
  Finally
  $$(\forall \idx. \idxinitial < \idx \leq \idxfinal) \land (\framecond{\prog} \land \preservecond{\prog}{\idx} \land \phi(\idx) 
  \Rightarrow \psi(\idx)) $$
\end{proof}
