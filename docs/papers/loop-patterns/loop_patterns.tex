...
if: 
\begin{itemize}
\item $(\mathrm{Init}_0(L)  \wedge i=\idxinitial) \Rightarrow \phi$. 
...
Let $L = \KWloop  \idx \KWin \idxinitial ..\idxfinal ~ \KWdo B ~
\KWend$. Let $\Psi$ and  $\Phi$ be two inductive invariants for $L$ under
$\Init(B)$. Let $\Phi$ be such that:
\begin{enumerate}
\item[(a)] $(\Init(B) \land \idx = \idxinitial) \Leftrightarrow (\Phi(\idx) \land \idx=\idxinitial)$ \label{init-max}
...
\item[(b-bis)] $\exists p.( \idx=p+1 \land  \idxinitial \leq \idx \leq \idxfinal+ 1
  ...
  \Phi(\idx)))$ 
\end{enumerate}
\noindent Then, $\Phi$ is the most precise invariant for $L$ under $\Init(B)$, that is:
$$
...
By definition, as $\Psi$ is an inductive invariant under $\Init(B) $, the following holds:
$$(\Init(B) \land \idx= \idxinitial) \Rightarrow \Psi(\idxinitial)$$
Also by (a), we have :
$$
(\Phi(\idx) \land \idx=\idxinitial) \Leftrightarrow (\Init(B) \land \idx= \idxinitial) \Rightarrow  \Psi(\idxinitial)
$$
...
$\Init(B)$. And, $SP$ the stronguest possible postcondition at each time a loop body 
...
\item[(b)] $\phi(\idx+1) \wedge \idxinitial \leq \idx \leq \idxfinal \Leftrightarrow 
...
\begin{proof}
 TODO
\ifdefined\longversion
  Given the loop index \idx, the proof proceeds by induction on \idx.

  \subproof{\textbf{Base Case} : $\idx=\idxinitial$}
  By definition, as $\Psi$ is an inductive invariant under $\Init(B) $, the following holds:
  $$(\Init(B) \land \idx= \idxinitial) \Rightarrow \Psi(\idxinitial)$$
  Also by (a), we have :
  $$(\Phi(\idx) \land \idx=\idxinitial) \Leftrightarrow (\Init(B) \land \idx= \idxinitial) 
  \Rightarrow  \Psi(\idxinitial)$$

  \subproof{\textbf{Inductive Step} } 
  Now assume 
  $$(\forall j) (\idxinitial \leq j \leq \idx \Rightarrow (\phi(j) \Rightarrow \psi(j)))$$ 
  and let us prove that 
  $$(\forall j) (\idxinitial \leq j \leq \idx + 1 \Rightarrow (\phi(j) \Rightarrow \psi(j)))$$
  TODO
\fi
\end{proof}
...
\begin{proof}
  ...
  $a$ and current value of the loop index, $\Init(\curpattern) \equiv (a = a_0)$.
  $$(i = \idxinitial \land a = a_0) \impl \Inv{\idx}$$
  and since $\idx$ is exactly equal to $\idxinitial$ then
  $$(\Init(\curpattern) \land \idx = \idxinitial) \Leftrightarrow (\Inv{\idx} \land \idx = \idxinitial)$$
...
%\ifdefined\longversion
  ...
  $$\Init(\curpattern) \equiv (\forall j)(\idxinitial \leq j \leq \idxfinal \impl A[j] = A_0[j])$$\\
  When $i = \idxinitial$, $\nexists j$ such that $\idxinitial \leq j <i$, i.e $\seen{j}{\idx}=\mathit{false}$\\
  ...
  then 
  $$(\Init(\curpattern) \land i = \idxinitial) \Leftrightarrow (\Inv{\idx} \land \idx = \idxinitial)$$
