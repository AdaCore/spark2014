...
%\newcommand{\Pgroup}[1]{\ensuremath{\mathit{G}_{#1}}\xspace}
...
\newcommand{\initcond}[1]{\ensuremath{\mathit{Init(#1)}}}
\newcommand{\framecond}[1]{\ensuremath{\mathit{Frame(#1)}}}
\newcommand{\preservecond}[1]{\ensuremath{\mathit{Preserve(#1)}}}
...
%%
%%Proof preliminaires
\newcommand{\hoareconj}{\textsc{hoare-conj}\xspace}
\newcommand{\maxinv}{\textsc{max-inv}\xspace}
\newcommand{\sppost}{\textsc{sp-postcond}\xspace}
\newcommand{\spstrong}{\textsc{sp-strong}\xspace}
\newcommand{\spmonoton}{\textsc{sp-monotone}\xspace}
...
\Sga    & ::= & b  \rightarrow \Sloc :=  e &
\mathrm{guarded~assigns}\\
\Sloc   & ::= & x ~|~ X[a]  &
\mathrm{locations}\\
a       &  ::= &  n  ~|~ \Sloc  ~|~ e_1 + e_2 ~|~ e_1 - e_2 ~|~ e_1 *
e_2  &
...
\begin{definition} \emph{(Disjoints writings and well-fordmeness)}\\
...
\begin{definition} \emph{(Satisfaction under partial correctness $\vDash_p$ )}\\
We say that a Hoare triple $\{\phi\} P \{ \psi \}$ is \textit{satisfied under partial correcteness}, 
noted $\vDash_{p} \{\phi\} P \{ \psi \}$, \textbf{iff} for all states which satisfies $\phi$, 
...
precise ones under the same initialisation conditions for their corresponding loop patterns. 
...
$$
\sigma' \vDash_{p}  \spost{C}{P}  \Leftrightarrow \exists
\sigma. (\eval{C}{\sigma} \rightsquigarrow \sigma' \land \sigma \vDash_{p}  P)
$$
\end{definition} 

\begin{corollary}[Properties on $sp(c,P)$] The following are useful properties of predicate \spost{C}{P}:
\begin{align*}
[\sppost]  &   \quad \vDash_{p} \{P\} ~C ~ \{ \spost{C}{P} \} & \textnormal{ ~is actually a postconditon} \\
[\spstrong] &  \quad \vDash_{p} \{P\} ~C~ \{ Q\} \Rightarrow (\spost{C}{P} \Rightarrow Q )  & \textnormal{ is the strongest one} \\
[\spmonoton] &   \quad  (P \Rightarrow Q) \Rightarrow (\spost{C}{P} \Rightarrow \spost{C}{Q} ) & \textnormal{ ~monotonicity}\\
[\hoareconj] &  \quad \vDash_{p} \{P_1\} ~C~ \{ Q_1\} \land \vDash_{p} \{P_2\} ~C~ \{ Q_2\} \Rightarrow 
\vDash_{p} \{P_1 \land P_2\} ~C~ \{P_2 \land Q_2\}  & \textnormal{Hoare conjonction} \\
\end{align*}
\end{corollary}

\begin{proof}
\subproof{
To verify that (1) holds let $\sigma'$ states, there exists $\sigma$ such that 
...
\subproof{
To verify that (2) holds let assume that $\vDash_{par} \{ P \} C \{ Q \}$ and let 
$\sigma \vDash P$. If $\{C,\sigma\} \rightsquigarrow \sigma'$ then $\sigma \vDash P$ 
(because $\vDash_{par} \{ P \} C \{ Q \}$). So 
$\exists \sigma, \{C,\sigma\} \rightsquigarrow \sigma'$ and $\sigma \vDash P$
so clearly $\sigma' \vDash \spost{C}{P}$ by the definition of $\spost{S}{P}$.
}
\subproof{
To verify that (3) holds let $\sigma'$ states, such that 
$\sigma' \vDash \spost{C}{P}$, from the definition 3 of $\spost{C}{P}$ we get that
$\exists \sigma, \{C,\sigma\} \rightsquigarrow \sigma' \quad \mathsf{(a)}$ and 
$\sigma \vDash Q \quad \mathsf{(b)}$.\\
In particular $\sigma \vDash (P \Rightarrow Q) \quad \mathsf{(c)}$, 
therefore from $(b)$ and $(c)$ we get that $\sigma \vDash Q \quad \mathsf{(d)}$.
Now with $(a)$ and $(d)$ we get that 
...
\item \preservecond{\prog} : describes an inductive invariant using an expression $e$.
\end{enumerate}
...
$$\preservecond{\prog} \Rightarrow (e = e_0)$$
% 
...
in this case $$\preservecond{\prog} = (e = e_0)$$
\item $e$ may refer to (array or scalar) variables that are not modified in the loop;
in this case $$\preservecond{\prog} = \framecond{\prog}$$
...
$$\preservecond{\prog} = \framecond{\prog} \land (\forall j)(\idx \leq j \leq \idxfinal \Rightarrow A[j]=A_0[j])$$
\end{enumerate}
...
\begin{definition} \emph {(Inductive loop invariant under initialisation conditions)}\\
A formula $\Phi$ is an \textit{inductive invariant under initialisation conditions} \initcond{\prog} for the loop 
\hspace{1cm} $L = \KWloop  \idx \KWin \idxinitial ..\idxfinal ~ \KWexit \gexit \KWdo B ~\KWend$ \\
\textbf{iff}: 
\begin{itemize}
\item $(\initcond{\prog}  \wedge i=\idxinitial) \Rightarrow \phi$. 
      That is, $\phi$ holds after executing the initialisation;
\item $\vDash_{par} \{i \leq \idxfinal \wedge \neg \gexit \wedge\phi \} \texttt{B; i= i+1} \{ \phi \}$ holds. 
...
\begin{definition} \emph {(Most precise invariant under initialisation conditions)}\\
A formula $\phi$ is the \textit{most precise invariant under initialisation conditions}, 
...
\KWexit g_{exit}~ \KWdo B ~\KWend$} with $B$ a body loop not writing
the index \idx. Let 
 $\psi$ and  $\phi$  be two inductive invariants for $L$ under \initcond{\prog}
Let  $\Phi$ be
such that : 
\begin{itemize}
\item[(a)] $(\initcond{\prog} \land \idx = \idxinitial) \Rightarrow \phi(\idxinitial)$ 
\item[(b)] $sp(B, \framecond{\prog} \land \preservecond{\prog} \land \initcond{\prog} \land \idx = \idxinitial \land \neg g_{exit}^\at{\idx} \land \phi(\idx)) 
             \Leftrightarrow  
            \framecond{\prog} \land \preservecond{\prog} \land \idx = \idxinitial+1 \land g_{0_{exit}} \land  \phi(\idx+1)$
\item[(c)] $sp(B, \framecond{\prog} \land \preservecond{\prog} \land \idxinitial < \idx \leq \idxfinal \land \neg g_{exit}^\at{\idx} \land \phi(\idx)) 
             \Leftrightarrow  
            \framecond{\prog} \land \preservecond{\prog} \land \idxinitial < \idx \leq \idxfinal \land g_{0_{exit}} \land  \phi(\idx+1)$.
\end{itemize}
\noindent Then, $\phi$ is the most precise invariant for \prog under \initcond{\prog},
that is:
\begin{align*}
\initcond{\prog} \land \idx= \idxinitial \land \phi(\idx) \Rightarrow  \psi(\idx)\\
\framecond{\prog} \land \preservecond{\prog}  \land \idxinitial < i \leq \idxfinal \land \neg g_{exit}^\at{\idx} \land \phi(i) \Rightarrow \psi(i)  & \quad \quad [\maxinv]
\end{align*}
 %Then $\forall \idx) (\idxinitial \leq \idx \leq \idxfinal \Rightarrow (\phi(\idx) \Rightarrow \psi(\idx))$
\end{theorem}

\begin{proof}
  \subproof{\textbf{Init case} : $\idx=\idxinitial$}
  By definition, as $\psi$ is an inductive invariant under \initcond{\prog}, the following holds:
  $$\initcond{\prog} \land \idx= \idxinitial \Rightarrow \psi(\idxinitial)$$
  Obviously wa have thus :
  $$\initcond{\prog} \land \idx= \idxinitial \land \phi(\idxinitial) \Rightarrow  \psi(\idxinitial)$$

  \subproof{\textbf{Presevation case : $\idx \geq \idxinitial$}} 
  Here the proof proceeds by induction on \idx.

  \subsubproof{\textbf{Base case : $\idx = \idxinitial+1$}}  
  We have by Init case
  $$\initcond{\prog} \land \idx= \idxinitial \land \phi(\idx) \Rightarrow  \idx= \idxinitial \land \psi(\idx)$$
  %
  $g_{exit}^\at{\idx}$ is a value preserving hence $\preservecond{\prog} \Rightarrow (g_{exit}^\at{\idx} = g_{0_{exit}})$ so can write
  \begin{align*}
    \framecond{\prog} \land \preservecond{\prog} \land \initcond{\prog} \land g_{0_{exit}} \land \idx= \idxinitial \land \phi(\idx) \Rightarrow \\
    \framecond{\prog} \land \preservecond{\prog} \land g_{0_{exit}} \land \idx= \idxinitial \land \psi(\idx)
  \end{align*}
  %
  Apply \spmonoton we get 
  \begin{align*}
    \spost{\texttt{B}}{\framecond{\prog} \land \preservecond{\prog} \land \initcond{\prog} \land g_{0_{exit}} \land \idx= \idxinitial \land \phi(\idx)} 
    \Rightarrow\\
    \spost{\texttt{B}}{\framecond{\prog} \land \preservecond{\prog} \land g_{0_{exit}} \land \idx= \idxinitial \land \psi(\idx)}\\
  \end{align*}
  %
  By hypothesis $b$
  \begin{align*}
    sp(B, \framecond{\prog} \land \preservecond{\prog} \land \initcond{\prog} \land \idx = \idxinitial \land \neg g_{exit}^\at{\idx} \land \phi(\idx)) 
             \Leftrightarrow\\
            \framecond{\prog} \land \preservecond{\prog} \land \idx = \idxinitial+1 \land g_{0_{exit}} \land  \phi(\idx+1)
  \end{align*}
  %
  ...
  \begin{align*}
    \framecond{\prog} \land \preservecond{\prog} \land \idx = \idxinitial+1 \land g_{0_{exit}} \land  \phi(\idx+1)
    \Rightarrow\\
    \spost{\texttt{B}}{\framecond{\prog} \land \preservecond{\prog} \land g_{0_{exit}} \land \idx= \idxinitial \land \psi(\idx)} \quad \mathsf{(1)}
  \end{align*}
  %
  $\psi$ is an inductive invariant,  and thus, the Hoare triple below holds.
  $$
    \{\idxinitial \leq \idx \leq \idxfinal \land \neg g_{exit}^\at{\idx} \land \psi(\idx) \} 
    \texttt{B;\idx:=i+1} 
    \{\psi(\idx) \} 
  $$
  %
  By the hypothesis that the body loop $B$ doest not writing the index \idx, we get the following Hoare triple
  $$
    \{\idxinitial \leq i \leq \idxfinal \land \neg g_{exit}^\at{\idx} \land \psi(i) \} 
    \texttt{B} 
    \{\psi(i+1) \} \quad \mathsf{(2)}
  $$
  %
  \preservecond{\prog} and \framecond{ \prog} are inductive invariants,  and thus, the Hoare triples below hold.
  \begin{align*}
    &\{ \preservecond{\prog} \} \texttt{B} \{ \preservecond{\prog} \}& \\
    &\{ \framecond{ \prog} \} \texttt{B} \{ \framecond{ \prog} \}& \quad \mathsf{(3)}
  \end{align*}
  %
  We instantiate the property \hoareconj on $\mathsf{(2)}$ and $\mathsf{(3)}$. 
  \begin{align*}
    \{\framecond{ \prog} \land \preservecond{\prog} \land \idxinitial \leq i \leq \idxfinal \land \neg g_{exit}^\at{\idx} \land  \psi(i) \}
    \texttt{B} 
    \{ \framecond{ \prog} \land \preservecond{\prog} \land \psi(i+1)\}  \quad \mathsf{(4)}
  \end{align*}  
  %
  We instantiate the property \spstrong on $\mathsf{(4)}$
  \begin{align*}
    \spost{\texttt{B}}{\idxinitial \leq i \leq \idxfinal \land \neg g_{exit}^\at{\idx} \land \framecond{ \prog} \land  \preservecond{\prog} \land \psi(i) } 
    &\Rightarrow &\\
    \framecond{ \prog} \land \preservecond{\prog} \land \psi(i+1) \mathsf{(5)}
  \end{align*}  
  With $\mathsf{(1)}$ and $\mathsf{(5)}$ for ($\idx = \idxinitial+1$) we get
  \begin{align*}
    \framecond{\prog} \land \preservecond{\prog} \land \idx = \idxinitial+1 \land g_{0_{exit}} \land  \phi(\idx+1)
    \Rightarrow\\
    \framecond{ \prog} \land \preservecond{\prog} \land \idx = \idxinitial+1 \land \psi(i+1) 
  \end{align*}


  \subsubproof{\textbf{Inductive step}}  
  Following is the inductive hypothesis
  $$\framecond{\prog} \land \preservecond{\prog}  \land \idxinitial < i \leq \idxfinal \land \phi(i) \Rightarrow \psi(i)$$
  \begin{align*}
  \framecond{\prog} \land \preservecond{\prog} \land g_{0_{exit}} \land \idxinitial < i \leq \idxfinal \land \phi(i) \Rightarrow\\
  \framecond{\prog} \land \preservecond{\prog} \land g_{0_{exit}} \land  \psi(i)
  \end{align*}
  %
  Apply \spmonoton we get 
  \begin{align*}
    \spost{\texttt{B}}{\framecond{\prog} \land \preservecond{\prog} \land g_{0_{exit}} \land \idxinitial < i \leq \idxfinal \land \phi(i)} 
    \Rightarrow\\
    \spost{\texttt{B}}{\framecond{\prog} \land \preservecond{\prog} \land g_{0_{exit}} \land  \psi(i)}\\
  \end{align*}
  %
  ...
  \begin{align*}
   \spost{\texttt{B}}{\framecond{\prog} \land \preservecond{\prog} \land g_{0_{exit}} \land \idxinitial < i \leq \idxfinal \land \phi(i)}
   \Leftrightarrow \\
   \framecond{\prog} \land \preservecond{\prog} \land \idxinitial < \idx \leq \idxfinal \land  \phi(\idx+1) 
  \end{align*}
  so 
  \begin{align*}
   \framecond{\prog} \land \preservecond{\prog} \land \idxinitial < \idx \leq \idxfinal \land  \phi(\idx+1) 
   \Rightarrow \\
    \spost{\texttt{B}}{\framecond{\prog} \land \preservecond{\prog} \land g_{0_{exit}} \land  \psi(i)} \quad \mathsf{(6)}
  \end{align*}
  %
  By $\mathsf{(6)}$ and $\mathsf{(5)}$ we get 
  \begin{align*}
    \framecond{\prog} \land \preservecond{\prog} \land \idxinitial \leq \idx \leq \idxfinal \land  \phi(\idx+1)
    &\Rightarrow &\\
    \framecond{ \prog} \land \preservecond{\prog} \land \idxinitial \leq \idx \leq \idxfinal \land \psi(i+1)
  \end{align*}  
  Finally $\phi(\idx+1) \Rightarrow \psi(i+1)$ as required.
\end{proof}
...
\begin{proof}
  ...
  $a$ and current value of the loop index, $\initcond{\prog} \equiv (a = a_0)$.
  $$(i = \idxinitial \land a = a_0) \impl \Inv{\idx}$$
  and since $\idx$ is exactly equal to $\idxinitial$ then
  $$\initcond{\prog} \land \idx = \idxinitial) \Leftrightarrow (\Inv{\idx} \land \idx = \idxinitial)$$
...
%\ifdefined\longversion
  ...
  $$\initcond{\prog} \equiv (\forall j)(\idxinitial \leq j \leq \idxfinal \impl A[j] = A_0[j])$$\\
  When $i = \idxinitial$, $\nexists j$ such that $\idxinitial \leq j <i$, i.e $\seen{j}{\idx}=\mathit{false}$\\
  ...
  then 
  $$(\initcond{\prog} \land i = \idxinitial) \Leftrightarrow (\Inv{\idx} \land \idx = \idxinitial)$$
