...
package body Flow_Classwide is
   ...
   procedure Check_Classwide_Global
     (E : Entity_Id; Scope : Flow_Scope; Valid : in out Boolean)
   with Pre => Is_Subprogram (E) and then Has_Controlling_Formal_Or_Result (E);
   --  Enforce the rules of SRM 6.1.6

   procedure Check_Classwide_Depends
     (E : Entity_Id; Scope : Flow_Scope; Valid : in out Boolean)
   with Pre => Is_Subprogram (E) and then Has_Controlling_Formal_Or_Result (E);
   --  Enforce the rules of SRM 6.1.6
   ...
   is
      ...
      function Mode (F : Flow_Id) return String is
            ...
            Error_Msg_Flow
              (E          => E,
               Msg        =>
                 "& is an "
                 & Mode (F)
                 & " of overriding subprogram, but it "
   ...
   is
      ...
      My_Dep  : constant Dependency_Maps.Map := Get_Or_Make_Depends (E);
   ...
   begin
      ...
      begin
         while Present (Anc_Ptr) loop
            ...
            My_To_Anc.Insert
              (Direct_Mapping_Id (My_Ptr), Direct_Mapping_Id (Anc_Ptr));
            Anc_To_My.Insert
              (Direct_Mapping_Id (Anc_Ptr), Direct_Mapping_Id (My_Ptr));
            Next_Formal (Anc_Ptr);
            Next_Formal (My_Ptr);
         end loop;
         if Ekind (E) = E_Function then
      ...
      for C in My_Dep.Iterate loop
         ...
         begin
            ...
            if Present (My_Output) then
               Anc_Inputs :=
                 Anc_Dep
                   (if My_To_Anc.Contains (My_Output)
                    then My_To_Anc (My_Output)
                    else My_Output);
               Tmp := My_Inputs;
               for F of Anc_Inputs loop
                  Tmp.Exclude
                    (if Anc_To_My.Contains (F) then Anc_To_My (F) else F);
               end loop;
            end if;
   ...
   procedure Check_Classwide_Contracts (E : Entity_Id) is
