...
package Flow_Utility is
   ...
   --     - if we have a dependency we work out the globals from that, else
   --     - globals generated by flow in phase 1 for subprograms in SPARK, else
   ...
   --  If Map_In_Owning_Scope is True, map formals when Scop is the instance's
   --  owning scope; this is needed while serializing a generated contract for
   --  callers outside the instance.
