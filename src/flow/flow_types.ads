...
package Flow_Types is
   ...
   type Analyzed_Subject_Kind is (Kind_Subprogram, Kind_Task, Kind_Package);
   ...
   type Param_Mode is
     (Mode_Invalid, Mode_Proof, Mode_In, Mode_In_Out, Mode_Out);
   pragma Ordered (Param_Mode);

   subtype In_Global_Modes is Param_Mode range Mode_Proof .. Mode_In;

   subtype Initialized_Global_Modes is
     Param_Mode range Mode_Proof .. Mode_In_Out;

   subtype Exported_Global_Modes is Param_Mode range Mode_In_Out .. Mode_Out;
   ...
   subtype Initial_Or_Final_Variant is
   ...
   subtype Declarative_Part is
     Any_Declarative_Part range Visible_Part .. Body_Part;
   ...
   with
     Dynamic_Predicate =>
       (Flow_Scope.Ent = Empty) = (Flow_Scope.Part = Null_Part);
   ...
   with Dynamic_Predicate => (for all E of Global_Set => Is_Global_Entity (E));
   ...
   with
     Dynamic_Predicate =>
       (for all G of Global_Nodes.Proof_Ins =>
          not Global_Nodes.Inputs.Contains (G)
          and then not Global_Nodes.Outputs.Contains (G));
   ...
   with
     Dynamic_Predicate =>
       (for all G of Global_Names.Proof_Ins =>
          not Global_Names.Inputs.Contains (G)
          and then not Global_Names.Outputs.Contains (G));
   ...
   Null_Flow_Id : constant Flow_Id :=
     Flow_Id'(Kind => Null_Value, Variant => Normal_Use);

   Null_Export_Flow_Id : constant Flow_Id :=
     Flow_Id'(Kind => Synthetic_Null_Export, Variant => Normal_Use);
   ...
   function Direct_Mapping_Id
     (N       : Node_Or_Entity_Id;
      ...
      Facet   : Variable_Facet_T := Normal_Part) return Flow_Id
   with Pre => Present (N);
   ...
   function Record_Field_Id (N : Node_Id) return Flow_Id
   with Pre => Present (N) and then Nkind (N) = N_Selected_Component;
   --  Create a Flow_Id for the given record field

   function Add_Component (F : Flow_Id; Comp : Entity_Id) return Flow_Id
   with
     Pre  =>
   ...
   function Magic_String_Id
     (S : Entity_Name; Variant : Flow_Id_Variant := Normal_Use) return Flow_Id;
   ...
   is (F.Kind in Direct_Mapping | Record_Field and then F.Facet = The_Tag);
   --  Returns True if the given Flow_Id represents the tag of a classwide type

   function Is_Bound (F : Flow_Id) return Boolean
   is (F.Kind in Direct_Mapping | Record_Field and then F.Facet = The_Bounds)
   ...
   function Is_Volatile_For_Reading
     (F : Flow_Id)
      return Boolean
   ...
   function Change_Variant
     (F : Flow_Id; Variant : Flow_Id_Variant) return Flow_Id
   with Pre => Present (F);
   ...
   function Flow_Id_To_String
     (F : Flow_Id; Pretty : Boolean := False) return String
   ...
   package Flow_Id_Surjection is new
     Ada.Containers.Hashed_Maps
       (Key_Type        => Flow_Id,
        Element_Type    => Flow_Id,
        Hash            => Hash,
   ...
   package Ordered_Flow_Id_Sets is new
   ...
   function To_Ordered_Flow_Id_Set
     (S : Flow_Id_Sets.Set) return Ordered_Flow_Id_Sets.Set;
   --  Convert a hashed flow id set into an ordered flow id set

   function To_Entire_Variables (S : Flow_Id_Sets.Set) return Flow_Id_Sets.Set
   with
     Post =>
       (for all X of To_Entire_Variables'Result => X.Kind /= Record_Field);
   ...
   function To_Flow_Id_Set
     (S : Node_Sets.Set; View : Flow_Id_Variant := Normal_Use)
      return Flow_Id_Sets.Set
   with
     Post => (for all F of To_Flow_Id_Set'Result => F.Kind = Direct_Mapping);
   --  Convert a node set to a Flow_Id set

   function Change_Variant
     (FS : Flow_Id_Sets.Set; Variant : Flow_Id_Variant)
      return Flow_Id_Sets.Set;
   ...
   type V_Attributes is record
      ...
      --  The set of all nodes that can have indirect calls, such as calls to
      --  equality or predicate functions.
   ...
   Null_Node_Attributes : constant V_Attributes :=
     (Null_Attributes
      with delta Is_Null_Node => True, Is_Program_Node => True);
