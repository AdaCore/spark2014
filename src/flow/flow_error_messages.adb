...
package body Flow_Error_Messages is
   ...
   is
      ...
      begin
         ...
         --  Alignment checks are static, so we make them of high severity.

         elsif Tag = VC_UC_Align_Overlay
   ...
   begin
      ...
      elsif Tag = VC_Discriminant_Check
        and then Nkind (N) = N_Assignment_Statement
        and then Nkind (Name (N)) in N_Identifier | N_Expanded_Name
        and then Is_Formal (Entity (Name (N)))
        and then Has_Mutable_Discriminants (Retysp (Etype (Entity (Name (N)))))
      then
         return
           "formal parameter "
           & Pretty_Source_Name (Name (N))
   ...
   begin
      ...
      elsif Tag = VC_UC_Align_Overlay then
      ...
      --  an explanation for a check involving Func'Result.
         ...
         begin
            ...
            if Tag = VC_Discriminant_Check
              and then Nkind (N) = N_Assignment_Statement
              and then Nkind (Name (N)) in N_Identifier | N_Expanded_Name
              and then Is_Formal (Entity (Name (N)))
              and then
                Has_Mutable_Discriminants (Retysp (Etype (Entity (Name (N)))))
            then
               if Is_Private_Type (Etype (Entity (Name (N)))) then
                  return
                    Create
                      ("introduce a wrapper for the full view of "
                       & Pretty_Source_Name (Etype (Entity (Name (N))))
                       & " defined "
                       & Get_Line_Number (N, Sloc (Etype (Entity (Name (N))))),
                       Secondary_Loc => Secondary_Loc);
               else
                  return
                    Create
                      ("add precondition (not "
                       & Raw_Source_Name (Entity (Name (N)))
                       & "'Constrained)"
                       & " to subprogram "
   ...
   begin
      ...
      case Kind is
         ...
         when VC_UC_Align_UC                      =>
            return
              "alignment of source type of unchecked conversion might not be "
              & "an integral multiple of alignment of target type";
   ...
   begin
      case Kind is
         ...
         when VC_UC_Align_UC                      =>
            return
              Prefix
              & "alignment of source type of unchecked conversion is "
              & "compatible with alignment of target type";
