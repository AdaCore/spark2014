...
package Flow_Refinement is
   ...
   function Present (S : Flow_Scope) return Boolean
   is (Present (S.Ent));
   --  Returns True iff S is not the Null_Flow_Scope

   function No (S : Flow_Scope) return Boolean
   is (No (S.Ent));
   ...
   function Is_Visible (N : Node_Id; S : Flow_Scope) return Boolean;
   ...
   function Is_Visible
     (Target_Scope : Flow_Scope; Looking_From : Flow_Scope) return Boolean;
   --  Returns True iff Target_Scope is visible from Looking_From

   function Get_Flow_Scope (N : Node_Id) return Flow_Scope
   with
     Pre  => Nkind (N) /= N_Subunit,
     Post =>
       (if Present (Get_Flow_Scope'Result)
        then
          not Is_Generic_Unit (Get_Flow_Scope'Result.Ent)
          and then not Is_Tagged_Predefined_Eq (Get_Flow_Scope'Result.Ent));
   ...
   function State_Refinement_Is_Visible
     (E : E_Abstract_State_Id; S : Flow_Scope) return Boolean;
   ...
   function Up_Project (Var : Flow_Id; Scope : Flow_Scope) return Flow_Id
   with
     Post =>
       Up_Project'Result = Var or else Is_Abstract_State (Up_Project'Result);
   ...
   with
     Pre  =>
       ...
       and then (for all Var of Vars.Inputs => Var.Variant = In_View)
       and then (for all Var of Vars.Outputs => Var.Variant = Out_View),
     Post =>
   ...
   with
     Pre  => (for all V of Vars => V in N_Entity_Id),
     Post => (for all V of Down_Project'Result => V in N_Entity_Id);
   --  Same as above, but for many nodes

   function Down_Project
     (Var : Flow_Id; S : Flow_Scope) return Flow_Id_Sets.Set;
   --  Given a variable Var and a scope S, recursively expand abstract states
   --  whose refinement is visible in S.
   ...
   function Find_In_Initializes (E : N_Entity_Id) return Entity_Id
   with
     Pre  => Ekind (E) in E_Abstract_State | E_Constant | E_Variable,
     Post =>
       (if Present (Find_In_Initializes'Result)
        then Find_In_Initializes'Result in E | Encapsulating_State (E));
