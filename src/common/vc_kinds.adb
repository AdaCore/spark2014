...
package body VC_Kinds is
   ...
   is (case Kind is
         ...
         when Dead_Code => "A statement is never executed.",
         ...
         when Depends_Wrong => "Extra input item in the dependency clause.",
         ...
         when Illegal_Update => "Illegal write of a global input.",
         ...
         when Side_Effects => "A function with side effects.",
         when Stable => "A loop with stable statement.",
         ...
         when Unused_Global => "A global object is never used.",
   ...
   is (case Kind is
         ...
         when Warn_Unreferenced_Function => "analyzing unreferenced function",
   ...
   is (case Kind is
         ...
         when Lim_No_Return_Function => "a function annotated as No_Return",
   ...
   begin
      return
        (case Kind is
           ...
           when VC_Loop_Invariant_Init => "loop invariant in first iteration",
           when VC_Loop_Invariant_Preserv =>
           ...
           when VC_Container_Aggr_Check => "Container_Aggregates annotation",
           when VC_Reclamation_Check => "reclamation annotation",
           ...
           when VC_Stronger_Post_Access =>
             ...
             & " target",
           when VC_Inconsistent_Pre => "precondition always False",
           when VC_Inconsistent_Post => "postcondition always False",
           when VC_Inconsistent_Assume => "pragma Assume always False",
           when VC_Unreachable_Branch => "unreachable branch",
           when VC_Dead_Code => "unreachable code",
           when VC_Initialization_Check => "use of an uninitialized variable",
           when VC_Unchecked_Union_Restriction =>
   ...
   is (case Kind is
         when Aliasing => "aliasing between subprogram parameters",
         when Call_In_Type_Invariant => "invalid call in type invariant",
         when Call_To_Current_Task =>
         ...
         when Export_Depends_On_Proof_In =>
           "subprogram output depends on a Proof_In global",
         when Ghost_Wrong => "non-ghost output of ghost subprogram",
         when Global_Missing => "incomplete Global or Initializes contract",
         when Global_Wrong =>
         ...
         when Ineffective =>
           "a statement with no effect on subprogram's outputs",
         when Initializes_Wrong => "an extra item in the Initializes contract",
         when Inout_Only_Read =>
           "an IN OUT parameter or an In_Out global that is not written",
         when Missing_Return =>
           "all execution paths raise exceptions or do not return",
         ...
         when Refined_State_Wrong =>
           "constant with no variable inputs "
           & "as an abstract state's constituent",
         when Side_Effects => "function with side effects",
         when Stable => "loop with stable statement",
         when Subprogram_Termination =>
           "subprogram with aspect Always_Terminates may not terminate",
         when Uninitialized => "use of an uninitialized variable",
         when Unused_Global => "global object is not used",
         when Unused_Initial_Value => "initial value of an object is not used",
         when Unused_Variable => "object is not used",
         when Volatile_Function_Without_Volatile_Effects =>
   ...
   is (case Kind is
         ...
         when Warn_Potentially_Invalid_Read => "potentially-invalid-read",
         ...
         when Warn_Useless_Relaxed_Init_Obj => "useless-relaxed-init-object",
         when Warn_Variant_Not_Recursive => "variant-no-recursion",

         --  Warnings guaranteed to be issued
         when Warn_Imprecisely_Supported_Address =>
           "imprecise-address-specification",
         when Warn_Assumed_Always_Terminates => "assumed-always-terminates",
   ...
   begin
      return
        (case V.T is
           ...
           when Cnt_Array => Create_Array (V.Array_Indices, V.Array_Others),
