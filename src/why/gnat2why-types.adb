...
package body Gnat2Why.Types is
   ...
   procedure Generate_Type_Completion (E : Type_Kind_Id) is
      ...
      begin
         ...
         --  For incomplete types with entirely static invariants, add a
         --  defining axiom for the static_constraints predicate. It is a
         --  call to its dynamic invariant predicate with the flags set for
         --  components of access types.

         if Is_Access_Type (E)
           and then not Is_Access_Subprogram_Type (E)
           and then Designates_Incomplete_Type (E)
           and then Type_Has_Static_Constraints (Directly_Designated_Type (E))
         then
            declare
            ...
            begin
               ...
               begin
                  ...
                  Emit (Th, New_Defining_Bool_Axiom
                        (Fun_Name => To_String (WNE_Static_Constraint),
                         Name     => E_Symb (E, WNE_Static_Constraint),
                         Binders  =>
                           (1 =>
                              Binder_Type'(B_Name => Main_Arg, others => <>)),
                         Dep_Kind => EW_Axdep_Pred,
                         Def      => +Def));
               end;
               ...
               if Has_Init_Wrapper (E) then
                  ...
                  begin
                     ...
                     Emit (Th, New_Defining_Bool_Axiom
                           (Fun_Name =>
                              To_String (WNE_Static_Constraint) & "__r",
                            Name     => E_Symb
                              (E, WNE_Static_Constraint, Relaxed_Init => True),
                            Binders  =>
                              (1 => Binder_Type'
                                 (B_Name => Main_Arg, others => <>)),
                            Dep_Kind => EW_Axdep_Pred,
                            Def      => +Def));
                  end;
               end if;
