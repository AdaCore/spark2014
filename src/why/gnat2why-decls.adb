...
package body Gnat2Why.Decls is
   ...
   procedure Translate_Constant_Value (E : E_Constant_Id) is
      ...
      function Can_Be_Translated_As_Value (Expr : Node_Id) return Boolean
      is (not Expression_Contains_Old_Or_Loop_Entry (Expr)
          ...
          and then
            (not Within_Protected_Type (E)
             or else Is_Static_Expression (Expr)));
      --  Return True if Expr can be used in the definition axiom of the
      --  constant generated for E. References to Loop_Entry and Old might
      --  occur in constants generated by the frontend.
   ...
   begin
      ...
      --  If E has an expression, we try to generate a defining axiom for E

      elsif Present (Expr) and then Can_Be_Translated_As_Value (Expr) then
         declare
            B      : constant Item_Type :=
              Ada_Ent_To_Why.Element (Symbol_Table, E);
            pragma Assert (B.Kind = Regular and then not B.Main.Mutable);
      ...
      --  Constant overlays are always imported. Use a UC function to compute
      --  their value.

      elsif Present (Overlaid_Entity (E)) then
         declare
            Obj_Ty  : constant Type_Kind_Id := Type_Of_Node (E);
            Pref    : constant Node_Id := Prefix (Get_Address_Expr (Decl));
            Pref_Ty : constant Type_Kind_Id := Type_Of_Node (Pref);
            Params  : constant Transformation_Params :=
              (Logic_Params with delta Ref_Allowed => True);
            W_Pref  : constant W_Term_Id :=
              Transform_Term (Pref, Type_Of_Node (Pref_Ty), Params);

         begin
            pragma Assert (Is_Overlay_Handled_As_UC (E).Ok);

            --  Generate UC modules for the translation of E and emit the
            --  defining axiom.

            Create_Module_For_UC_If_Needed
              (Pref_Ty, Obj_Ty, Is_Potentially_Invalid (E));

            --  Emit an axiom for the definition of E if W_Pref does not
            --  contain any dereferences.

            if not Has_Dereference_Or_Any_Or_Self (W_Pref) then
               declare
                  B     : constant Item_Type :=
                    Ada_Ent_To_Why.Element (Symbol_Table, E);
                  UC_Id : constant W_Identifier_Id :=
                    Get_UC_Function
                      (Pref_Ty,
                       Obj_Ty,
                       Potentially_Invalid => B.Valid.Present);
                  Def   : W_Term_Id :=
                    New_Call
                      (Name => UC_Id,
                       Args => (1 => +W_Pref),
                       ...
                       (Th,
                        New_Defining_Axiom
                          (Ada_Node => E,
                           ...
                           Binders  => (1 .. 0 => <>),
                           Def      =>
                             +New_Function_Valid_Flag_Access (Obj_Ty, +Def)));
                     Def :=
            ...
            --  Output information on reason for imprecise handling of overlay

            declare
               Precise_Overlay : constant True_Or_Explain :=
                 Is_UC_With_Precise_Definition
                   (Pref_Ty, Obj_Ty, Is_Potentially_Invalid (E));

            begin
               if not Precise_Overlay.Ok then
                  Warning_Msg_N
                    (Warn_Imprecise_Overlay,
                     ...
                     Msg =>
                       Create_N
                         (Warn_Imprecise_Overlay,
                          Names => [To_String (Precise_Overlay.Explanation)]));
               end if;
            end;
         end;
      end if;
   ...
   procedure Translate_Variable (E : Object_Kind_Id) is
      ...
      Call     : W_Expr_Id := Why_Empty;
      --  Call to the UC function if we are translating an overlay as a UC
      Th       : Theory_UC;
      ...
      with Pre => (if Present (Def) then Mutable and then Present (Alias));
      ...
      --  for mutable parts of overlays. If Def is supplied, use it for the
   ...
   begin
      --  For overlays, generate the UC modules needed for the translation
      --  and use it to initialize Call.

      if Present (Alias) then
         declare
            Obj_Ty          : constant Type_Kind_Id := Type_Of_Node (E);
            Root_Ty         : constant Type_Kind_Id := Type_Of_Node (Alias);
            Precise_Overlay : True_Or_Explain := Is_Overlay_Handled_As_UC (E);
         begin
            if Precise_Overlay.Ok then
               pragma Assert (not Is_Potentially_Invalid (E));

               Create_Module_For_UC_If_Needed
                 (Root_Ty, Obj_Ty, Potentially_Invalid => False);

               Create_Module_For_UC_If_Needed
                 (Obj_Ty, Root_Ty, Potentially_Invalid => False);
            ...
            if Var.Fields.Present then
               ...
               Emit_Global_Ref_Or_Function
                 (Name    => Var.Fields.Binder.B_Name,
                  ...
                  Def     =>
                    (if Call /= Why_Empty
                     then New_Fields_Access (Name => Call, Ty => Var.Typ)
                     else Why_Empty));
            end if;
         ...
         when UCArray                =>
            ...
            Emit_Global_Ref_Or_Function
              (Name    => Var.Content.B_Name,
               ...
               Def     =>
                 (if Call /= Why_Empty
                  then Array_Convert_To_Base (EW_Term, Call)
                  else Why_Empty));
