...
package Why.Sinfo is
   ...
   --
   --  The only difference between EW_Term and EW_Pterm is wrt handling of
   --  logic function calls, for which we introduce an assume for EW_Pterm
   --  (in programs) and an epsilon for EW_Term (in logic), to add the
   --  postcondition in the context of the call. See New_Function_Call
   --  in why-gen-expr.adb
