...
package body Why.Gen.Scalars is
   ...
   procedure Define_Enumeration_Rep_Convertions (Th : Theory_UC; E : Entity_Id)
   with
     Pre =>
       Is_Enumeration_Type (Retysp (E))
   ...
   procedure Define_Scalar_Attributes
     (Th : Theory_UC; E : Entity_Id; Base_Type : W_Type_Id);
   ...
   function Ureal_Name (R : Ureal) return String
   is (UI_Image (Norm_Num (R), Decimal)
       & "_"
   ...
   procedure Create_Fixed_Point_Theory_If_Needed (Typ : Entity_Id) is
      use Name_Id_Fixed_Point_Map;

      Module_Name : constant Symbol :=
        Get_Fixed_Point_Theory_Name (Typ => Typ);
      Module      : constant W_Module_Id :=
        New_Module (File => No_Symbol, Name => Img (Module_Name));
      ...
      Num_Small : constant W_Name_Id := New_Name (Symb => NID ("num_small"));
      Den_Small : constant W_Name_Id := New_Name (Symb => NID ("den_small"));
      ...
      Subst :=
        (1 =>
           New_Clone_Substitution
             (Kind => EW_Function, Orig_Name => Num_Small, Image => Num_Small),
         2 =>
           New_Clone_Substitution
             (Kind      => EW_Function,
              Orig_Name => Den_Small,
   ...
   is
      ...
      Small_R     : constant Ureal :=
        (if Has_Fixed_Point_Type (Typ_Right)
         then Small_Value (Retysp (Typ_Right))
         elsif Has_Integer_Type (Typ_Right)
         then Ureal_1
      ...
      Small_Op     : constant Ureal :=
        (if Has_Fixed_Point_Type (Typ_Result)
         then Small_Value (Retysp (Typ_Result))
         elsif Has_Integer_Type (Typ_Result)
         then Ureal_1
         else raise Program_Error);
   ...
   ------------------------------------------------------
   -- Declare_Additional_Symbols_For_Unsigned_Overflow --
   ------------------------------------------------------

   procedure Declare_Additional_Symbols_For_Unsigned_Overflow (Th : Theory_UC)
   is
      High : constant Uint :=
        Expr_Value (High_Bound (Get_Range (Standard_Long_Long_Unsigned)));
        ...
        (Th,
         Why.Gen.Binders.New_Function_Decl
           (Domain      => EW_Term,
            ...
            Binders     => (2 .. 1 => <>),
            Return_Type => EW_Int_Type,
            Def         => New_Integer_Constant (Value => High)));
      Emit
        (Th,
         New_Clone_Declaration
            ...
            Origin        =>
              New_Module
                (File => Gnatprove_Standard_File,
                 Name => "Unsigned_Base_Range_Minimized_Overflow_Check"),
            Substitutions =>
              (1 =>
                 New_Clone_Substitution
                   (Kind      => EW_Function,
                    Orig_Name => New_Name (Symb => NID ("last")),
                    Image     => To_Local (Last)))));
   end Declare_Additional_Symbols_For_Unsigned_Overflow;

   -------------------------
   -- Declare_Scalar_Type --
   -------------------------

   procedure Declare_Scalar_Type (Th : Theory_UC; E : Entity_Id) is
      ...
      procedure Generate_Range_Predicate
        (Ty : W_Type_Id := Base_Why_Type (E); Name : Why_Name_Enum);
      ...
      function Compute_Clone_Subst return W_Clone_Substitution_Array is
           ...
           (if not Is_Static
            ...
            then
              (1 =>
                 New_Clone_Substitution
                   (Kind      => EW_Type_Subst,
                    Orig_Name => New_Name (Symb => NID ("rep_type")),
         ...
         Mod_Clone_Subst             : constant W_Clone_Substitution_Array :=
           (if Is_Static
              and then Is_Bitvector_Type_In_Why (E)
      ...
      is
         ...
         Need_Int_Suffix : constant Boolean :=
           not Is_Static
           or else (Is_Bitvector_Type_In_Why (E) and then Ty = EW_Int_Type);
      ...
      begin
              ...
              (Th,
               Why.Gen.Binders.New_Function_Decl
                 (Domain   => EW_Pred,
                  ...
                  Labels   => Symbol_Sets.Empty_Set,
                  Binders  =>
                    (1 => Binder_Type'(B_Name => Var, others => <>))));
              ...
              (Th,
               Why.Gen.Binders.New_Function_Decl
                 (Domain   => EW_Pred,
                  ...
                  Def      =>
                    +New_Identifier
                       (Name   => "uint_in_range x",
                        Module => MF_BVs (Base_Why_Type (E)).Module),
                  Location => No_Location,
                  Labels   => Symbol_Sets.Empty_Set,
                  Binders  =>
                    (1 =>
                       Binder_Type'
                         (B_Name =>
                            New_Identifier (Name => "x", Typ => EW_Int_Type),
                          others => <>))));

            return;
         elsif Has_Floating_Point_Type (E) then
         ...
         declare
            Static_Binders : constant Binder_Array :=
              Binder_Array'(1 => Binder_Type'(B_Name => Var, others => <>));
            Binders        : constant Binder_Array :=
              (if Is_Static
               then Static_Binders
               else
                 Binder_Array'
                   (1 => Binder_Type'(B_Name => Fst, others => <>),
                    2 => Binder_Type'(B_Name => Lst, others => <>))
                 & Static_Binders);
         begin
            Emit
              (Th,
      ...
      begin
         if Is_Static then
            if Is_Bitvector_Type_In_Why (E) then
               declare
                  Modulus_Val : constant Uint := Modulus (E);
               begin
                  return
                    (if Typ = EW_BitVector_8_Type
                     then
                       (if UI_Lt (Modulus_Val, UI_Expon (2, 8))
                        then Static_Modular_lt8
                        else Static_Modular_8)
                     elsif Typ = EW_BitVector_16_Type
                     then
                       (if UI_Lt (Modulus_Val, UI_Expon (2, 16))
                        then Static_Modular_lt16
                        else Static_Modular_16)
                     elsif Typ = EW_BitVector_32_Type
                     then
                       (if UI_Lt (Modulus_Val, UI_Expon (2, 32))
                        then Static_Modular_lt32
                        else Static_Modular_32)
                     elsif Typ = EW_BitVector_64_Type
                     then
                       (if UI_Lt (Modulus_Val, UI_Expon (2, 64))
                        then Static_Modular_lt64
                        else Static_Modular_64)
                     elsif Typ = EW_BitVector_128_Type
                     then
                       (if UI_Lt (Modulus_Val, UI_Expon (2, 128))
                        then Static_Modular_lt128
                        else Static_Modular_128)
                     else raise Program_Error);
               end;
               ...
               return
                 (if Typ = EW_Float_32_Type
                  then Static_Float32
                  elsif Typ = EW_Float_64_Type
                  then Static_Float64
                  elsif Typ = EW_Float_80_Type
                  then Static_Float80
                  else raise Program_Error);
            end if;
         else
   ...
   begin
      ...
      if not Is_Static then
         Emit
           (Th, New_Type_Decl (Name => Why_Name, Alias => Base_Type_In_Why));
      ...
      --  For other static types, declare an abstract type
         ...
         Emit
           (Th,
            New_Type_Decl (Name => Why_Name, Labels => Symbol_Sets.Empty_Set));
      end if;

      --  retrieve and declare the attributes first, last, small, and modulus

      Define_Scalar_Attributes
        (Th => Th, E => E, Base_Type => Base_Why_Type (E));

      --  define first_int and last_int for static modular types

      if Is_Static and then Is_Bitvector_Type_In_Why (E) then
         Emit
           (Th,
            Why.Atree.Builders.New_Function_Decl
              (Domain      => EW_Term,
               Name        =>
                 New_Identifier (Name => "first_int", Typ => EW_Int_Type),
               Binders     => (1 .. 0 => <>),
               Return_Type => EW_Int_Type,
               Labels      => Symbol_Sets.Empty_Set,
               Location    => No_Location,
         ...
         Emit
           (Th,
            Why.Atree.Builders.New_Function_Decl
              (Domain      => EW_Term,
               Name        =>
                 New_Identifier (Name => "last_int", Typ => EW_Int_Type),
               Binders     => (1 .. 0 => <>),
               Return_Type => EW_Int_Type,
               Labels      => Symbol_Sets.Empty_Set,
               Location    => No_Location,
      ...
      Generate_Range_Predicate
        (Name => (if Is_Static then WNE_Range_Pred else WNE_Dynamic_Property));
   ...
   is
      ...
      X_Ident : constant W_Identifier_Id :=
        New_Identifier (Domain => EW_Term, Name => "x", Typ => EW_Int_Type);
      ...
      is (New_Comparison
            (Symbol => Why_Eq,
             Left   => +X_Ident,
             Right  => New_Integer_Constant (Value => Enumeration_Rep (Lit)),
             Domain => Domain));

      function X_Is_Lit_Rep (Lit : Entity_Id) return W_Pred_Id
      is (+X_Is_Lit_Rep (Lit, EW_Pred));

      function X_Is_Lit_Pos (Lit : Entity_Id) return W_Expr_Id
      is (New_Comparison
            (Symbol => Why_Eq,
             Left   => +X_Ident,
             Right  => New_Integer_Constant (Value => Enumeration_Pos (Lit)),
             Domain => EW_Term));

      To_Rep_Cond        : W_Expr_Id;
      To_Rep_Then_Part   : W_Expr_Id;
      To_Rep_Elsif_Parts :
        W_Expr_Array (1 .. Integer (Num_Literals (Retysp (E))) - 2);
      ...
      To_Pos_Guard       :
        W_Pred_Array (1 .. Integer (Num_Literals (Retysp (E))));
   ...
   begin
      ...
      Emit
        (Th,
         Why.Gen.Binders.New_Function_Decl
           (Domain      => EW_Pterm,
            Name        => To_Local (E_Symb (E, WNE_Pos_To_Rep)),
            Binders     =>
              (1 => Binder_Type'(B_Name => X_Ident, others => <>)),
            Return_Type => EW_Int_Type,
            Labels      => Symbol_Sets.Empty_Set,
            Location    => No_Location,
            Def         => To_Rep_Def));
      ...
      Emit
        (Th,
         Why.Gen.Binders.New_Function_Decl
           (Domain      => EW_Prog,
            Name        =>
              To_Program_Space (To_Local (E_Symb (E, WNE_Rep_To_Pos))),
            Binders     =>
              (1 => Binder_Type'(B_Name => X_Ident, others => <>)),
            Return_Type => EW_Int_Type,
            Labels      => Symbol_Sets.Empty_Set,
   ...
   begin
         ...
         begin
                 ...
                 (if UI_Lt (Modulus_Val, UI_Expon (2, 8))
                  then
                    New_Modular_Constant
                      (Value => Modulus_Val, Typ => EW_BitVector_8_Type)
                  else Why_Empty)
               elsif Typ = EW_BitVector_16_Type
               then
                 (if UI_Lt (Modulus_Val, UI_Expon (2, 16))
                  then
                    New_Modular_Constant
                      (Value => Modulus_Val, Typ => EW_BitVector_16_Type)
                  else Why_Empty)
               elsif Typ = EW_BitVector_32_Type
               then
                 (if UI_Lt (Modulus_Val, UI_Expon (2, 32))
                  then
                    New_Modular_Constant
                      (Value => Modulus_Val, Typ => EW_BitVector_32_Type)
                  else Why_Empty)
               elsif Typ = EW_BitVector_64_Type
               then
                 (if UI_Lt (Modulus_Val, UI_Expon (2, 64))
                  then
                    New_Modular_Constant
                      (Value => Modulus_Val, Typ => EW_BitVector_64_Type)
                  else Why_Empty)
               elsif Typ = EW_BitVector_128_Type
               then
                 (if UI_Lt (Modulus_Val, UI_Expon (2, 128))
                  then
                    New_Modular_Constant
                      (Value => Modulus_Val, Typ => EW_BitVector_128_Type)
                  else Why_Empty)
               elsif Typ = EW_Int_Type
               then New_Integer_Constant (Value => Modulus_Val)
               else raise Program_Error);
            ...
            Emit
              (Th,
               Why.Atree.Builders.New_Function_Decl
                 (Domain      => EW_Pterm,
                  Name        => To_Local (E_Symb (E, WNE_Attr_Modulus)),
                  Binders     => (1 .. 0 => <>),
         ...
         begin
            Emit
              (Th,
               Why.Atree.Builders.New_Function_Decl
                 (Domain      => EW_Pterm,
                  Name        => To_Local (E_Symb (E, WNE_Small_Num)),
                  Binders     => (1 .. 0 => <>),
                  Return_Type => Base_Type,
                  Labels      => Symbol_Sets.Empty_Set,
                  Location    => No_Location,
            ...
            Emit
              (Th,
               Why.Atree.Builders.New_Function_Decl
                 (Domain      => EW_Pterm,
                  Name        => To_Local (E_Symb (E, WNE_Small_Den)),
                  Binders     => (1 .. 0 => <>),
                  Return_Type => Base_Type,
                  Labels      => Symbol_Sets.Empty_Set,
                  Location    => No_Location,
      ...
      if not (Type_Is_Modeled_As_Base (E) and then Is_Itype (E)) then
         declare
            Binders : Item_Array :=
              Get_Binders_From_Expression (Low_Bound (Rng));
         begin
            ...
            Emit
              (Th,
               New_Function_Decl
                 (Domain      => EW_Pterm,
                  Name        => To_Local (E_Symb (E, WNE_Attr_First)),
                  Items       => Binders,
                  Return_Type => Base_Type,
                  Location    => No_Location,
                  Labels      => Symbol_Sets.Empty_Set,
         ...
         declare
            Binders : Item_Array :=
              Get_Binders_From_Expression (High_Bound (Rng));
         begin
            Localize_Binders (Binders, Only_Variables => False);
            Emit
              (Th,
               New_Function_Decl
                 (Domain      => EW_Pterm,
                  Name        => To_Local (E_Symb (E, WNE_Attr_Last)),
                  Items       => Binders,
                  Return_Type => Base_Type,
                  Location    => No_Location,
                  Labels      => Symbol_Sets.Empty_Set,
   ...
   procedure Define_Scalar_Rep_Proj (Th : Theory_UC; E : Entity_Id) is
      ...
      begin
         if Rep_Type = EW_Float_32_Type then
            ...
            begin
               return
                 (if Rep_Type = EW_BitVector_8_Type
                  then
                    (if UI_Lt (Modulus_Val, UI_Expon (2, 8))
                     then Rep_Proj_Lt8
                     else Rep_Proj_8)
   ...
   begin
      ...
      if Is_Bitvector_Type_In_Why (E) then
         ...
         Add_With_Clause (Th, MF_Floats (Rep_Type).Module, EW_Clone_Default);
      end if;
   ...
   is
      ...
      R_Small   : constant Ureal :=
        (if Has_Fixed_Point_Type (Typ_Right)
         then Small_Value (Retysp (Typ_Right))
         elsif Has_Overflow_Operations (Typ_Right)
         then Ureal_1
         else raise Program_Error);
      ...
      Name : constant String :=
        To_String (WNE_Fixed_Point_Mult_Div_Prefix)
        & "__"
        & Ureal_Name (L_Small)
        & "__"
        & Ureal_Name (R_Small)
   ...
   function Get_Fixed_Point_Theory_Name (Typ : Entity_Id) return Symbol is
      Small : constant Ureal := Small_Value (Typ);
      Name  : constant String :=
        To_String (WNE_Fixed_Point_Prefix) & "__" & Ureal_Name (Small);
   begin
      return NID (Name);
   ...
   begin
      if Is_Bitvector_Type_In_Why (Ty) then
         ...
         return
           New_Float_Constant
             (Value => Expr_Value_R (N), Typ => Base_Why_Type (Ty));
      else
