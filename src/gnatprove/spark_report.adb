...
package body Spark_Report is
   ...
   begin
      ...
      if SPARK_Mode_OK then
         ...
         --  If nothing has produced checks, we generate:
         --  - an error when analysis was restricted with limit switches,
         --    even in quiet mode;
         --  - otherwise, a warning unless the "quiet" switch was present.
         --  In both cases, suppress the diagnostic in check/check all mode
         --  and when there were previous errors in gnatprove.
         ...
         then
            if Has_Limit_Switches then
               ...
               Ada.Text_IO.Put_Line
                 (Ada.Text_IO.Standard_Error,
                  "error: no checks generated by GNATprove");
               Ada.Text_IO.Put_Line
                 (Ada.Text_IO.Standard_Error,
                  "possible reason: wrong parameters to switches"
                  & " such as --limit-subp or --limit-line");

            elsif Verbosity /= Quiet_Level then
               Ada.Text_IO.Put_Line
                 (Ada.Text_IO.Standard_Error,
                  "warning: no checks generated by GNATprove");
               Ada.Text_IO.Put_Line
                 (Ada.Text_IO.Standard_Error,
      ...
      if Verbosity /= Quiet_Level and then not Has_Errors and then Has_Check
      then
         declare
