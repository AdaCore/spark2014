...
package body SPARK_Util is
   ...
   package body Exception_Sets is
      ...
      is (All_But => True, Exc_Set => Node_Sets.Empty_Set);
      ...
      procedure Disclose
        (S : Set; All_But : out Boolean; Exc_Set : out Node_Sets.Set) is
      begin
         All_But := S.All_But;
         Exc_Set := S.Exc_Set;
      end Disclose;
      ...
      is (All_But => False, Exc_Set => Node_Sets.Empty_Set);
      ...
      is (All_But => False, Exc_Set => Node_Sets.To_Set (E));
   ...
   function Can_Be_Deeply_Updated (Variable : Entity_Id) return Boolean
   is (declare
   ...
   function No_Deep_Updates
     (Stmts       : Local_CFG.Vertex_Sets.Set;
      Variable    : Entity_Id;
      Explanation : out Unbounded_String) return Boolean
   ...
   package Node_To_Exceptions is new
     Ada.Containers.Hashed_Maps
       (Key_Type        => Node_Id,
   ...
   begin
      return (if Has_Element (C) then Element (C) else Standard.Types.Empty);
   end Partial_View;
   ...
   is ((Is_Type (E) or else Ekind (E) = E_Constant)
   ...
   procedure Set_Call_Simulates_Contract_Dispatch (N : Node_Id) is
   begin
   ...
   begin
      while Present (P) loop
         ...
         if Nkind (P) = N_Assignment_Statement and then Prev = Expression (P)
         then
            return True;
         end if;
   ...
   is (not Is_Entity_Name (Alt) or else not Is_Type (Entity (Alt)));
   ...
   begin
      ...
      begin
         return
           S
           & Osint.Multi_Unit_Index_Character
   ...
   procedure Append (To : in out Node_Lists.List; Elmts : Node_Lists.List) is
   begin
      for E of Elmts loop
         To.Append (E);
      end loop;
   ...
   is (Nkind (N) not in N_Expanded_Name | N_Identifier
       or else Ekind (Entity (N)) not in E_Out_Parameter | E_In_Out_Parameter
       --  As an extension to Ada, GNAT allows Constrained on any object
       --  of a generic formal type. Unless the object has a type with
       --  discriminants, the result is statically True.
   ...
   is (not By_Reference (Obj) and then Is_By_Copy_Type (Etype (Obj)));
   ...
   function Call_Simulates_Contract_Dispatch (N : Node_Id) return Boolean is
   begin
   ...
   function Canonical_Entity
     (Ref : Entity_Id; Context : Entity_Id) return Entity_Id is
   begin
      ...
      then
         ...
         then
            ...
            then
               ...
               --  Altenatively, there are no non-scalar object declarations,
               --  precisely to avoid that their types have contradictory
               --  constraints across loop iterations.
   ...
   begin
      ...
      case C is
         ...
         --  get the names as it returns the character itself instead of a
         --  name for C greater than 160.
   ...
   begin
      ...
      elsif Is_Protected_Type (Full_Ty) or else not Is_Tagged_Type (Full_Ty)
      then
         return
           Entity_In_SPARK (Full_Ty)
   ...
   function Contains_Allocator (N : Node_Id) return Boolean is
      ...
      function Is_Allocator (N : Node_Id) return Traverse_Result is
      begin
      ...
      function Check_Allocator is new Traverse_More_Func (Is_Allocator);
   ...
   function Contains_Function_Call (N : Node_Id) return Boolean is
      ...
      function Is_Function_Call (N : Node_Id) return Traverse_Result is
      begin
   ...
   is (Is_Access_Object_Type (Retysp (Etype (Expr)))
   ...
   begin
      loop
         if No (S) then
            return Empty;
         elsif Ekind (S) in Entry_Kind | E_Function | E_Procedure then
            return S;
   ...
   is (if Is_Part_Of_Concurrent_Object (E)
   ...
   is
      ...
      function Aggr_Has_Relaxed_Init (Aggr : Node_Id) return Boolean is
         ...
         Exprs  : constant List_Id :=
           (if Nkind (Aggr) = N_Delta_Aggregate
            then No_List
   ...
   is ((Is_Type (E) and then Is_Standard_Boolean_Type (E)));
   ...
   begin
      ...
      if Copy_Requires_Init (Retysp (Etype (Subp))) then
         ...
         return
           Has_Relaxed_Initialization (Subp)
      ...
      while Present (Actual) loop
         ...
         begin
            if Nkind (Actual_Expl) in N_Has_Entity then
               ...
               begin
                  if Present (E_Actual)
                    and then Ekind (E_Actual) in E_Function | E_Procedure
                  then
                     ...
                     begin
                        Results.Include
                          (if Present (Renamed) then Renamed else E_Actual);
                     end;
                  end if;
               end;
            end if;
   ...
   function Get_Initialized_Object (N : N_Subexpr_Id) return Opt_Object_Kind_Id
   is
   ...
   is
      function Is_Handler (N : Node_Id) return Boolean
      is (Nkind (N) = N_Exception_Handler);
   ...
   is
      function Get_Entity (X : Node_Id) return Entity_Id
      is (if Nkind (X) in N_Expanded_Name | N_Identifier
            and then Is_Object (Entity (X))
          then Entity (X)
          else Empty);

      Alts : constant Node_Vectors.Vector := Terminal_Alternatives (Expr);
      Res  : constant Entity_Id :=
        Get_Entity (Get_Root_Expr (Alts.First_Element, Through_Traversal));
   begin
   ...
   is
      ...
      procedure Collect_Params is new
        Iterate_Call_Parameters (Store_Specialized_Param);
   ...
   is (not Get_Exceptions_For_Subp (E).Is_Empty);
   ...
   function Has_Volatile_Property
     (E : N_Entity_Id; P : Volatile_Pragma_Id) return Boolean is
   begin
   ...
   is (not Is_Declared_In_Main_Unit_Or_Parent (N)
   ...
   begin
      ...
      return
        Ekind (Root) in E_In_Parameter | E_In_Out_Parameter
   ...
   begin
      ...
      --  (everywhere else).
      return
        Ekind (E) in E_Component | E_Discriminant | E_In_Parameter
      ...
      loop
         declare
            Decl : constant Node_Id :=
              (if Is_Itype (Current)
               then Associated_Node_For_Itype (Current)
   ...
   function Is_Declared_In_Main_Unit_Or_Parent (N : Node_Id) return Boolean is
   ...
   pragma
     Annotate
       (Xcov,
        Exempt_On,
        "Only used in the predicate of Flow_Types.Global_Set");
   ...
   begin
      return
        Is_Pragma_Check (N, Name_Precondition)
        or else Is_Pragma_Check (N, Name_Pre)
        or else Is_Pragma_Check (N, Name_Postcondition)
        or else Is_Pragma_Check (N, Name_Post)
        or else Is_Pragma_Check (N, Name_Refined_Post)
        or else Is_Pragma_Check (N, Name_Static_Predicate)
        or else Is_Pragma_Check (N, Name_Predicate)
        or else Is_Pragma_Check (N, Name_Dynamic_Predicate);
   ...
   function Is_In_Statically_Dead_Branch (N : Node_Id) return Boolean is
      ...
      function Comes_From_This_Dead_Branch
        (If_Stmt, Stmt : Node_Id) return Boolean
   ...
   begin
      return
        Ekind (E) in E_Variable | E_Constant
   ...
   function Is_Null_Or_Reclaimed_Expr
     (Expr       : N_Subexpr_Id;
      ...
      Null_Value : Boolean := False) return Boolean is
   begin
      return
        Nkind (Expr) = N_Null
   ...
   begin
      case Nkind (Expr) is
         when N_Attribute_Reference =>
            if Attribute_Name (Expr)
               in Name_Access | Name_First | Name_Last | Name_Length
            then
               return Is_Path_Expression_Ann (Prefix (Expr));
            else
   ...
   is (Nkind (N) = N_Pragma and then Get_Pragma_Id (Pragma_Name (N)) = Name);
   ...
   begin
      return Present (Orig) and then Is_Pragma (Orig, Pragma_Assert_And_Cut);
   ...
   function Is_Rooted_In_Constant (Expr : N_Subexpr_Id) return Boolean is
      Root : constant Entity_Id :=
        (if Is_Path_Expression (Expr) then Get_Root_Object (Expr) else Empty);
   begin
      return
        Present (Root)
   ...
   begin
      if Ekind (Obj) = E_Variable and then Is_Quantified_Loop_Param (Obj) then
         declare
   ...
   is (Ekind (Formal) = E_In_Parameter
       and then Is_Anonymous_Access_Type (Etype (Formal))
       and then Is_Access_Subprogram_Type (Etype (Formal))
       and then Is_Function_Type (Directly_Designated_Type (Etype (Formal))));
   ...
   function Is_Strict_Subpath (Expr : N_Subexpr_Id) return Boolean is
      function Is_Strict_Witness (N : Node_Id) return Boolean
      is (Nkind (N) in N_Indexed_Component | N_Selected_Component);
   begin
   ...
   function Supported_Alias (Expr : Node_Id) return Entity_Id is
      ...
      Prefix_Expr    : constant Node_Id :=
        (if Simple_Address then Prefix (Expr) else Empty);
   ...
   begin
      ...
      if Buf.Chars (1) in '"' | ''' or else Buf.Chars (Name_Len) = ')' then
   ...
   begin
      return
        Is_Access_Variable (Base_Type (Etype (E)))
   ...
   package body Local_CFG is
      ...
      package Vertex_To_Neighborhood is new
        Ada.Containers.Hashed_Maps
          (Key_Type        => Vertex,
      ...
      is
         ...
         begin
            for V_Pred of Predecessors (Graph, V) loop
               if not Targets.Contains (V_Pred) and then Pred (V_Pred) then
      ...
      procedure Cache_Graph_If_Needed (G : Graph_Id) is
         function Is_Body (N : Node_Id) return Boolean
         is (Nkind (N) in N_Entity_Body);
         function Find_Enclosing_Body is new
      ...
      procedure Cache_Graph_Of_Body (Enclosing_Body : Node_Id) is
         ...
         procedure Cache_For_Statement
           (Stmt : Node_Id; Depth : Natural; Exit_Vertex : in out Vertex);
         ...
         begin
            All_Graphs.Reference (All_Graphs.Find (Tgt))
              .Ctrl_Predecessors
              .Include (Src);
         end Add_Edge;
         ...
         procedure Insert_Neighborhood (V : Vertex; Depth : Natural) is
         begin
            All_Graphs.Insert
              (V,
               Neighborhood'
      ...
      function Predecessors (G : Graph_Id; V : Vertex) return Vertex_Sets.Set
      is
         function Depth (Neighbor : Vertex) return Natural
         is (All_Graphs.Constant_Reference (Neighbor).Depth_In_Enclosing);
   ...
   is
      ...
      is
         ...
         procedure Check_Param (Formal : Formal_Kind_Id; Actual : N_Subexpr_Id)
         is
         begin
   ...
   is
      function Is_Qualified_Expression (N : Node_Id) return Boolean
      is (Nkind (N) = N_Qualified_Expression);
   begin
   ...
   is (Raise_To_Handlers_Map (Call_Or_Stmt));
   ...
   is (if Instantiation_Location (Sloc (N)) = No_Location
   ...
   function Shape_Of_Node (Node : Node_Id) return String is
      ...
      function Label_Append (Buf : Unbounded_String) return Unbounded_String
      is (if Buf = Null_Unbounded_String
            ...
            =>
               declare
                  ...
                  Name_Str : constant String :=
                    (if I_Name /= No_Name and then I_Name /= Error_Name
                     then Get_Name_String (I_Name) & "_"
                     else "");
               begin
            ...
            when N_Assignment_Statement =>
               ...
               begin
                  ...
                  if Present (Obj) then
                     ...
                     if Obj_Name /= No_Name and then Obj_Name /= Error_Name
                     then
               ...
               declare
                  Tmp : constant String :=
                    (if Present (Identifier (Node_It))
                     then Get_Name_String (Chars (Identifier (Node_It))) & "_"
                     else "");
               begin
            ...
            when N_Raise_Statement =>
               Buf :=
                 ...
                 & (if Present (Name (Node_It))
                    then "_" & Get_Name_String (Chars (Name (Node_It)))
                    else "")
                 & Label_Append (Buf);
   ...
   function String_Of_Node (N : N_Subexpr_Id) return String is
      ...
      function Ident_Image
        (Expr : Node_Id; Orig_Expr : Node_Id; Expand_Type : Boolean)
         return String
      with Pre => Present (Expr);

      function Real_Image_10 (U : Ureal) return String
      is (Real_Image (U, 10));

      function String_Image (S : String_Id) return String
      is ('"' & Get_Name_String (String_To_Name (S)) & '"');
   ...
   is
      ...
      Variant     : constant Node_Id := First (Component_Associations (Aggr));
   ...
   begin
      ...
      begin
         loop
            ...
            Root :=
              (if Is_Conditional_Path_Selection (Expr)
            ...
            if not Decreases then
               Decreases :=
                 (for all Path of Terminal_Alternatives (Expr) =>
                    Is_Strict_Subpath (Path));
            end if;
      ...
      begin
         ...
         then
            ...
            --  the subprogram.
               ...
               begin
                  Vertices.Insert (Vertex'(Kind => Body_Exit, Node => Caller));
                  ...
                  loop
                     ...
                     then
                        ...
                        declare
                           ...
                           function Not_Reborrow (V : Vertex) return Boolean
                           is (not Is_Strict_Reborrow_Of (V.Node, Borrower));
                        ...
                        begin
                           ...
                           exit when
                             not Vertices.Contains (Starting_Vertex (Graph));
                        end;
                     end if;
   ...
   is
      ...
      function Not_Reborrow (V : Vertex) return Boolean
      is (not Is_Strict_Reborrow_Of (V.Node, Brower));
   ...
   function Unique_Component
     (E : Record_Field_Kind_Id) return Record_Field_Kind_Id is
   begin
      ...
      loop
         case Nkind (Context) is
            ...
            when N_Object_Declaration =>
               ...
               begin
                  return
                    ((not Nested and then Ekind (Obj) = E_Constant)
               ...
               when N_Object_Declaration =>
                  ...
                  begin
                     if Ekind (Obj) = E_Variable then
                        ...
                        pragma Assert (Ekind (Obj) = E_Constant);
