...
package SPARK_Util is
   ...
   procedure Set_Expr_Has_Side_Effects (N : N_Subexpr_Id);
   --  Register that expression has side effets, due to a sub-expression
   --  function call with side effects.

   function Expr_Has_Side_Effects (N : N_Subexpr_Id) return Boolean;
   ...
   function In_Loop_Entry_Old_Or_At_Attribute (N : Node_Id) return Boolean;
   --  @param N any node
   --  @return True if the node is a Loop_Entry, Old, or At attribute
   ...
   procedure Candidate_For_Loop_Unrolling
     (Loop_Stmt       : N_Loop_Statement_Id;
      ...
      Low_Val         : out Uint;
      High_Val        : out Uint);
   --  @param Output_Info whether a positive message that the loop is
   --         unrolled should be displayed when applicable.
   --  @param Output_Warnings whether the reason for not unrolling should be
   --         displayed when the loop has no loop invariant.
   ...
   function Enclosing_Statement_Or_Decl (N : N_Subexpr_Id) return Node_Id;
   --  Return the statement or declaration enclosing N.
   ...
   --  control, or an expression (occuring at a syntaxic location where it is
   --  legal to perform side effects), iterates over scopes exited upon such
   --  indirect transfer. Defaulted parameters list the potential indirect
   --  transfer of control we may be using to exit the current node.
   ...
   --  * Declare expressions. These can only be exited when starting from an
   --    expression.
   --
   ...
   --  most common cases. For Source a statement/expression potentially causing
   --  indirect transfer of control, call
   --  Iter_Exited_Scopes_With_Specified_Transfer with the transfer of control
   --  potentially generated by the source.

   procedure Register_At_Attribute (Ref : N_Attribute_Reference_Id);
   --  Store Ref in the map from labels entities to references to the At
   --  attribute.

   function Get_At_Attributes_For_Label (E : Entity_Id) return Node_Sets.Set;
   --  Return the set of all references to 'At mentioning label E
   ...
   package Local_CFG is
      ...
      --    a construct. There is also an Entrance vertex for entities with a
      --    body, corresponding to the entry point.
      --    Entrance vertices are only valid for procedural constructs
      --    (statements/bodies), there are never entrance vertices for
      --    expressions regardless of side-effects. Expression that requires
      --    vertices (e.g calls to function with side effects) have Effect
      --    vertices instead.
      --  * Effect vertices: corresponds to the point where the 'effect' of a
      --    construct happens, if it cannot be identified with Entrance.
      --    By 'effect', we refer to writes or transfer of control (including
      --    branching) directly produced by the construct.
      --    It is loosely specified when these are actually generated,
      --    (Is_)Main_Vertex should be used to ensure consistency,
      --    never use 'Effect' directly.
          ...
          (case Vertex.Kind is
             ...
             when Effect                            =>
               ...
               in N_Assignment_Statement
                ...
                | N_Simple_Return_Statement
               and then
                 (Nkind (Vertex.Node) /= N_Function_Call
                  or else Expr_Has_Side_Effects (Vertex.Node)),
             when Completion                        =>
               Nkind (Vertex.Node)
      ...
      with Post => Main_Vertex'Result.Kind in Entrance | Effect;
      --  Get 'main' vertex for a construct/entity.
      --  * If there is a generated 'Effect' vertex, return that
      --  * Otherwise, return the Entrance vertex.
      --  Main_Vertex is intended to represents the point where the main effect
      --  of a construct happens (e.g actual updates of an assignment/calls,
      --  actual transfer-of-control for a jump, actual branch selection for
      --  a case...). As a rule of thumb, that makes it the most appropriate
      --  vertex to represent the construct.

      function Is_Main_Vertex (V : Vertex) return Boolean
      with Post => Is_Main_Vertex'Result = (V = Main_Vertex (V.Node));
      --  Test if V is Main_Vertex for its construct. Assumes V is
      --  a vertex that actually exists in some graph.
