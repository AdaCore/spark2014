...
Inductive well_typed_expr: symtb -> expr -> typ -> Prop :=
    ...
    | WT_Evar: forall ast_id x tb m t,
...
Inductive well_typed_stmt: symtb -> stmt  -> Prop :=
    | WT_Sassign: forall ast_id tb x e m t,
        lookup x tb = Some (m, t) ->
...
Inductive well_defined_expr: mode_map -> expr -> Prop :=
    | WD_Econst_Int: forall ast_id m n,
        well_defined_expr m (Econst ast_id (Ointconst n))
    | WD_Econst_Bool: forall ast_id m b,
        well_defined_expr m (Econst ast_id (Oboolconst b))
    | WD_Evar: forall ast_id x m,
        fetch2 x m = Some (Init, In) \/ fetch2 x m = Some (Init, InOut) ->
        well_defined_expr m (Evar ast_id x)
    | WD_Ebinop: forall ast_id m op e1 e2,
        well_defined_expr m e1 ->
        well_defined_expr m e2 ->
        well_defined_expr m (Ebinop ast_id op e1 e2)
    | WD_Eunop: forall ast_id m op e,
        well_defined_expr m e ->
        well_defined_expr m (Eunop ast_id op e). 
...
Inductive well_defined_stmt: mode_map -> stmt -> mode_map -> Prop :=
    | WD_Sassign: forall ast_id m m' i x e,
        well_defined_expr m e ->
        fetch2 x m = Some (i, Out) \/ fetch2 x m = Some (i, InOut) ->
        update2 m x (Init, InOut) = Some m' ->
        well_defined_stmt m (Sassign ast_id x e) m'
    | WD_Sseq: forall ast_id c1 c2 m m' m'',
        well_defined_stmt m c1 m' ->
        well_defined_stmt m' c2 m'' ->
        well_defined_stmt m (Sseq ast_id c1 c2) m''
    | WD_Sifthen: forall ast_id m m' b c,
        well_defined_expr m b ->
        well_defined_stmt m c m' ->
        well_defined_stmt m (Sifthen ast_id b c) m
    | WD_Swhile: forall ast_id m m' b c,
        well_defined_expr m b ->
        well_defined_stmt m c m' ->
        well_defined_stmt m (Swhile ast_id b c) m.
...
Lemma mode_map_exists: forall tb s,
...
Lemma ast_ids_lt_add: forall ast_id n cks flag,
    ast_id < n ->
    ast_ids_lt cks n ->
    ast_ids_lt ((ast_id, flag) :: cks) n.
Proof.
    intros.
    constructor; auto.
Qed.
...
Inductive check_generator_expr: check_points -> expr -> check_points -> Prop :=
    | CG_Econst: forall ast_id n ls,
        check_generator_expr ls (Econst ast_id n) ls
    | CG_Evar: forall ast_id x ls,
        check_generator_expr ls (Evar ast_id x) ls
    | CG_Ebinop: forall ls e1 ls1 e2 ls2 ast_id op flag,
        check_flag (Ebinop ast_id op e1 e2) (Some flag) -> 
        check_generator_expr ((ast_id, flag) :: ls) e1 ls1 ->
        check_generator_expr ls1 e2 ls2 ->
        check_generator_expr ls (Ebinop ast_id op e1 e2) ls2
    | CG_Ebinop_None: forall ls e1 ls1 e2 ls2 ast_id op,
        check_flag (Ebinop ast_id op e1 e2) None -> 
        check_generator_expr ls e1 ls1 ->
        check_generator_expr ls1 e2 ls2 ->
        check_generator_expr ls (Ebinop ast_id op e1 e2) ls2
    | CG_Eunop: forall ls e ls1 ast_id op,
        check_generator_expr ls e ls1 ->
        check_generator_expr ls (Eunop ast_id op e) ls1.
...
Inductive eval_expr_with_checks (cps: check_points): stack -> expr -> return_val -> Prop :=
    | eval_Const: forall cst v s ast_id,
        eval_constant cst = v ->
        eval_expr_with_checks cps s (Econst ast_id cst) (ValNormal v)
    ...
    | eval_Binop4: forall s e1 v1 e2 v2 ck ast_id op v,
        ...
        eval_expr_with_checks cps s (Ebinop ast_id op e1 e2) (ValNormal v)
    | eval_Binop5: forall s e1 v1 e2 v2 ast_id op v,
        eval_expr_with_checks cps s e1 (ValNormal v1) ->
        eval_expr_with_checks cps s e2 (ValNormal v2) ->
        fetch_ck ast_id cps = None ->
        eval_binexpr op v1 v2 v ->
        eval_expr_with_checks cps s (Ebinop ast_id op e1 e2) (ValNormal v)
    ...
    | eval_Unop2: forall s e v ast_id op v1,
        ...
        eval_expr_with_checks cps s (Eunop ast_id op e) (ValNormal v1).
...
Proof.
  ...
  - specialize (ast_ids_lt_trans _ _ _ h3 H1); intros hz1.
    specialize (ast_ids_lt_add _ _ _ flag H1 hz1); intros hz2.
