...
(**** begin ***)
Lemma optimize_expression_checks_subset: forall st e v' e',
  optimize_expression_x st e (v', e') ->
  subset_of (exp_check_flags e') (exp_check_flags e) = true.
Proof.
  admit.
Qed.

Lemma subset_preserve_notin_property: forall cks cks' ck,
  subset_of cks cks' = true ->
  ~(List.In ck cks') ->
  ~(List.In ck cks).
Proof.
  admit.
...
Lemma subset_subset: forall cks1 ck cks2 cks1' cks2',
  subset_of (cks1 ++ ck :: cks2) (cks1' ++ ck :: cks2') = true ->
  subset_of (cks1 ++ cks2) (cks1' ++ cks2') = true.
Proof.
  admit.
Qed.
(**** end ***)
...
Lemma optimize_expression_range_check_notin: forall e cks1 cks2 st v' e',
  exp_check_flags e = cks1 ++ Do_Range_Check :: cks2 ->
...
Proof.
  intros.
  specialize (optimize_expression_reserve_range_check _ _ _ _ _ _ H H1); intro HZ; smack.
  exists x x0; split; auto.
  specialize (optimize_expression_checks_subset _ _ _ _ H1); intros HZ1.
  rewrite H, H2 in HZ1.
  specialize (subset_subset _ _ _ _ _ HZ1); intros HZ2.
  specialize (subset_preserve_notin_property _ _ _ HZ2 H0); auto.  
Qed.
...
- (* indexed component *)
  ...
  (let HZ := fresh "HZ" in intro HZ);
  specialize (optimize_expr_to_same_checkflags _ _ _ _ _ _ _ H12 H19 HZ0);
  (let HZ := fresh "HZ" in intro HZ);
  match goal with
  | [H1: forall (e' : expression_x) (st : symboltable_x) (s : STACK.stack) (checkflags : check_flags)
                (v : Return value) (v' : valueO),
  ...
  * 
  ...
  specialize (optimize_expression_range_check_notin _ _ _ _ _ _ H5 HZ H12);
  (let HZ := fresh "HZ" in intros HZ; destruct HZ); destruct H2; destruct H2;

  [ apply Eval_E_Indexed_Component_E_RTE_X with (cks1:=x) (cks2:=x0); auto |
    apply Eval_E_Indexed_Component_Range_RTE_X with (cks1:=x) (cks2:=x0) (i:=i0)
                                                    (t:=t0) (l:=l0) (u:=u0); auto |
    apply Eval_E_Indexed_Component_Range_X with (cks1:=x) (cks2:=x0) (i:=i0) (t:=t0)
                                                (l:=l0) (u:=u0) (a:=a1); auto
  ];
  specialize (remove_a_unique_check _ _ _ _ H2 H3); 
  (let HZ := fresh "HZ" in intros HZ);
  specialize (optimize_expr_to_same_checkflags _ _ _ _ _ _ _ H12 HZ1 HZ0); 
  (let HZ := fresh "HZ" in intros HZ);
  match goal with
  | [H1: forall (e' : expression_x) (st : symboltable_x) (s : STACK.stack) (checkflags : check_flags)
                (v : Return value) (v' : valueO),
