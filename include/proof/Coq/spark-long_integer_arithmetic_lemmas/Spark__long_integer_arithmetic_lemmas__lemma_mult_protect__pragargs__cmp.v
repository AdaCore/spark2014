...
Axiom mod_def : forall (x:Z) (y:Z),
                 ((0%Z < y)%Z ->
                  ((mod1 x y) = (int.EuclideanDivision.mod1 x y)))
                 /\ ((~ (0%Z < y)%Z) ->
...
Parameter to_rep1: nat -> Z.

Parameter of_rep1: Z -> nat.

Parameter user_eq1: nat -> nat -> bool.

Parameter dummy1: nat.

Axiom inversion_axiom1 : forall (x:nat), ((of_rep1 (to_rep1 x)) = x).

Axiom range_axiom1 : forall (x:nat), (in_range1 (to_rep1 x)).
...
(* Why3 assumption *)
Definition nat__ref___projection (a:nat__ref): nat := (nat__content a).

(* Why3 assumption *)
Definition dynamic_invariant1 (temp___expr_150:Z) (temp___is_init_147:bool)
  (temp___do_constant_148:bool) (temp___do_toplevel_149:bool): Prop :=
  ((temp___is_init_147 = true) \/ (0%Z <= 9223372036854775807%Z)%Z) ->
  (in_range1 temp___expr_150).
...
Parameter upper_bound: Z.

Parameter attr__ATTRIBUTE_ADDRESS2: Z.

(* Why3 goal *)
Theorem WP_parameter_def : ((in_range arg1)
                            /\ ((in_range1 arg2)
                                /\ ((in_range1 upper_bound)
                                    /\ ((arg2 = 0%Z)
                                        \/ (arg1 <= (ZArith.BinInt.Z.quot upper_bound arg2))%Z)))) ->
                           (*      Post => Arg1 * Arg2 <= Upper_Bound;                                                                                     *)
                           (*              ^ spark-arithmetic_lemmas.ads:61:14:instantiated:spark-long_integer_arithmetic_lemmas.ads:31:1:VC_POSTCONDITION *)
                           ((arg1 * arg2)%Z <= upper_bound)%Z.
(* Why3 intros (h1,(h2,(h3,h4))). *)
intros (h1,(h2,(h3,h4))).
Open Scope Z_scope.
unfold in_range in h1.
unfold in_range1 in h2.
unfold in_range1 in h3.

(* case arg2 is zero *)
destruct (Z.eq_dec arg2 0).
rewrite e.
auto with zarith.
...
destruct h2.
destruct h3.
rewrite Zquot.Zquot_Zdiv_pos in H by auto.
pose (Int.CompatOrderMult arg1 (upper_bound / arg2) arg2 H H0).
apply Int.Trans with (y := (upper_bound / arg2) * arg2); auto.
assert (0 < arg2) as Pos_Arg2 by auto with zarith.
pose (Z.mul_div_le upper_bound arg2 Pos_Arg2).
replace (arg2 * (upper_bound / arg2)) with ((upper_bound / arg2) * arg2) in l0 by auto with zarith.
trivial.
