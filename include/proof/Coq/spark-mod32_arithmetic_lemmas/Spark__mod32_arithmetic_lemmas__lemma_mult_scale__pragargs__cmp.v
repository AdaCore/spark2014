...
Theorem WP_parameter_def : ((in_range scale_denom)
                            /\ ((ule scale_num scale_denom)
                                /\ (((scale_num = (of_int 0%Z)) \/ (ule val__
                                     (udiv (of_int 4294967295%Z) scale_num)))
                                    /\ (res = (udiv (mul val__ scale_num)
                                                scale_denom))))) ->
                           (*      Post => Res <= Val;                                                                                                 *)
                           (*              ^ spark-mod_arithmetic_lemmas.ads:21:14:instantiated:spark-mod32_arithmetic_lemmas.ads:4:1:VC_POSTCONDITION *)
                           (ule res val__).
...
pose (Int.CompatOrderMult (to_uint val__) (4294967295 / to_uint scale_num) (to_uint scale_num) H1 Scale_Num_Nat).
replace (4294967295 / to_uint scale_num * to_uint scale_num)%Z with (to_uint scale_num * (4294967295 / to_uint scale_num))%Z in l1 by auto with zarith.
assert (0 < to_uint scale_num)%Z as Scale_Num_Pos by auto with zarith.
pose (Z.mul_div_le 4294967295%Z (to_uint scale_num) Scale_Num_Pos).
auto with zarith.
contradict n0.
apply Z_mod_lt.
auto with zarith.
