...
Axiom Nth_ones : forall (n:Z),
                  ((0%Z <= n)%Z /\ (n < 32%Z)%Z) -> ((nth ones n) = true).
...
Axiom to_uint_add_bounded : forall (v1:t) (v2:t),
                             (((to_uint v1) + (to_uint v2))%Z < 4294967296%Z)%Z ->
...
Definition in_range (x:t): Prop :=
  (ule (of_int 1%Z) x) /\ (ule x (of_int 4294967295%Z)).

(* Why3 assumption *)
Definition in_range_int (x:Z): Prop := (1%Z <= x)%Z /\ (x <= 4294967295%Z)%Z.
...
Definition dynamic_invariant1 (temp___expr_222:t) (temp___is_init_219:bool)
  ...
  ((temp___is_init_219 = true) \/ (ule (of_int 1%Z) (of_int 4294967295%Z))) ->
  (in_range temp___expr_222).
...
(* Why3 goal *)
Theorem WP_parameter_def : ((in_range factor)
                            /\ ((ult val1 val2) /\ (ule val2
                                (udiv (of_int 4294967295%Z) factor)))) ->
                           (*      Post => Val1 * Factor < Val2 * Factor;                                                                               *)
                           (*              ^ spark-mod_arithmetic_lemmas.ads:49:14:instantiated:spark-mod32_arithmetic_lemmas.ads:33:1:VC_POSTCONDITION *)
...
destruct h1.
unfold ule in H, H0, h3.
unfold ult in h2.
rewrite to_uint_udiv in h3.
assert (0 <= to_uint val1 /\ to_uint val1 < 4294967296) as Val1_Bounds.
apply to_uint_bounds.
destruct Val1_Bounds.
assert (0 <= to_uint val2 /\ to_uint val2 < 4294967296) as Val2_Bounds.
apply to_uint_bounds.
destruct Val2_Bounds.
unfold EuclideanDivision.div in h3.
rewrite to_uint_of_int in H, H0, h3 by auto with zarith.
case Z_le_dec in h3.
...
SearchPattern (0 < ?c -> ?a < ?b -> ?a * ?c < ?b * ?c).
assert (0 < to_uint factor) as Factor_Pos by auto with zarith.
apply (Zmult_lt_compat_r _ _ _ Factor_Pos h2).

(* no overflow in val2 * factor *)
split.
auto with zarith.
assert (0 <= to_uint factor) as Factor_Nat by auto with zarith.
pose (Int.CompatOrderMult (to_uint val2) (4294967295 / to_uint factor) (to_uint factor) h3 Factor_Nat).
replace (4294967295 / to_uint factor * to_uint factor) with (to_uint factor * (4294967295 / to_uint factor)) in l0 by auto with zarith.
assert (0 < to_uint factor) as Factor_Pos by auto with zarith.
pose (Z.mul_div_le 4294967295 (to_uint factor) Factor_Pos).
auto with zarith.

(* no overflow in val1 * factor *)
split.
auto with zarith.
assert (to_uint val1 <= 4294967295 / to_uint factor) as Val1_Bound by auto with zarith.
assert (0 <= to_uint factor) as Factor_Nat by auto with zarith.
pose (Int.CompatOrderMult (to_uint val1) (4294967295 / to_uint factor) (to_uint factor) Val1_Bound Factor_Nat).
replace (4294967295 / to_uint factor * to_uint factor) with (to_uint factor * (4294967295 / to_uint factor)) in l0 by auto with zarith.
assert (0 < to_uint factor) as Factor_Pos by auto with zarith.
pose (Z.mul_div_le 4294967295 (to_uint factor) Factor_Pos).
auto with zarith.
