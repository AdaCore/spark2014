...
Theorem WP_parameter_def : ((in_range factor)
                            /\ ((ult val1 val2) /\ (ule val2
                                (udiv (of_int 18446744073709551615%Z) factor)))) ->
                           (*      Post => Val1 * Factor < Val2 * Factor;                                                                               *)
                           (*              ^ spark-mod_arithmetic_lemmas.ads:49:14:instantiated:spark-mod64_arithmetic_lemmas.ads:33:1:VC_POSTCONDITION *)
...
assert (0 <= to_uint val1 /\ to_uint val1 < 18446744073709551616) as Val1_Bounds.
apply to_uint_bounds.
destruct Val1_Bounds.
assert (0 <= to_uint val2 /\ to_uint val2 < 18446744073709551616) as Val2_Bounds.
apply to_uint_bounds.
...
pose (Int.CompatOrderMult (to_uint val2) (18446744073709551615 / to_uint factor) (to_uint factor) h3 Factor_Nat).
replace (18446744073709551615 / to_uint factor * to_uint factor) with (to_uint factor * (18446744073709551615 / to_uint factor)) in l0 by auto with zarith.
assert (0 < to_uint factor) as Factor_Pos by auto with zarith.
pose (Z.mul_div_le 18446744073709551615 (to_uint factor) Factor_Pos).
auto with zarith.

(* no overflow in val1 * factor *)
split.
auto with zarith.
assert (to_uint val1 <= 18446744073709551615 / to_uint factor) as Val1_Bound by auto with zarith.
assert (0 <= to_uint factor) as Factor_Nat by auto with zarith.
pose (Int.CompatOrderMult (to_uint val1) (18446744073709551615 / to_uint factor) (to_uint factor) Val1_Bound Factor_Nat).
replace (18446744073709551615 / to_uint factor * to_uint factor) with (to_uint factor * (18446744073709551615 / to_uint factor)) in l0 by auto with zarith.
assert (0 < to_uint factor) as Factor_Pos by auto with zarith.
pose (Z.mul_div_le 18446744073709551615 (to_uint factor) Factor_Pos).
auto with zarith.
