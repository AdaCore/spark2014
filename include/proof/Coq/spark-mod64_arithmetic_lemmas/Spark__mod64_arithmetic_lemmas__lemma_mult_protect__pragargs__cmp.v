...
(* Why3 goal *)
Theorem WP_parameter_def : ((arg2 = (of_int 0%Z)) \/ (ule arg1
                            (udiv upper_bound arg2))) ->
                           (*      Post => Arg1 * Arg2 <= Upper_Bound;                                                                                  *)
                           (*              ^ spark-mod_arithmetic_lemmas.ads:58:14:instantiated:spark-mod64_arithmetic_lemmas.ads:33:1:VC_POSTCONDITION *)
                           (ule (mul arg1 arg2) upper_bound).
intros h1.
Open Scope Z_scope.

(* rewrite hypotheses *)
pose (to_uint_bounds arg1) as Arg1_Bounds.
pose (to_uint_bounds arg2) as Arg2_Bounds.
pose (to_uint_bounds upper_bound) as Upper_Bounds.
destruct Arg1_Bounds, Arg2_Bounds.
unfold ule in h1.
...
(* split on arg2 *)
case (Z_eq_dec (to_uint arg2) 0); intros.

* assert (0 <= to_uint arg1 * to_uint arg2 < 18446744073709551616) as Zero_Bound.
  rewrite e, Z.mul_0_r.
  auto with zarith.
  pose (BV_Gen.mod1_out (to_uint arg1 * to_uint arg2) 18446744073709551616 Zero_Bound).
  rewrite e0, e, Z.mul_0_r.
  auto with zarith.

* destruct h1.
  rewrite H3 in n.
  rewrite to_uint_of_int in n by auto with zarith.
  contradict n.
  trivial.
  ...
  case Z_le_dec in H3; [|contradict n0; apply Z_mod_lt; auto with zarith].
  assert (0 < to_uint arg2) as Arg2_Pos by auto with zarith.
  pose (Z.mul_div_le (to_uint upper_bound) (to_uint arg2) Arg2_Pos).
  apply Int.Trans with (y := to_uint arg2 * (to_uint upper_bound / to_uint arg2)).
  replace (to_uint arg2 * (to_uint upper_bound / to_uint arg2)) with ((to_uint upper_bound / to_uint arg2) * to_uint arg2) by auto with zarith.
  apply (Zmult_le_compat_r _ _ _ H3 H1).
  trivial.
