...
procedure Gnatprove is
   ...
   function CVC4_Resource_Step_Flags return String is
      use Ada.Strings.Unbounded;

      type CVC4_Resource_Kind is (Bitblast,
                                  BV_SAT_Conflict,
                                  CNF,
                                  Decision,
                                  Lemma,
      ...
      is
         S : String := "--" & R'Img & "-step";
      begin
         S := Translate (S, Lower_Case_Map);
         S := Translate (S, To_Mapping (From => "_",
                                        To   => "-"));
         return S;
      end Flag_Name;
   ...
   begin
      for R in CVC4_Resource_Kind loop
         if CVC4_Resource (R) /= 1 then
            ...
            Append (S, Flag_Name (R) & "=" & Image (CVC4_Resource (R), 1));
         end if;
      end loop;
      return To_String (S);
   end CVC4_Resource_Step_Flags;
   ...
   procedure Generate_Why3_Conf_File
     (Gnatprove_Subdir : String)
   is
      type CVC4_Mode is (Normal, Alternative, Counter_Examples);
      ...
      procedure Generate_CVC4_Section (Mode : CVC4_Mode);
      ...
      procedure Generate_CVC4_Section (Mode : CVC4_Mode) is
         ...
         --    Seems to have a small benefit in experiments.
         ...
         Command : constant String :=
           CVC4_Binary & " " &
           Common_CVC4_Options &
           --  Disable CBQI normally, it causes more issues than it helps
           --  with out kind of VCs.
           (case Mode is
            when Normal           |
                 Counter_Examples => " --no-cbqi --no-cond-rewrite-quant",
            when Alternative      => "") &
           --  Special flags for Counter_Examples mode.
           (case Mode is
            when Normal | Alternative => "",
            when Counter_Examples     => " --macros-quant --quiet");

         The_Driver : constant String :=
           (case Mode is
            when Normal | Alternative => "cvc4_gnatprove.drv",
            when Counter_Examples     => "cvc4_gnatprove_ce.drv");
      begin
         Start_Section ("prover");
         Put_Keyval ("command",       Command                  & " %f");
         Put_Keyval ("command_steps", Command & " --rlimit=%S" & " %f");
         Put_Keyval ("driver",
                     Ada.Directories.Compose
                       (File_System.Install.Share_Why3_Drivers, The_Driver));
         case Mode is
            when Normal =>
               Put_Keyval ("name", "CVC4");
               Put_Keyval ("shortcut", "cvc4");
            when Alternative =>
               Put_Keyval ("name", "CVC4_ALT");
               Put_Keyval ("shortcut", "cvc4_alt");
            when Counter_Examples =>
   ...
   begin
      ...
      for Mode in CVC4_Mode loop
         Generate_CVC4_Section (Mode);
      end loop;
