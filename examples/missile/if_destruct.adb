-- MCU interface to the self-destruct, implementation
--
-- $Log: if_destruct.adb,v $
-- Revision 1.1.1.1  2004/01/12 19:29:12  adi
-- Added from tarfile
--
--
-- Revision 1.2  2003/09/10 20:52:43  adi
-- Added get_ibit_state
--
-- Revision 1.1  2003/09/01 19:33:09  adi
-- Initial revision
--
--
with Bc1553, bus;
package body If_Destruct
  --# own State is Last_state, Last_Valid, Ibit_Request;
is
   Last_State   : Stage := Destruct_Cfg.Inactive;
   Last_Valid   : Boolean := False;
   Ibit_Request : IBIT.Phase := IBIT.Off;

   Bus_Id : constant Bc1553.Lru_Name := Bc1553.Destruct;
   ...
   begin
      ...
      if Last_Valid then
         ...
         Action_Stage := Destruct_Cfg.inactive;
      end if;
   end Get_stage;
   ...
   is begin
      ...
      BC1553.Write_Word(Data => Destruct_Cfg.State_To_Code(New_Stage),
                        Idx =>  1,
                        Subaddress_Idx => 1,
                        Dest => Bus_id);
   end Set_stage;
      ...
      if BC1553.Is_Valid(Src => Bus_id,
        ...
        BC1553.Is_Fresh(Src => Bus_id,
         ...
         BC1553.Read_Word(Src => Bus_id,
                          ...
                          Subaddress_Idx => 1,
                          Data => Word);
         last_State := Destruct_Cfg.Code_To_state(Word);
         Last_Valid := True;
         -- 1T word 2 - BIT
         BC1553.Read_Word(Src => Bus_id,
                          Idx => 2,
   ...
   --#         Bus.Outputs from *;
   is begin
      Last_State := Destruct_Cfg.inactive;
      Last_Valid := False;
