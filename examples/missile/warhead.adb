...
--
--
-- Revision 1.2  2003/09/01 18:57:32  adi
-- Corrected bug with stage transition
--
-- Revision 1.1  2003/09/01 18:25:33  adi
-- Initial revision
--
--
with Clock_Utils,SystemTypes;
with Bus,RT1553,IBIT,Bit_Machine;
package body Warhead
  --# own State is
  --#     current_stage,
  --#     stage_timer,
  --#     BIT_State;
is
   -- The current action state of the warhead
   Current_Stage : Stage := Warhead_Cfg.inactive;
   -- What time the warhead state last changed
   stage_timer  : Clock.Millisecond := Clock.Millisecond'First;
   -- The BIT state
   Bit_State  : Bit_Machine.Machine := Bit_Machine.Initial_Machine;

   Bus_Id : constant Rt1553.Lru_Name := Rt1553.Warhead;

   -- maximum times to remain in states
   type Time_Array is array(Stage) of Clock.Millisecond;
   Max_Stage_Time : constant Time_Array
     := Time_Array'(Warhead_Cfg.Inactive  => 0,
                    Warhead_Cfg.Awake     => 3_000,
                    Warhead_Cfg.Armed     => 2_000,
                    Warhead_Cfg.Final     => 1_000,
                    Warhead_Cfg.Detonated => 0);
      ...
      if T_Delta > 0 then
         ...
         if T_Valid and then
           ...
           (T_Now - Stage_Timer) >= T_Delta then
            ...
            Current_Stage := Warhead_Cfg.inactive;
         else
   ...
   begin
      ...
      Rt1553.Write_Word(Data =>
                          Warhead_cfg.State_To_code(Current_Stage),
                        Idx => 1,
                        Subaddress_Idx => 1,
                        Src => Bus_Id);
      ...
      if Rt1553.Is_Valid(Src => Bus_Id,
        ...
        Rt1553.Is_Fresh(Src => Bus_Id,
         ...
         Rt1553.Read_Word(Src => Bus_id,
                          ...
                          Subaddress_Idx => 1,
                          Data => Datum);
         if Warhead_Cfg.Code_To_State(Datum) /=
           Current_Stage then
            New_Stage := Warhead_Cfg.Transition(Old_State => Current_Stage,
                                                New_Code  => Datum);
            Set_Stage(New_Stage);
         end if;
      end if;
