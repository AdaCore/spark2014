...
import GPS, os_utils, os.path, re
...
def compute_trace_filename(msg):
    """compute the trace file name in which the path information is
       stored
    """
    match = Tag_regex.search(msg.get_text())
    if match:
        tag = match.group(1)
    else:
        return None
    ...
    fn = "%(file)s_%(line)d_%(col)d_%(tag)s.trace" % \
         { "file" : os.path.basename(msg.get_file().name()),
           "line" : msg.get_line(),
           "col"  : msg.get_column(),
           "tag"  : tag }
    return os.path.join(gnatprove_dir, fn)

def parse_trace_file(fn):
...
def show_trace(lines):
    ...
    clearbufs.append(buf)
    goto_location(first_sloc)
    overlay = get_overlay(buf, "trace")
    buf.remove_overlay(overlay)
    for sloc in lines:
        start = GPS.EditorLocation(buf, sloc.line(), 1)
        end = start.end_of_line()
        buf.apply_overlay(overlay, start, end)

def try_to_show_path(msg):
    text = msg.get_text()
    if (text.find("info:") != -1):
        return
    ...
    if not (fn and os.path.isfile(fn)):
        return
    lines = parse_trace_file(fn)
    if not lines or lines == []:
        return
    show_trace(lines)

def on_show_path(self):
    on_clear_highlighting(self)
    loc = self.location()
    my_file = loc.file()
    my_line = loc.line()
    my_col = loc.column()
...
class GNATProve_Plugin:
    ...
    def __init__(self):
        ...
        GPS.Contextual(prefix + "/" + prove_subp).create(
            ...
            filter = is_ada_file_context)
        GPS.Contextual (prefix + "/" + show_path).create(
            on_activate = on_show_path,
            filter = is_ada_file_context)
