...
facilitates the use of unit proof in place unit testing; an approach now
formalised in DO-178C and the DO-333 formal methods supplement.
...
In expressing the apects, pragmas, attributes and rules of |SPARK|, 
...
#. Syntax: this section gives the format of the |SPARK| aspects and pragmas.
   The expression defining the aspect and pragamas are specialisations of the
...
and the use of implementation-defined attributes avoided a |SPARK| program
may be compiled and executed by any Ada 2012 implementation. The |SPARK|
tools will still be able to undertake all its static checks and proofs. 
...
language subset which may be analaysed but may make analysis and proof easier, 
...
- |SPARK| shall provide support for constructive, generative and retrospective
   ...
   * |SPARK| shall complete by generation from the body code, where possible, 
     ...
     This may shorten development time should simplify maintenance.
...
- |SPARK| shall support the development, analysis and verification of programs 
  ...
  when retrosepcively analysing pre-existing code it may well not all conform to
  the |SPARK| subset.
...
- The language shall offer an *unambiguous* semantics. In Ada terminology,
  ...
  be eliminated either by exclusion or by adding extra rules to eradicate the 
  potential for the behaviour.  An example is the rule which prohibits 
  an expression evaluation from having side-effects. 
  [This means implementation-defined and partially-specified features may be outside of
...
verification will have shown that the pre-condition of T holds. Hence, testing
...
specifying the behaviour of a program, such as Pre and Post conditions
...
- Legacy code is analysed which has no or incomplete contracts.

Hence, as well as still fully supporting the constructive development, 
|SPARK| is designed to facilitate generative contracts and retrospective analysis.
...
could be overriden. An new aspect is provided, which may be applied to a unit
declaration or a unit body, to indicate when a unit declaration or just its body
is not in SPARK and should not be analysed. If just the body is not in |SPARK| a
...
the given contract is met by the body as it is not analysed. It is a matter of
trust that the contract represents the behavior of the body as seen by the
|SPARK| parts of the program.
...
- A construct appearing in a unit may not be, or depend on features not, in the 
  |SPARK| subset. The constract may generate a warning or an error which may be 
