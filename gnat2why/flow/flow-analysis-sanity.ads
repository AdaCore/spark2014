...
private package Flow.Analysis.Sanity is
   ...
   --  analysis.
   --
   --  In debug mode we emit an error message that analysis is aborted
   --  here.
   ...
   procedure Check_Variable_Free_Expressions (FA   : Flow_Analysis_Graphs;
                                              Sane : out Boolean);
   --  Enforces SRM 4.4(2): certain expressions must be variable-free

   procedure Check_Illegal_Reads (FA   : Flow_Analysis_Graphs;
                                  Sane : out Boolean);
   --  Enforces SRM 7.1.3(14): a volatile out parameter must never be read
   --  from

   procedure Check_Illegal_Writes (FA   : Flow_Analysis_Graphs;
                                   Sane : out Boolean);
   --  Enforces a number of rules concerning illegal updates:
   --
   --     * A package may not update another package's state during
   --       elaboration
   --
