...
--                    end if;
   ...
   begin
      ...
      for Cur in Delayed_Type_Aspects.Iterate loop
         declare
            ...
            Mark_DIC_Procedure : Boolean := False;
         begin
            ...
            --  stored as value in the Delayed_Type_Aspects map.
               ...
               Mark_DIC_Procedure := False;
            end if;

            if Mark_DIC_Procedure then
               declare
               ...
               begin
                  ...
                  Current_Delayed_Aspect_Type := Etype (First_Formal (Subp));

                  Mark_Entity (First_Formal (Subp));
   ...
   procedure Mark_Subprogram_Body (N : Node_Id) is
      ...
      --
      --  This pragma cannot be directly specified for types nor declare
      --  blocks; it must come from the most immediate scope where pragma
      --  SPARK_Mode can be attached. For SPARK_Pragma coming from package
      --  entity (either body or spec) it may be the pragma given for
      --  private/statements section.
   ...
   begin
      ...
      --  general context.
         ...
         --  or while processing predicate function in discovery mode.
