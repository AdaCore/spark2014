...
package body SPARK_Definition is
   ...
   begin
      ...
      --  Mark delayed type aspects and full views of accesses to incomplete
      --  or partial types.

      while not (Delayed_Type_Aspects.Is_Empty
                 and then Access_To_Incomplete_Types.Is_Empty)
      loop

         --  Make local copies of Delayed_Type_Aspects and
         --  Access_To_Incomplete_Types structures in case some delayed aspects
         --  or incomplete accesses are encountered during marking.

         declare
            C_Delayed_Type_Aspects       : Node_Maps.Map;
            C_Access_To_Incomplete_Types : Node_Sets.Set;
         begin
            C_Delayed_Type_Aspects.Move (Delayed_Type_Aspects);
            C_Access_To_Incomplete_Types.Move (Access_To_Incomplete_Types);
            ...
            for Cur in C_Delayed_Type_Aspects.Iterate loop
               declare
               ...
               begin
                  ...
                  if Nkind (Delayed_Mapping) = N_Pragma then
                     ...
                     --  Or if the type entity has been found to be in SPARK.
                     --  In this case (scope not marked SPARK_Mode(On)), the
                     --  type entity was stored as value in the
                  ...
                  if Mark_Delayed_Aspect then
                     declare
                        ...
                        --  Default_Initial_Condition or Type_Invariant are
                        --  stored as keys in the Delayed_Type_Aspects map.
            ...
            --  Ensure that global variables are restored to their initial
            --  values.
            pragma Assert (No (Current_Delayed_Aspect_Type));

            --  Mark full views of incomplete types and make sure that they are
            --  in SPARK (otherwise an error is raised). Also populate the
            --  Incomplete_Views map.

            for E of C_Access_To_Incomplete_Types loop
