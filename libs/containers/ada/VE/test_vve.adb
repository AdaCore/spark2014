...
procedure test_vve is
   ...
   procedure Test_Element (Container :  Vector; Position :  Cursor;
...
begin
   ...
   if  Has_Element(L2, Next(L2, First(L1))) then
      Ada.Text_IO.Put_Line("OK");
      Test_Element(L2, Next(L2, First(L1)),2,"Has_Element of inserted Element after Insertion");
   else
   ...
   -- &
   L4 := L1 & L2;
   if Element(L4,1) /= 1 or Element(L4,5) /= 3 then
      Ada.Text_IO.Put_Line("& => KO ???");
   else
      Ada.Text_IO.Put_Line("OK");
   end if;
   ...
   if  Find(L3,3, No_Element) /=  Next(L3, First(L3)) or Find(L3,3, No_Element) /= Last(L3) then
      Ada.Text_IO.Put_Line("Find of Left in range => KO ???");
   else
      Ada.Text_IO.Put_Line("OK");
   end if;
   if  Find(L3,1, No_Element) /=  First(L3) or Find(L3,1, No_Element) /= Previous(L3,Last(L3)) then
      Ada.Text_IO.Put_Line("Find of Left in range => KO ???");
   else
      Ada.Text_IO.Put_Line("OK");
   end if;
   ...
   if  Has_Element(L3, Last(L1)) then
   ...
   declare
      E : Cursor;
   begin
      E := Find(L4,3, Previous(L1,Last(L1)));
   ...
   -- &
   L4 := L1 & L3;
   if Element(L4,1) /= 1 or Element(L4,5) /= 4 then
      Ada.Text_IO.Put_Line("& => KO ???");
   else
      Ada.Text_IO.Put_Line("OK");
   end if;
