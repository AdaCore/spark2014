...
class SpecMap:
    ...
    def __init__(self, map):
        ...
        """
        self.map = { key : MergeSpec(map[key], MergeStatusFactory(),
                                     name=key)
                     for key in map }
...
def unit_testing():
    ...
    spec_map = SpecMap({"PROVED"           : "some OK and no   KO",
                        "PARTIALLY PROVED" : "some OK and some KO",
                        "NOT PROVED"       : "no   OK and some KO",
                        "NO VCS"           : "no   OK and no   KO"})
    model = {}
    assert(spec_map.check(model) == {"NO VCS"})
    ...
    model["OK"] = {m1}
    assert(spec_map.check(model) == {"PROVED"})
    ...
    model["KO"] = {m2}
    assert(spec_map.check(model) == {"PARTIALLY PROVED"})
    assert(spec_map.proof(model, True)  == {"PARTIALLY PROVED" : {m1, m2}})
