...
def gprbuild(
    ...
    """Call gprbuild -q **opt. Sort the output if sort_lines is True.

    For SPARKlib projects, the SPARKlib location is added with "-aP".
    """
    if opt is None:
        opt = []
...
def create_sparklib(cwd=None, logger=None):
...
def gnatprove(
    ...
    if sparklib:
        create_sparklib(cwd=cwd, logger=logger)
