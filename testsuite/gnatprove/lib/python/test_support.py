...
def resolve_sparklib_location(path=None):
    ...
    def installed_location(project_dir, root_dir):
        if not os.path.isfile(os.path.join(project_dir, "sparklib_internal.gpr")):
            raise RuntimeError("SPARKlib tests require an installed-tree layout")
        if not os.path.isdir(os.path.join(root_dir, "include", "spark")):
            raise RuntimeError("SPARKlib tests require an installed-tree layout")
        return (project_dir, root_dir)
    ...
    if path is None:
        ...
        return installed_location(os.path.join(root_dir, "lib", "gnat"), root_dir)
    ...
    if (candidate / "sparklib_internal.gpr").is_file():
        if (candidate / "src").is_dir():
            raise RuntimeError("SPARKlib tests require an installed-tree layout")
        return installed_location(str(candidate), str(candidate.parent.parent))

    installed_project_dir = candidate / "lib" / "gnat"
    if (installed_project_dir / "sparklib_internal.gpr").is_file():
        return installed_location(str(installed_project_dir), str(candidate))
...
def create_sparklib(sparklib_bodymode=False, cwd=None, logger=None):
    ...
    If sparklib_bodymode is true, change GPR_PROJECT_PATH to point to the
    shared body-mode SPARKlib generated by the testsuite driver.
    ...
    # resolve_sparklib_location rejects source-tree layouts, so the
    # sparklib_common.gpr default SPARKLIB_INSTALLED=True is the expected mode.
