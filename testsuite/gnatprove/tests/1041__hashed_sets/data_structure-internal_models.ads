...
private package Data_Structure.Internal_Models
...
is
   ...
   is (Length (S1) = Length (S2)
       ...
       and then
         (for all P in Interval'(1, Last (S1)) =>
            (if (P < P1 and P < P2) or (P > P1 and P > P2)
             then Get (S2, P) = Get (S1, P)))
       and then
         (for all P in Interval'(1, Last (S2)) =>
   ...
   --  Returns the position of index I in sequence S, or 0 if not found
   ...
   with
     Post =>
       ...
       and then
         (for all K in Interval'(Find'Result + 1, Last (S)) =>
            Get (S, K) /= I);

   --  Returns the position in S of the first index whose mapped value in V is equivalent to E
   function Find
...
end Data_Structure.Internal_Models;
