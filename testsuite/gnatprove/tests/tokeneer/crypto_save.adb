...
is
   ...
   --
   ------------------------------------------------------------------
   procedure Create(FileHandle :    out Ada.Text_IO.File_Type;
                    Created    :    out Boolean)
   is
   begin
      Ada.Text_IO.Create(File => FileHandle,
                         Name => "./System/keystore",
                         Form => "");
      Created := Ada.Text_IO.Is_Open (FileHandle);
   end Create;

   procedure OpenForAppending(FileHandle :    out Ada.Text_IO.File_Type;
                              Open       :    out Boolean)
   is
   begin
      Ada.Text_IO.Open(File => FileHandle,
                       Mode => Ada.Text_IO.Append_File,
                       Name => "./System/keystore",
                       Form => "");
      Open := Ada.Text_IO.Is_Open (FileHandle);
   end OpenForAppending;

   procedure OpenForReading(FileHandle :    out Ada.Text_IO.File_Type;
                            Open       :    out Boolean)
   is
   begin
      Ada.Text_IO.Open(File => FileHandle,
   ...
   procedure Close(FileHandle : in out Ada.Text_IO.File_Type;
                   Closed     :    out Boolean)
   is
   begin
      Ada.Text_IO.Close(File => FileHandle);
      Closed := not Ada.Text_IO.Is_Open (FileHandle);
   end Close;
   ...
   is
      ...
      is
         ...
         begin
            ...
            while not Ada.Text_IO.End_Of_File(File => Keys) loop
               Ada.Text_IO.Get(File => Keys,
                               Item => IDTitle);
            ...
            if not Ada.Text_IO.End_Of_File(File => Keys) then
         ...
         begin
            ...
            if Index = IsPublicIndexT'Last and then
               ...
               Ada.Text_IO.Get_Char(File  => Keys,
                                 Item  => TheChar);
               ...
               Ada.Text_IO.Skip_Line(File    => Keys,
                                  Spacing => 1);
      ...
      begin -- UpdateLocalStore
         loop
            ReadOwnerID;
            exit when Ada.Text_IO.End_Of_File(File => Keys);
   ...
   is
      ...
      begin
         ...
         Ada.Text_IO.Put_String(File => Keys,
                             Item => OwnerIDTitle,
                             Stop => 0);
         ...
         Ada.Text_IO.New_Line(File => Keys,
                           Spacing => 1);
   ...
   is
      Status : Ada.Text_IO.File_Status;
      Keys   : Ada.Text_IO.File_Type;
   begin
      Ada.Text_IO.Open(File => Keys,
                    Mode_Of_File => Ada.Text_IO.Out_File,
                    Name_Of_File => "./System/keystore",
                    Form_Of_File => "",
                    Status       => Status);
      Ada.Text_IO.Close(File   => Keys,
                     Status => Status);
   end ClearStore;
