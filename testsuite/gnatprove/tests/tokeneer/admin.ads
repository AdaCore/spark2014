...
is pragma SPARK_Mode (On);
   ...
   pragma Postcondition (not IsPresent(TheAdmin) and then
                         not IsDoingOp(TheAdmin));
   ...
   pragma Postcondition
     ((Role = PrivTypes.Guard) = (RolePresent(TheAdmin) = PrivTypes.Guard)
      and then
      not IsDoingOp(TheAdmin) and then
      IsPresent(TheAdmin));
   ...
   pragma Precondition (IsPresent(TheAdmin));
   pragma Postcondition
     ((Op = OverrideLock) = (RolePresent(TheAdmin) = PrivTypes.Guard) and then
      TheCurrentOp(TheAdmin) = Op and then
      IsDoingOp(TheAdmin) and then
      RolePresent(TheAdmin) = RolePresent(TheAdmin'Old) and then
      IsPresent(TheAdmin));
   ...
   --#      IsPresent(TheAdmin);
   pragma Precondition (IsPresent(TheAdmin));
   pragma Postcondition (not IsDoingOp(TheAdmin) and then
