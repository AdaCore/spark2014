...
is pragma SPARK_Mode (On);
      ...
      is
         ...
         begin
            for I in LogFileIndexT loop
               ...
               pragma Assert (I in LogFileIndexT);
        ...
        and UsedLogFiles.Length = LogFileCountT'Last then
         ...
         pragma Assert
           (NumberLogEntries =
              ...
              LogFileEntries(CurrentLogFile));
         pragma Assert (LogFileEntries(CurrentLogFile) = MaxLogFileEntries);
         pragma Assert (UsedLogFiles.Length = LogFileCountT'Last - 1);
         ...
         pragma Assert
           (NumberLogEntries =
              LogEntryCountT(UsedLogFiles.Length -1)*MaxLogFileEntries +
              LogFileEntries(CurrentLogFile));
         pragma Assert (LogFileEntries(CurrentLogFile) = 1);
         pragma Assert (UsedLogFiles.Length = LogFileCountT'Last);
   ...
   begin
      ...
      pragma Assert
        (UsedLogFiles.Length in LogFileCountT and then
         UsedLogFiles.LastI in LogFileIndexT) ;
      if UsedLogFiles.Length = 0 then
