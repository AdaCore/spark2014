...
--       Depends => (till_invariant'Result => (Account, Children));
   ...
   -- for checking that the sum of money received and potential money is constant.


   procedure start_pumping
     with Global => (In_Out => (State, Account, Children)),
     Depends => (State => State, Account =>+ (Children, State), Children =>+ State),
     Pre => is_ready and then get_outstanding < Natural'Last - get_cur_price and then
     get_cur_resevoir > Nat_Type'First,
     Post =>  is_pumping and then get_outstanding = get_outstanding'Old + get_cur_price and then
     get_cur_resevoir = get_cur_resevoir'Old - 1;
   ...
   procedure lift_nozel(pump_index : in Integer)
     with Global => (In_Out => (Children, State)),
     Depends => (Children =>+ (pump_index), State => State),
     Pre => (is_base or else
               is_waiting) and then
     pump_index in Pump_Array_Type'Range,
     Post => is_ready;

   procedure replace_nozel
     with Global => (In_Out => State),
     Depends => (State => State),
     Pre => is_ready,
     Post => is_waiting;
   ...
   procedure Initialize
     with Global => (Output => (State, Account, Children)),
