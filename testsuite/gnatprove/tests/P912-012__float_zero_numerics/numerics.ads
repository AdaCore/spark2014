...
is
   ...
   function "+" (Left : T_Float; Right : T_Float) return T_Float with Inline, Pre => True, Post => "+"'Result = T_Float (T_Base_Float (Left) + T_Base_Float (Right));
   ...
   function "/" (Left : T_Float; Right : T_Float) return T_Float with Inline, Pre => Right /= 0.0, Post => "/"'Result = T_Float (T_Base_Float (Left) / T_Base_Float (Right));
   pragma Annotate (GNATprove, Inline_For_Proof, "/");
