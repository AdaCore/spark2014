...
is
    ...
    with
       ...
       Post => (Values = Values'Old'Update (X => Values'Old (Y),
                                            Y => Values'Old (X)))
    is
    begin
       Values := Values'Update (X => Values (Y),
                                Y => Values (X));
    end Swap;
...
--           if Smallest /= Current then
--              Temp              := Values (Current);
--              Values (Current)  := Values (Smallest);
...
--           end if;
         ...
         pragma Loop_Invariant   -- Simple partition property
          ...
          pragma Loop_Invariant  -- order property
            ((for all J in Values'First .. Current - 1 =>
                Values (J) <= Values (J + 1)));
      end loop;
