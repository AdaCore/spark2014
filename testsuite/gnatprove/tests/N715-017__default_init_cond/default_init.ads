package Default_Init with SPARK_Mode is
   ...
   type Rte is private with
     Default_Initial_Condition => Rte_Ok (Rte);
   type Glob is private with
     Default_Initial_Condition => Glob_Ok (Glob);
   ...
   function Pre_Rte_Ok (X : Rte) return Boolean;
...
private
   ...
   function Pre_Rte_Ok (X : Rte) return Boolean is (N in X'Range);
   function Rte_Ok (X : Rte) return Boolean is (X (N) = 0);
   function Glob_Ok (X : Glob) return Boolean is (X.F = N);
   function Discr_Ok (X : Discr) return Boolean is
     (if X.B then X.F = 0);
   function Mut_Discr_Ok (X : Mut_Discr) return Boolean is
     (if X.B then X.F = 0);
