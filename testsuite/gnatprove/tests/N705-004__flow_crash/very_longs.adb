...
package body Very_Longs is
   ...
   procedure ModAdd_And_Carry(L, R : in Very_Long; Result : out Very_Long; Carry : out Double_Octet)
     with
   ...
   procedure ModSubtract_And_Borrow(L, R : in Very_Long; Result : out Very_Long; Borrow : out Double_Octet)
     with
   ...
   procedure Divide(Dividend : in Very_Long; Divisor : in Very_Long; Quotient : out Very_Long; Remainder : out Very_Long) is
      ...
      begin
         ...
         for I in reverse Number.Long_Digits'Range loop
            ...
            Number.Long_Digits(I) := Number.Long_Digits(I) or Shift_Left(Old_Overflow, 8 - Distance);
            Old_Overflow := New_Overflow;
         end loop;
      end Right_Shift;
      ...
      is
         ...
         R_Hat           : Double_Octet;
      begin
         Temporary_Digit := 256 * Double_Octet(U.Long_Digits(J + N - 1)) + Double_Octet(U.Long_Digits((J + N) - 2));
         Q_Hat := Temporary_Digit / Double_Octet(V.Long_Digits(N - 1));    -- Q_Hat has at most 9 bits.
         R_Hat := Temporary_Digit mod Double_Octet(V.Long_Digits(N - 1));  -- R_Hat has at most 8 bits.
         ...
         --
         if Q_Hat = 256 or Q_Hat * Double_Octet(V.Long_Digits(N - 2)) > 256 * R_Hat + Double_Octet(U.Long_Digits((J + N) - 3)) then
            Q_Hat := Q_Hat - 1;
            R_Hat := R_Hat + Double_Octet(V.Long_Digits(N - 1));
         end if;
         if R_Hat < 256 then
      ...
      begin
         ...
         for Divisor_Index in Digit_Index_Type range 1 .. N loop
            ...
            if Double_Octet(U.Long_Digits(J + Divisor_Index - 1)) >= Temporary_Digit + Current_Borrow then
               Future_Borrow := 0;
            else
               Future_Borrow := 1;
            end if;
         ...
         if Double_Octet(U.Long_Digits(J + N - 1)) >= Carry + Current_Borrow then
            Future_Borrow := 0;
         else
            Future_Borrow := 1;
         end if;
         U.Long_Digits(J + N - 1) := Octet(((Double_Octet(U.Long_Digits(J + N - 1)) - Carry) - Current_Borrow) and 16#00FF#);
         Current_Borrow := Future_Borrow;
      end D4;
      ...
      is
         ...
         Sum   : Double_Octet;
      begin
         Quotient.Long_Digits(J) := Octet(Q_Hat);  -- ??? This is where it really matters that Q_Hat be eight bits!
         if Current_Borrow /= 0 then
            ...
            Carry := 0;
            for Divisor_Index in Digit_Index_Type range 1 .. N loop
               Sum := Double_Octet(U.Long_Digits(J + Divisor_Index - 1)) + Double_Octet(V.Long_Digits(Divisor_Index)) + Carry;
   ...
   begin
      ...
      Mask         := Shift_Left(1, Natural(Bit_Position));
   ...
   begin
      ...
      if Bit_Value = 1 then
         Digit := Digit or Shift_Left(1, Natural(Bit_Position));
      else
         Digit := Digit and not Shift_Left(1, Natural(Bit_Position));
      end if;
