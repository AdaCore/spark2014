...
procedure Normal with SPARK_Mode is
   ...
   procedure Add_Overflow is
      X : constant U := Id (3000000245);
      Y : constant U := Id (2123456789);
      Z : constant U := X + Y; --@OVERFLOW_CHECK:FAIL
   begin
      null;
   end Add_Overflow;
   ...
   function Is_Prime (X : U) return Boolean
     ...
     Post =>
       Is_Prime'Result
         = (for all I in U =>
              (for all J in U => (if I * J = X then I = 1 or J = 1))); --@OVERFLOW_CHECK:FAIL
   ...
   begin
      loop
         ...
         pragma Loop_Invariant
           (for all I in U =>
              (for all J in U => (if I * J = X then I = 1 or I >= Test))); --@OVERFLOW_CHECK:FAIL
