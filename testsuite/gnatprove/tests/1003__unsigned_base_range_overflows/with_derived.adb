...
procedure With_Derived with SPARK_Mode is
   type U_Base is range 0 .. 2 ** 32 - 1 with Unsigned_Base_Range, Size => 32;
   function Id (X : U_Base) return U_Base is (X);
   subtype U_Derived is U_Base range Id (0) .. Id (2 ** 16  - 1);
   type U is new U_Derived;
