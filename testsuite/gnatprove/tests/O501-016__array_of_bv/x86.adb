...
is
   ...
   begin
      ...
      --return (Unsigned16(ReadMem8(addr)) or (Unsigned16(ReadMem8(addr + 1)) * (256)));
   ...
   begin
      ...
      --WriteMem8(addr,     Unsigned8(tmp and 16#00FF#));
      --tmp := tmp/256;

      --WriteMem8(addr + 1, Unsigned8(tmp and 16#00FF#));
   ...
   is
   begin
      -- (Unsigned32(ReadMem16(addr)) or Shift_Left(Unsigned32(ReadMem16(addr + 2)),16))
      pragma Assert ((Unsigned16(ReadMem8(addr)) or
                       Shift_Left(Unsigned16(ReadMem8(addr + 1)), 8)) =
                       ReadMem16(addr));
      pragma Assert ((Shift_Left(Unsigned32(ReadMem8(addr + 2)), 16) or
   ...
   begin
      ...
      pragma Assume (Unsigned16(Memory(addr + 2)) = Unsigned16(Shift_Right(Val and 16#00FF0000#, 16)));
      pragma Assume (ReadMem16(addr) = Unsigned16(Val and 16#0000FFFF#));
      pragma Assert ((Unsigned32(ReadMem16(addr)) or Shift_Left(Unsigned32(ReadMem16(addr + 2)), 16)) = Val);
   end WriteMem32;
   ...
   begin
      ...
      pragma Assume (Result = (Unsigned64(ReadMem32(addr)) or
   ...
   begin
      ...
      pragma Assume (ReadMem64(addr) = Val);
