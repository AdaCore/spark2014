...
package External_Variables_Legal
  with SPARK_Mode
is
   --  N110-011 Restore these test cases when
   --  Volatile types are re-introduced.
   --
   --  Leaving them here uncommented prevents the other
   --  units in this test case being analysed, so they are
   --  commend out for now.
