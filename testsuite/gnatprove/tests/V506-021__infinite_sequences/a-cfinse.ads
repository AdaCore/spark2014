...
package Ada.Containers.Functional_Infinite_Sequences with SPARK_Mode is
   ...
   with
     Global => null,
     Post   => Iter_Has_Element'Result = (Position <= Length (Container));
   pragma Annotate (GNATprove, Inline_For_Proof, Iter_Has_Element);

   function Iter_First
     (Container : Sequence) return Big_Positive
   with
     Global => null,
...
private
   ...
   function Iter_First (Container : Sequence) return Big_Positive is
     (1);
