...
procedure Test_Arith64 with
  SPARK_Mode
is
   subtype Small_Float is Long_Float range -2.0**20 .. 2.0**20;
   subtype Small_Nat_Float is Long_Float range 0.0 .. 2.0**20;
   subtype Small_Pos_Float is Long_Float range 1.0 .. 2.0**20;

   --  Addition

   procedure Add_Is_Monotonic (X1, X2, Y : Small_Float; R1, R2 : out Long_Float) with
     Pre => X1 <= X2,
     Post => R1 <= R2
   is
   begin
   ...
   procedure Sub_Is_Monotonic (X1, X2, Y : Small_Float; R1, R2 : out Long_Float) with
     Pre => X1 <= X2,
     Post => R1 <= R2
   is
   begin
   ...
   procedure Mult_Zero (X, Y : Long_Float; R : out Long_Float) with
     Pre => X = 0.0 or Y = 0.0,
     Post => R = 0.0
   is
   begin
   ...
   procedure Mult_Is_Monotonic (X1, X2 : Small_Float; Y : Small_Nat_Float; R1, R2 : out Long_Float) with
     Pre => X1 <= X2,
     Post => R1 <= R2
   is
   begin
   ...
   procedure Div_Is_Monotonic (X1, X2 : Small_Float; Y : Small_Pos_Float; R1, R2 : out Long_Float) with
     Pre => X1 <= X2,
     Post => R1 <= R2
   is
   begin
