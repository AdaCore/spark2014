package body Test_Set with SPARK_Mode is
   ...
   begin
      ...
      pragma Assert (not Contains (K, 3));
      Move (L, K);
      pragma Assert (Contains (L, 1));
      pragma Assert (not Contains (L, 3));
      Assign (K, L);
   ...
   begin
      ...
      pragma Assert (for all J in 1 .. 102 => not Contains (L, J));
      for Idx in 1 .. 100 loop
         pragma Loop_Invariant (for all J in Idx .. 102 => not Contains (L, J));
         pragma Loop_Invariant (Length (L) = Count_Type (Idx) - 1);

         Insert (L, Idx);
         pragma Assert (for all J in Idx + 1 .. 102 => not Contains (L, J));
      end loop;

      pragma Assert (not Contains (L, 102));
      pragma Assert (not Contains (L, 101));
      Insert (L, 101);
