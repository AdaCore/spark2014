package Test is
   ...
   generic
      ...
      with function Sum (X, Y : Item_T) return Item_T;
   ...
   is
      procedure Fill_Empty_Slots (Inventory : in out Inventory_T; Fill_With : Item_T)
         with Depends => (Inventory =>+ Fill_With),
              Pre     => (Inventory'First < Inventory'Last),
              Post    => (for all I in Inventory'Range => not Is_Empty (Inventory (I)));
