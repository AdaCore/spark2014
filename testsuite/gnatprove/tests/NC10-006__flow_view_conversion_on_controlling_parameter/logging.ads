package Logging is
   ...
   type Log_Type_Public is tagged record
      Tmp : Log_Count;
   end record;

   function Log_Size (Log : Log_Type_Public) return Log_Count;

   procedure Init_Log (Log : out Log_Type_Public) with
     Post'Class => Log.Log_Size = 0;

   procedure Append_To_Log (Log  : in out Log_Type_Public;
                            Incr : in Integer) with
     Pre'Class  => Log.Log_Size < Max_Count,
     Post'Class => Log.Log_Size = Log.Log_Size'Old + 1;
