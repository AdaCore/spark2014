...
package body Conts.Functional.Base with SPARK_Mode => Off is
   ...
   function Delete (C : Container; I : Index_Type) return Container is
      A : constant Element_Array_Access :=
        new Element_Array'(1 .. C.Elements'Length - 1 => <>);
      P : Count_Type := 0;
   begin
   ...
   function Prepend (C : Container; E : Element_Type) return Container is
   begin
      return Container'(Elements =>
                           new Element_Array'
                          (new Element_Type'(E) & C.Elements.all));
   end Prepend;
