...
package Allocator.Pools.Sized_8 with SPARK_Mode is
   ...
   with Ghost, Global => (Memory_8, Memory_16, Memory_32, Memory_64);
   ...
   with
     Global => (Memory_8, Memory_16, Memory_32, Memory_64),
   ...
   with
     Global         => (In_Out => (Memory_8, Memory_16, Memory_32, Memory_64)),
     Modifies       =>
       (Memory_8 when not Is_Null (P) and then not Is_Promoted (P),
...
private
   ...
   with
     Predicate =>
       (if not With_Promotion then Pointer.Kind in None_Bucket | In_8)
       and
   ...
   is (Wrapper_8.Num_Free
       + (if With_Promotion
          then Wrapper_16.Num_Free + Wrapper_32.Num_Free + Wrapper_64.Num_Free
          else 0));
