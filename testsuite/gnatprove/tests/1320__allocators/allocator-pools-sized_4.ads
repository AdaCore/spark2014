...
--  Per-size wrapper for objects whose smallest fitting bucket is 4 bytes.
--  Stores in bucket 4 and, when With_Promotion is True and bucket 4 is full,
--  transparently promotes to buckets 8, 16, 32, 64. Provides the same API as
...
--  instantiation) and must lie in 1 .. 4 for this wrapper.

generic
   type Object_Type is private;
   Object_Bytes : Positive;
   With_Promotion : Boolean := True;
package Allocator.Pools.Sized_4 with SPARK_Mode is

   pragma
     Compile_Time_Error
       (Object_Bytes /= Object_Type'Object_Size / 8,
        "Object_Bytes is inconsistant with Object_Type");
   pragma Compile_Time_Error (Object_Bytes > 4, "Object_Bytes bigger than 4");
   ...
   --  Free cells usable by this wrapper: bucket 4, plus (when promoting) the
   --  larger buckets.
   function Num_Free return Big_Natural
   with Ghost, Global => (Memory_4, Memory_8, Memory_16, Memory_32, Memory_64);
   ...
   with
     Global => (Memory_4, Memory_8, Memory_16, Memory_32, Memory_64),
...
private
   ...
   package Codec_4 is new Codec (Object_Type, Storage_4, 4, Object_Bytes);
   package Codec_8 is new Codec (Object_Type, Storage_8, 8, Object_Bytes);
   ...
   type Bucket_Kind is (None_Bucket, In_4, In_8, In_16, In_32, In_64);
   ...
   with
     Predicate =>
       (if not With_Promotion then Pointer.Kind in None_Bucket | In_4)
       and
         (case Pointer.Kind is
            when None_Bucket => True,
            when In_4        =>
              Wrapper_4.Is_Null (Pointer.P4)
              or else
                Codec_4.From_Raw (Wrapper_4.Deref (Pointer.P4))'Valid_Scalars,
            when In_8        =>
              Wrapper_8.Is_Null (Pointer.P8)
              or else
                Codec_8.From_Raw (Wrapper_8.Deref (Pointer.P8))'Valid_Scalars,
            when In_16       =>
   ...
   is (case P.Pointer.Kind is
         when None_Bucket => True,
         when In_4        => Wrapper_4.Is_Null (P.Pointer.P4),
         when In_8        => Wrapper_8.Is_Null (P.Pointer.P8),
