from test_support import sleep, prove_all, spark_install_path
import json
import os.path
import re
...
""" This tests is a commandline test for the itp server. It launches a server
as a background process, and then pass it request in JSON. The output to be
checked are notifications written in JSON"""
...
def launch_server(limit_line, input_file):
    ...
    cmd = [
        os.path.join(bindir, "gnat_server"),
        "--limit-line",
        limit_line,
        "test__parity",
    ]
    ...
    with open("proc.out", "w") as outfile:
        process = Run(
            cmd, cwd="gnatprove", input=read, output=outfile, bg=True, timeout=8
        )
