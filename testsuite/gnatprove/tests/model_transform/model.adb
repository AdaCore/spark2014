package body Model is
   ...
   begin
	 ...
	 AV(UML_Action_Access(S)) := 
      ...
      for T in Owned_Transition(SM)'Range loop
	 pragma Assert
	 ...
	 Set_From (CFV(UML_Control_Flow_Access(T)),
		   UML_Action_Access(From(Owned_Transition(SM)(T))));
	 Set_To (CFV(UML_Control_Flow_Access(T)),
		 UML_Action_Access(To(Owned_Transition(SM)(T))));
      end loop;
