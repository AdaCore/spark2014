...
generic
   type Num is digits <>;
package SPARK.Text_IO.Float_IO is

   Default_Fore : constant Field := 2;
   Default_Aft  : constant Field := Num'Digits - 1;
   Default_Exp  : constant Field := 3;

   type Float_Result (Status : File_Status := Status_Error) is record
      case Status is
