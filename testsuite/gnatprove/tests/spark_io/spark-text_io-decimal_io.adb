...
package body SPARK.Text_IO.Decimal_IO is
   ...
   begin
      case File.Sort is
         ...
         when Std_In =>
            Ada_Decimal_IO.Get (Ada.Text_IO.Standard_Input, Re, Width);
            Item := (Status => Success, Item => Re);
            File.Status := Success;
         when A_File =>
            Ada_Decimal_IO.Get (File.File, Re, Width);
            Item := (Status => Success, Item => Re);
            File.Status := Success;
      end case;
   exception
   ...
   procedure Get (Item  : out Decimal_Result;
                  Width : in  Field := 0) is
      Re : Num;
   begin
      Ada_Decimal_IO.Get (Ada.Text_IO.Standard_Input, Re, Width);
   ...
   begin
      case File.Sort is
         when Std_In =>
            File.Status := Mode_Error;
         when Std_Out =>
            Ada_Decimal_IO.Put (Ada.Text_IO.Standard_Output,
                                Item, Fore, Aft, Exp);
            File.Status := Success;
         when Std_Error =>
            Ada_Decimal_IO.Put (Ada.Text_IO.Standard_Error,
                                Item, Fore, Aft, Exp);
            File.Status := Success;
         when A_File =>
            Ada_Decimal_IO.Put (File.File,
                                Item, Fore, Aft, Exp);
            File.Status := Success;
      end case;
   exception
   ...
   begin
      Ada_Decimal_IO.Put
        (Ada.Text_IO.Standard_Output, Item, Fore, Aft, Exp);
   ...
   procedure Get (From   : in  String;
      ...
      Re : Num;
   begin
      Ada_Decimal_IO.Get (From, Re, Last);
      Item := (Status => Success, Item => Re);
   exception
      when Ada.Text_IO.Data_Error =>
         Item := (Status => Data_Error);
