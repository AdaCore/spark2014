...
package body SPARK.Text_IO.Fixed_IO is
   ...
   begin
      case File.Sort is
         ...
         when Std_In =>
            Ada_Fixed_IO.Get (Ada.Text_IO.Standard_Input, Re, Width);
            Item := (Status => Success, Item => Re);
            File.Status := Success;
         when A_File =>
            Ada_Fixed_IO.Get (File.File, Re, Width);
            Item := (Status => Success, Item => Re);
            File.Status := Success;
      end case;
   exception
   ...
   begin
      case File.Sort is
         when Std_In =>
            File.Status := Mode_Error;
         when Std_Out =>
            Ada_Fixed_IO.Put (Ada.Text_IO.Standard_Output,
                              Item, Fore, Aft, Exp);
            File.Status := Success;
         when Std_Error =>
            Ada_Fixed_IO.Put (Ada.Text_IO.Standard_Error,
                              Item, Fore, Aft, Exp);
            File.Status := Success;
         when A_File =>
            Ada_Fixed_IO.Put (File.File,
                              Item, Fore, Aft, Exp);
            File.Status := Success;
      end case;
   exception
   ...
   procedure Put (Item : in Num;
                  ...
                  Exp  : in Field := Default_Exp) is
   begin
      Ada_Fixed_IO.Put (Ada.Text_IO.Standard_Output, Item, Fore, Aft, Exp);
